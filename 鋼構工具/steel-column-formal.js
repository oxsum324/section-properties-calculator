(function initSteelColumnFormal() {
  ...
  function setInputStatus(message) {
    const inputStatus = $("columnInputStatus");
    ...
    const reminders = [
      {
        ...
        title: "規範檢核摘要",
        body: overallOk
          ? "本次鋼柱於細長比、軸壓與軸力—彎矩互制檢核均通過，可產生計算書預覽。"
          : "本頁已同步提供計算書預覽、計算流程與控制項目；若有 NG，請優先檢視 KL/r、Fcr 與互制比。",
      },
      {
        level: "info",
        title: "控制軸向",
  ...
  function runCheck() {
    ...
    if (sectionType === "h") {
      ...
      if (clsC.isSlender) {
        return inputFail("目前方管正式檢核僅支援非細長 SHS；請調整 B/t 或改用 H 型鋼。");
      }
      ...
      if (Hmm < Bmm) {
        return inputFail("矩形管請以 H ≥ B 輸入，讓 Mux 對應強軸、Muy 對應弱軸。");
      }
      ...
      if (clsC.isSlender) {
        return inputFail("目前矩形管正式檢核僅支援非細長 RHS；請調整 H/t、B/t 或改用其他斷面。");
      }
    } else {
      ...
      if (clsC.isSlender) {
        return inputFail("目前圓管正式檢核僅支援非細長 CHS；請調整 D/t 或改用 H 型鋼。");
      }
    }
  ...
  function buildColumnReportConfig(result) {
  ...
  function collectColumnSourceFields() {
    const fields = {};
  ...
  function buildColumnSourcePayload(result = runCheck()) {
    if (!result) return null;
    const reportConfig = buildColumnReportConfig(result);
  ...
  function exportColumnSourceJson() {
    const payload = buildColumnSourcePayload();
    if (!payload) return;
