...
function exampleInput() {
  return {
    ...
    steel: { catalogId: 'rh-500x304x15x24', grade: 'A572 Gr.50', fysKgfCm2: 3500, esKgfCm2: 2_040_000 },
    ...
    detailing: {
      ...
      seismicColumnShearSubcheck: false, seismicStrongColumnWeakBeamSubcheck: false,
      seismicConfinementSubcheck: false, redistributeToSteelBoundary: true,
...
assert.equal(Page.PAGE_VERSION, 'v0.1');
...
assert.equal(config.inputs.length, 3);
assert.equal(config.diagrams.length, 1);
assert.equal(config.checks.flatMap(group => group.items).length, 6);
...
for (const needle of [
  'SRC 柱耐震軸向研究核算計算書', '產出工具', '工具版本', '計算引擎', '計算指紋',
  '規範、構材與分析條件', '採用斷面與材料', '第 9.3 節採用地震軸力資料',
  'SRC 柱計算斷面', '計算過程明細', '檢核結論',
