...
`結構工具箱/tools/calculation-book-content-boundary.json` is the single versioned source for universal forbidden wording. HTML, PDF, DOCX, XLSX and attachment-package checks consume that contract so a family-specific test cannot silently retain a retired governance section.
