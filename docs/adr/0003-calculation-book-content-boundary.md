...
`結構工具箱/tools/calculation-book-content-boundary.json` is the single versioned source for both universal forbidden wording and positive calculation-book content groups; `calculation-book-content-boundary.js` is the shared evaluator used by release evidence and daily attachment-package checks. A calculation-book artifact must contain recognizable adopted inputs, a calculation or check process, and an engineering result. An explicitly titled calculation summary may omit repeated detailed equations, but must still contain adopted inputs and an engineering result. Native tool reports additionally require the tool name, version, output time, and calculation fingerprint. HTML, PDF, DOCX, XLSX and attachment-package checks consume that contract so a family-specific test cannot silently retain a retired governance section or release an empty title-and-status shell. Attachment HTML is evaluated from visible static content; script, style, template, and noscript bodies cannot satisfy the engineering-content gate. Source JSON remains provenance data and is not treated as a calculation book.

Current v3 formal attachment packages are re-parsed during both pre-publication self-verification and later read-only verification. The verifier re-runs the content boundary and document-state checks against the packaged artifacts, pairs packaged source JSON with reports, and compares tool, version, output time, approval time, and calculation fingerprints extracted from the files with the manifest. Recomputing file hashes, the manifest, and the package fingerprint cannot make an empty calculation shell or inconsistent metadata acceptable. Legacy v1/v2 packages retain their original integrity-only review semantics and require re-export into a current package.
