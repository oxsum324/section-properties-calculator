...
| `結構工具箱/tools/delivery-artifacts.contract.test.js` | 納入 | 交付物一致性契約；鎖住覆工板 JSON / Word 報表與開挖擋土支撐 PDF / DOCX / latest download API 的 traceability、README、smoke fixture、報表產生器、前端狀態與下載邊界，並由平台 preflight 的 `delivery-artifacts-contract` 直接執行。 |
