...
| `結構工具箱/tools/delivery-artifacts.contract.test.js` | 納入 | 交付物一致性契約；鎖住石材 audit JSON / Word / PDF、覆工板 JSON / Word 報表與開挖擋土支撐 PDF / DOCX / latest download API 的 traceability、README、smoke fixture、報表產生器、前端狀態與下載邊界，並由平台 preflight 的 `delivery-artifacts-contract` 直接執行。 |
| `結構工具箱/tools/release-readiness.contract.test.js` | 納入 | 正式放行證據契約；以 `release-readiness-contract` 確認 release wrapper 固定使用 `ForceSlowChecks` 與 `ForcePlatformAudit` 強制慢測與平台 audit，preflight summary / history / homepage status 保留 force flags、慢測重用與平台 audit 重用資訊，並讓儀表板把正式放行 run 標示為 Release。 |
...
| `鋼筋混凝土/tools/rc-traceability.catalog.json`、`鋼筋混凝土/tools/rc-traceability.contract.test.js`、`鋼筋混凝土/tools/beam-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-beam.ps1`、`鋼筋混凝土/tools/column-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-column.ps1`、`鋼筋混凝土/tools/slab-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-slab.ps1`、`鋼筋混凝土/tools/wall-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-wall.ps1`、`鋼筋混凝土/tools/shear-wall-report-visual.test.js`、`鋼筋混凝土/tools/shear-wall-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-shear-wall-report.ps1`、`鋼筋混凝土/tools/foundation-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-foundation.ps1`、`鋼筋混凝土/tools/single-pile-report-visual.contract.test.js`、`鋼筋混凝土/tools/test-single-pile.ps1` | 納入 | RC 梁、柱、板、牆、剪力牆、基礎與單樁的條文語意追蹤 catalog、各正式工具報告視覺 smoke 契約與 wrapper；集中確認規範來源、輸入、計算核心、報告落點、regression / browser smoke 證據與人工複核邊界。梁、板、牆、基礎與單樁報告視覺 smoke 分別由對應 `*-report-visual.contract.test.js` 固定案例、page-only 附件閱讀狀態邊界與輸出證據檔名，再由各自 `test-*.ps1` 執行完整 regression 與視覺 smoke；柱報告視覺 smoke 由 `test-column.ps1` 包裝並透過平台 preflight 的 `rc-column-report-contract` 執行；剪力牆報告視覺 smoke 則由 `shear-wall-report-visual.contract.test.js` 固定案例與頁面 / 計算書邊界，再由 `test-shear-wall-report.ps1` 包裝並透過 `rc-shear-wall-report-contract` 執行，固定檢查頁面上的附件閱讀狀態不會寫入計算書或列印 PDF。 |
...
| `結構工具箱/assets/status/` | 納入 | GitHub Pages 可讀的首頁公開狀態快照；由 `tool-maturity-matrix.js --write` 從 git ignored `output/` 精簡產生，只保留狀態、runId、失敗數、筆數、耗時與 sourceHash 等公開欄位，不保留本機絕對路徑。首頁健康卡需直接用這組快照顯示 `完整檢查` / `快速檢查` / `正式放行` 與 `runId`，避免讀者把 quick run 誤判成正式交付證據。 |
