import{a as e,i as t,t as n}from"./appMeta-BENjzoK_.js";import{a as r,i,r as a}from"./basePlateStressState-wYduPT6T.js";import{c as o,o as s}from"./units-m4ngsLPL.js";import{a as c,i as l,n as u,o as d,t as f}from"./index-BwHKry3F.js";import{i as p,n as ee,r as te,t as ne}from"./reportHtmlSeal-hBwqWEKZ.js";import{t as re}from"./seismicRouteGuidance-CA5XgnZ0.js";function m(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function h(e){return Number.isFinite(e)?new Intl.NumberFormat(`zh-TW`,{maximumFractionDigits:2}).format(e):`—`}function g(e){switch(e){case`pass`:return`符合`;case`fail`:return`不符合`;case`screening`:return`初篩`;case`incomplete`:return`需補資料`;case`warning`:return`提醒`;default:return e}}function _(e){switch(e){case`cast_in`:return`預埋錨栓`;case`post_installed_expansion`:return`後置膨脹錨栓`;case`post_installed_bonded`:return`後置黏結式錨栓`;case`screw_anchor`:return`螺紋錨栓`;case`undercut_anchor`:return`擴底式錨栓`;case`shear_lug`:return`剪力榫`;default:return e}}function v(e){return e===`summary`?`摘要版`:`完整明細版`}function y(e,t){return`${t} ${e}`}function b(e){if(!e)return`—`;let t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleDateString(`zh-TW`,{year:`numeric`,month:`2-digit`,day:`2-digit`})}function x(e){if(!e)return`—`;let t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(`zh-TW`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`})}function S(e,t=12){return e?e.slice(0,Math.max(8,t)).toUpperCase():`—`}function C(e){switch(e){case`manual`:return`手動留存`;case`preview`:return`報表預覽`;case`print`:return`列印報表`;case`html`:return`匯出 HTML`;case`xlsx`:return`匯出 XLSX`;case`docx`:return`匯出 DOCX`;default:return`—`}}function w(e){return e===`product`?`產品值`:e===`code_fallback`?`規範退回值`:`規範值`}function T(e,t,n){return Number.isFinite(e)?`${h(o(e,t,n))} ${s(t,n)}`:`—`}function E(e,t){let n=e.anchorCountX*e.anchorCountY,r=Math.min(e.edgeLeftMm,e.edgeRightMm,e.edgeBottomMm,e.edgeTopMm);return[`${e.anchorCountX} × ${e.anchorCountY}（${n} 支）`,`hef ${T(e.effectiveEmbedmentMm,`length`,t)}`,`sx ${T(e.spacingXmm,`length`,t)}`,`sy ${T(e.spacingYmm,`length`,t)}`,`cmin ${T(r,`length`,t)}`].join(` / `)}function D(e,t,n){return e.presentation===`ratio`?h(t):e.presentation===`stress`?T(t,`stress`,n):e.presentation===`length`?T(t,`length`,n):T(t,`force`,n)}function ie(e){return!e.factors||e.factors.length===0?`—`:e.factors.map(e=>e.note?`${e.symbol}=${e.value}（${e.label}；${e.note}）`:`${e.symbol}=${e.value}（${e.label}）`).join(`；`)}function O(e){return e.governingDcr??e.maxDcr}function k(e){if(e.project.layout.basePlateLoadedAreaMm2>0)return e.project.layout.basePlateLoadedAreaMm2;let t=Math.max(0,e.project.layout.basePlateLoadedWidthMm??0),n=Math.max(0,e.project.layout.basePlateLoadedHeightMm??0);return t>0&&n>0?t*n:0}function ae(e){return e.project.layout.basePlateSectionType===`custom`?`custom`:`rectangle`}function A(e){return e===`custom`?`自訂 Sx / Sy`:`矩形承壓區`}function j(e){switch(e){case`i_h`:return`I / H 形柱`;case`rect`:return`矩形柱`;case`pipe`:return`圓管 / 圓柱`;default:return`手動輸入`}}function M(e){switch(e){case`ready`:return`pass`;case`configuration_issue`:return`warning`;default:return`incomplete`}}function N(e){return`zone-${e}`}function P(e){return`<span class="chip chip-${e}">${m(g(e))}</span>`}function F(e,t){let{layout:n}=e.project,r=d(n,e.anchorPoints),i=c(n,e.analysisLoads),a=new Map,o=new Map(e.visualization.anchors.map(e=>[e.anchorId,e]));e.visualization.edges.forEach(e=>{let t=a.get(e.edge)??[];t.includes(e.label)||t.push(e.label),a.set(e.edge,t)});let s=e.visualization.rectangles.map(e=>`<rect x="${e.x1}" y="${e.y1}" width="${e.x2-e.x1}" height="${e.y2-e.y1}" class="zone ${N(e.kind)}" />`).join(``),l=i?`<g>
        ...
        >${m(i.label)}</text>
      </g>`:``,u=e.anchorPoints.map(e=>{let t=o.get(e.id),n=t?.state===`tension`?`anchor-tension`:t?.state===`compression`?`anchor-compression`:`anchor-neutral`,r=t?`${t.elasticTensionKn>=0?`+`:``}${h(t.elasticTensionKn)}`:`0`;return`<g>
  ...
  .anchor-center { fill:#fff; }`;function L(e,t){return F(e,t).replace(/<svg([^>]*)>/,(e,t)=>`<svg${t} xmlns="http://www.w3.org/2000/svg"><defs><style>${I}</style></defs>`)}function R(o){let{batchReview:c,candidateProductReviews:d,layoutVariantReviews:g=[],review:N,selectedProduct:I,completeness:L,evaluationFieldStates:R,unitPreferences:z,reportSettings:B,auditEntry:V,auditTrail:H=[],autoPrint:oe=!1,reportGeneratedAt:U=new Date().toISOString()}=o,W=e(N.project.calcEngineVersion),G=u({batchReview:c,review:N,completeness:L,reportSettings:B}),K=V?.hash?`CF-${S(V.hash,16)}`:``,se=W.mismatch?`本案原始版本 ${W.projectVersion}，目前以 ${W.runtimeVersion} 重算；正式交付前應重新檢核並留痕。`:`本案計算版本與目前工具版本一致：${W.runtimeVersion}`,ce=new Set(N.project.excludedCheckIds??[]),q=N.results.filter(e=>!ce.has(e.id)),J=R.filter(e=>e.hasValue||e.hasEvidence),le=q.filter(e=>e.factors&&e.factors.length>0),ue=q.find(e=>e.id===`seismic`)??null,Y=N.project.loads.considerSeismic?re(N.project.loads,I,ue):null,X=ae(N),Z=a(N.project.layout),Q=r(N.project.layout),$=i(N.project.layout),de=c.loadCaseReviews.map(e=>{let t=d.map(t=>{let n=t.batchReview.loadCaseReviews.find(t=>t.loadCaseId===e.loadCaseId);return n?`<td>
...
<html lang="zh-Hant">
  ...
  <body>
    <div class="report-preview-toolbar" data-role="preview-toolbar">
      <span class="preview-approval-control">工作頁核可狀態：${m(G.label)}</span>
    ...
    <span
      ...
      data-report-title="${m(N.project.name)}"
      data-calculation-fingerprint="${m(K)}"
      data-approved="${G.status===`formal-attachment`?`true`:`false`}"
    ...
    <main>
      ...
      ${d.length>1?`<section class="card">
              ...
              <table>
                <thead>
                  <tr>
                    <th>載重組合</th>
                    ${d.map(e=>`<th>${m(e.product.brand)} ${m(e.product.model)}</th>`).join(``)}
                  </tr>
                </thead>
                <tbody>${de}</tbody>
              </table>
      ...
      <section class="card">
        ...
        <table>
          ...
          <tbody>
            ${q.map(e=>`<tr>
                  <td>${m(e.mode)}<br /><small class="meta">${m(l(e,z))}</small></td>
                  <td>${m(y(e.citation.title,e.citation.clause))}</td>
