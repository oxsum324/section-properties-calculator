import{a as e,i as t,t as n}from"./appMeta-Ca9NpYgV.js";import{a as r,i,r as a}from"./basePlateStressState-wYduPT6T.js";import{c as o,o as s}from"./units-m4ngsLPL.js";import{a as c,i as l,n as u,r as d,t as f}from"./index-DsOCxyBb.js";import{t as p}from"./seismicRouteGuidance-CMiH4g__.js";function m(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function h(e){return Number.isFinite(e)?new Intl.NumberFormat(`zh-TW`,{maximumFractionDigits:2}).format(e):`—`}function g(e){switch(e){case`pass`:return`符合`;case`fail`:return`不符合`;case`screening`:return`初篩`;case`incomplete`:return`需補資料`;case`warning`:return`提醒`;default:return e}}function _(e){switch(e){case`cast_in`:return`預埋錨栓`;case`post_installed_expansion`:return`後置膨脹錨栓`;case`post_installed_bonded`:return`後置黏結式錨栓`;case`screw_anchor`:return`螺紋錨栓`;case`undercut_anchor`:return`擴底式錨栓`;case`shear_lug`:return`剪力榫`;default:return e}}function v(e){return e===`summary`?`摘要版`:`完整明細版`}function y(e,t){return`${t} ${e}`}function b(e){if(!e)return`—`;let t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleDateString(`zh-TW`,{year:`numeric`,month:`2-digit`,day:`2-digit`})}function x(e){if(!e)return`—`;let t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleString(`zh-TW`,{year:`numeric`,month:`2-digit`,day:`2-digit`,hour:`2-digit`,minute:`2-digit`})}function S(e,t=12){return e?e.slice(0,Math.max(8,t)).toUpperCase():`—`}function C(e){switch(e){case`manual`:return`手動留存`;case`preview`:return`報表預覽`;case`print`:return`列印報表`;case`html`:return`匯出 HTML`;case`xlsx`:return`匯出 XLSX`;case`docx`:return`匯出 DOCX`;default:return`—`}}function ee(e){return e===`product`?`產品值`:e===`code_fallback`?`規範退回值`:`規範值`}function w(e,t,n){return Number.isFinite(e)?`${h(o(e,t,n))} ${s(t,n)}`:`—`}function T(e,t){let n=e.anchorCountX*e.anchorCountY,r=Math.min(e.edgeLeftMm,e.edgeRightMm,e.edgeBottomMm,e.edgeTopMm);return[`${e.anchorCountX} × ${e.anchorCountY}（${n} 支）`,`hef ${w(e.effectiveEmbedmentMm,`length`,t)}`,`sx ${w(e.spacingXmm,`length`,t)}`,`sy ${w(e.spacingYmm,`length`,t)}`,`cmin ${w(r,`length`,t)}`].join(` / `)}function E(e,t,n){return e.presentation===`ratio`?h(t):e.presentation===`stress`?w(t,`stress`,n):e.presentation===`length`?w(t,`length`,n):w(t,`force`,n)}function D(e){return!e.factors||e.factors.length===0?`—`:e.factors.map(e=>e.note?`${e.symbol}=${e.value}（${e.label}；${e.note}）`:`${e.symbol}=${e.value}（${e.label}）`).join(`；`)}function O(e){return e.governingDcr??e.maxDcr}function k(e){if(e.project.layout.basePlateLoadedAreaMm2>0)return e.project.layout.basePlateLoadedAreaMm2;let t=Math.max(0,e.project.layout.basePlateLoadedWidthMm??0),n=Math.max(0,e.project.layout.basePlateLoadedHeightMm??0);return t>0&&n>0?t*n:0}function A(e){return e.project.layout.basePlateSectionType===`custom`?`custom`:`rectangle`}function j(e){return e===`custom`?`自訂 Sx / Sy`:`矩形承壓區`}function te(e){switch(e){case`i_h`:return`I / H 形柱`;case`rect`:return`矩形柱`;case`pipe`:return`圓管 / 圓柱`;default:return`手動輸入`}}function ne(e){switch(e){case`ready`:return`pass`;case`configuration_issue`:return`warning`;default:return`incomplete`}}function M(e){return`zone-${e}`}function N(e){return`<span class="chip chip-${e}">${m(g(e))}</span>`}function P(e,t){let{layout:n}=e.project,r=c(n,e.anchorPoints),i=l(n,e.analysisLoads),a=new Map,o=new Map(e.visualization.anchors.map(e=>[e.anchorId,e]));e.visualization.edges.forEach(e=>{let t=a.get(e.edge)??[];t.includes(e.label)||t.push(e.label),a.set(e.edge,t)});let s=e.visualization.rectangles.map(e=>`<rect x="${e.x1}" y="${e.y1}" width="${e.x2-e.x1}" height="${e.y2-e.y1}" class="zone ${M(e.kind)}" />`).join(``),u=i?`<g>
    ...
    <text x="16" y="${n.concreteHeightMm-14}" class="sketch-meta">${m(`${w(n.concreteWidthMm,`length`,t)} × ${w(n.concreteHeightMm,`length`,t)}`)}</text>
  ...
  .anchor-center { fill:#fff; }`;function I(e,t){return P(e,t).replace(/<svg([^>]*)>/,(e,t)=>`<svg${t} xmlns="http://www.w3.org/2000/svg"><defs><style>${F}</style></defs>`)}function L(o){let{batchReview:c,candidateProductReviews:l,layoutVariantReviews:g=[],review:M,selectedProduct:F,completeness:I,evaluationFieldStates:L,unitPreferences:R,reportSettings:z,auditEntry:B,auditTrail:V=[],autoPrint:H=!1,reportGeneratedAt:U=new Date().toISOString()}=o,W=e(M.project.calcEngineVersion),G=u({batchReview:c,review:M,completeness:I,reportSettings:z}),K=B?.hash?`CF-${S(B.hash,16)}`:``,re=W.mismatch?`本案原始版本 ${W.projectVersion}，目前以 ${W.runtimeVersion} 重算；正式交付前應重新檢核並留痕。`:`本案計算版本與目前工具版本一致：${W.runtimeVersion}`,ie=new Set(M.project.excludedCheckIds??[]),q=M.results.filter(e=>!ie.has(e.id)),J=L.filter(e=>e.hasValue||e.hasEvidence),ae=q.filter(e=>e.factors&&e.factors.length>0),oe=q.find(e=>e.id===`seismic`)??null,Y=M.project.loads.considerSeismic?p(M.project.loads,F,oe):null,X=A(M),Z=a(M.project.layout),Q=r(M.project.layout),$=i(M.project.layout),se=c.loadCaseReviews.map(e=>{let t=l.map(t=>{let n=t.batchReview.loadCaseReviews.find(t=>t.loadCaseId===e.loadCaseId);return n?`<td>
...
<html lang="zh-Hant">
  <head>
    ...
    <title>${m(M.project.name)} - 錨栓檢討報告</title>
    <style>
      @page {
        size: A4;
  ...
  <body>
    ...
    <main>
      <section class="hero">
        ...
        <p>台灣《建築物混凝土結構設計規範》112年版第17章 錨栓檢討 ${m(v(z.reportMode))}</p>
        <div class="hero-grid">
          ${z.projectCode?`<div><small class="meta">案號 / 專案</small><div>${m(z.projectCode)}</div></div>`:``}
          ${z.designer?`<div><small class="meta">設計人員</small><div>${m(z.designer)}</div></div>`:``}
          ${z.checker?`<div><small class="meta">複核人員</small><div>${m(z.checker)}</div></div>`:``}
          <div><small class="meta">規範版本</small><div>${m(M.ruleProfile.versionLabel)}</div></div>
          <div><small class="meta">發行日期</small><div>${m(b(z.issueDate))}</div></div>
          <div><small class="meta">${f.editedAt}</small><div>${m(x(M.project.updatedAt))}</div></div>
          <div><small class="meta">${f.generatedAt}</small><div>${m(x(U))}</div></div>
          <div><small class="meta">${f.auditedAt}</small><div>${m(x(B?.createdAt))}</div></div>
          <div><small class="meta">${f.auditSource} / ${f.auditHash}</small><div>${m(B?`${C(B.source)} · ${S(B.hash)}`:`尚未留存`)}</div></div>
      ...
      <section class="grid">
        <article class="card">
          ...
          <ul>
            <li>錨栓直徑 da = ${m(w(F.diameterMm,`length`,R))}</li>
            <li>Ase = ${m(w(F.effectiveAreaMm2,`area`,R))}</li>
            ...
            <li>基板承壓 = ${m(M.project.layout.basePlateBearingEnabled?`已啟用（A1 ${w(k(M),`area`,R)} / ${j(X)}${Z.widthMm>0&&Z.heightMm>0?` / B × N ${w(Z.widthMm,`length`,R)} × ${w(Z.heightMm,`length`,R)}`:``}${(M.project.layout.columnCentroidOffsetXmm??0)!==0||(M.project.layout.columnCentroidOffsetYmm??0)!==0?` / 柱偏移 ${w(M.project.layout.columnCentroidOffsetXmm??0,`length`,R)} × ${w(M.project.layout.columnCentroidOffsetYmm??0,`length`,R)}`:``}${M.project.layout.basePlateBendingEnabled?` / tp ${w(M.project.layout.basePlateThicknessMm,`length`,R)} / Fy ${w(M.project.layout.basePlateSteelYieldMpa,`stress`,R)}`:``}）`:`未啟用`)}</li>
          </ul>
        ...
        <article class="card">
          ...
          <ul>
            ...
            <li>最新留痕 = ${m(B?`${S(B.hash)} / ${C(B.source)}`:`未留存`)}</li>
          </ul>
      ...
      <section class="card">
        ...
        <ul>
          <li>產出工具 = ${m(t)}</li>
          <li>工具版本 = <code>${m(W.runtimeVersion)}</code></li>
          <li>輸出時間 = ${m(x(U))}</li>
          <li>計算指紋 = ${m(K||`—`)}</li>
          <li>版本狀態 = ${m(re)}</li>
          <li>案件計算版本 = <code>${m(W.projectVersion)}</code></li>
          ...
          <li>留痕來源 / Hash = ${m(B?`${C(B.source)} / ${S(B.hash,16)}`:`未留存`)}</li>
        </ul>
      </section>
      ...
      <section class="card">
        ...
        ${M.project.layout.basePlateBearingEnabled?`<p class="meta">基板承壓：A1 ${m(w(k(M),`area`,R))} / A2 ${m(w(M.project.layout.basePlateSupportAreaMm2,`area`,R))}${(M.project.layout.basePlateLoadedWidthMm??0)>0&&(M.project.layout.basePlateLoadedHeightMm??0)>0?` / b1 × h1 ${m(w(M.project.layout.basePlateLoadedWidthMm??0,`length`,R))} × ${m(w(M.project.layout.basePlateLoadedHeightMm??0,`length`,R))}`:``}${Z.widthMm>0&&Z.heightMm>0?` / B × N ${m(w(Z.widthMm,`length`,R))} × ${m(w(Z.heightMm,`length`,R))}`:``}${(M.project.layout.columnCentroidOffsetXmm??0)!==0||(M.project.layout.columnCentroidOffsetYmm??0)!==0?` / 柱偏移 ${m(w(M.project.layout.columnCentroidOffsetXmm??0,`length`,R))} × ${m(w(M.project.layout.columnCentroidOffsetYmm??0,`length`,R))}`:``} / ${m(j(X))}${X===`custom`?` / Sx ${m(h(M.project.layout.basePlateSectionModulusXmm3??0))} mm³ / Sy ${m(h(M.project.layout.basePlateSectionModulusYmm3??0))} mm³`:``}</p>
               <p class="meta">${X===`custom`?`若以 b1 / h1 + 自訂 Sx / Sy 進入偏心承壓應力模式，b1 / h1 僅供接觸尺寸、kern 與 uplift 判讀；彎曲應力採自訂斷面模數。`:`若以 b1 / h1 進入偏心承壓應力模式，報表目前採矩形承壓區之 Sx / Sy 假設；非矩形基板請另按實際幾何檢算。`}${$===`manual`?``:` 基板抗彎若未手填 lx / ly，可由 ${m(te($))}、B / N 與柱尺寸自動推算。`}</p>
               ${Q?`<p class="meta">AISC DG1 自動推算：m = ${m(w(Q.mMm,`length`,R))} / n = ${m(w(Q.nMm,`length`,R))} / λn' = ${m(w(Q.lambdaPrimeMm,`length`,R))} / 建議 lx = ${m(w(Q.xMm,`length`,R))} / ly = ${m(w(Q.yMm,`length`,R))}</p>`:``}`:``}
      </section>
        ...
        <table>
          ...
          <tbody>
            ${c.loadCaseReviews.map(e=>`<tr>
                  ...
                  <td>${m(w(e.review.analysisLoads.tensionKn,`force`,R))}</td>
                  <td>${m(w(Math.hypot(e.review.analysisLoads.shearXKn,e.review.analysisLoads.shearYKn),`force`,R))}</td>
              ...
              <div class="route-matrix">
                ${Y.routeMatrix.map(e=>`<article class="route-matrix-card route-matrix-card-${e.state}${e.isCurrent?` route-matrix-card-current`:``}">
                      <div class="route-matrix-head">
                        ...
                        <span class="chip chip-${ne(e.state)}">${m(e.readinessLabel)}</span>
                      </div>
      ...
      ${g.length>1?`<section class="card">
              ...
              <table>
                <thead>
                  <tr>
                    <th>載重組合</th>
                    ${g.map(e=>`<th>${m(e.variant.name)}<br /><small class="meta">${m(e.isCurrent?`目前配置`:T(e.variant.layout,R))}</small></th>`).join(``)}
                  </tr>
                </thead>
                <tbody>${ce}</tbody>
              </table>
        ...
        <table>
          ...
          <tbody>
            ${M.dimensionChecks.map(e=>`<tr>
                  <td>${m(e.label)}</td>
                  <td>${m(w(e.actualMm,`length`,R))}</td>
                  <td>${m(w(e.requiredMm,`length`,R))}</td>
                  <td>${m(ee(e.source))}</td>
        ...
        <table>
          ...
          <tbody>
            ${q.map(e=>`<tr>
                  ...
                  <td>${m(y(e.citation.title,e.citation.clause))}</td>
                  <td>${m(E(e,e.demandKn,R))}</td>
                  <td>${m(E(e,e.designStrengthKn,R))}</td>
                  <td>${m(h(e.dcr))}</td>
      ...
      ${J.length>0?`<section class="card">
              ...
              <table>
                ...
                <tbody>
                  ${J.map(e=>{let t=typeof e.rawValue==`number`&&e.quantity?w(e.rawValue,e.quantity,R):e.rawValue===void 0||e.rawValue===null||e.rawValue===``?`未填`:String(e.rawValue);return`<tr>
                        <td>${m(e.label)}</td>
                        <td>${m(t)}</td>
      ...
      <footer
        ...
        data-approved-at="${m(z.documentApprovedAt||``)}"
