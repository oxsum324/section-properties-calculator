...
assert.equal((pagesDeployWorkflow.match(/if: always\(\)/g) || []).length, 2, 'Pages deploy workflow always emits staged and live CI summaries');
assert.equal((pagesDeployWorkflow.match(/write-ci-summary\.js/g) || []).length, 2, 'Pages deploy workflow centralizes CI summary rendering');
...
assert.ok(pagesCiSummary.buildSummary({ PAGES_NPM_CACHE_HIT: 'true', PAGES_BROWSER_CACHE_HIT: 'false' }).includes('exact hit'), 'Pages CI summary renders explicit cache outputs');
