...
assert.ok(Array.isArray(reportReadinessOverview.meta) && reportReadinessOverview.meta.length === 13, 'report readiness overview exposes compact metrics');
...
assert.ok(reportReadinessOverview.meta.some(item => item.text === '開挖結果鏈'), 'report readiness overview exposes excavation result reconciliation metric');
assert.ok(reportReadinessOverview.meta.some(item => item.text === '局部快算結果鏈'), 'report readiness overview exposes local quick result reconciliation metric');
...
if (Number.isInteger(reportReadinessStatusSnapshot.rcSourceReportPackageRequired)) {
  assert.equal(reportReadinessStatusSnapshot.rcSourceReportPackageRequired, 32, 'tracked report readiness snapshot expects 32 RC source/report package checks');
  assert.equal(reportReadinessStatusSnapshot.rcSourceReportPackageComplete, reportReadinessStatusSnapshot.rcSourceReportPackageRequired, 'tracked report readiness snapshot completes every RC source/report package check');
...
if (Number.isInteger(reportReadinessStatusSnapshot.rcFormalHtmlContentSealRequired)) {
  assert.equal(reportReadinessStatusSnapshot.rcFormalHtmlContentSealRequired, 34, 'tracked report readiness snapshot expects 34 RC formal HTML content seal checks');
  assert.equal(reportReadinessStatusSnapshot.rcFormalHtmlContentSealComplete, reportReadinessStatusSnapshot.rcFormalHtmlContentSealRequired, 'tracked report readiness snapshot completes every RC formal HTML content seal check');
...
if (Number.isInteger(reportReadinessStatusSnapshot.steelHtmlContentSealRequired)) {
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatusSnapshot.steelHtmlContentSealRequired), 'tracked report readiness snapshot expects a supported 5-to-9 steel formal HTML content seal transition count');
...
if (Number.isInteger(reportReadinessStatusSnapshot.steelHtmlApprovalSealRequired)) {
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatusSnapshot.steelHtmlApprovalSealRequired), 'tracked report readiness snapshot expects a supported 5-to-9 steel formal HTML approval seal transition count');
...
if (Number.isInteger(reportReadinessStatusSnapshot.steelResultReconciliationRequired)) {
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatusSnapshot.steelResultReconciliationRequired), 'tracked report readiness snapshot expects a supported 5-to-9 steel result reconciliation transition count');
...
if (Number.isInteger(reportReadinessStatusSnapshot.stoneResultReconciliationRequired)) {
  assert.equal(reportReadinessStatusSnapshot.stoneResultReconciliationRequired, 1, 'tracked report readiness snapshot expects 1 stone result reconciliation');
  assert.equal(reportReadinessStatusSnapshot.stoneResultReconciliationComplete, reportReadinessStatusSnapshot.stoneResultReconciliationRequired, 'tracked report readiness snapshot completes the stone result reconciliation');
  assert.equal(reportReadinessStatusSnapshot.stoneResultReconciliationIssueCount, 0, 'tracked report readiness snapshot stone result reconciliation issues empty');
  assert.equal(reportReadinessStatusSnapshot.stoneResultReconciliationPass, true, 'tracked report readiness snapshot stone result reconciliation passes');
  assert.ok(reportReadinessStatusSnapshot.details.join(' ').includes('不公開 golden 案例資料、來源 payload 雜湊、結果雜湊或成品雜湊'), 'tracked report readiness snapshot keeps stone evidence private');
}
if (Number.isInteger(reportReadinessStatusSnapshot.anchorResultReconciliationRequired)) {
  assert.equal(reportReadinessStatusSnapshot.anchorResultReconciliationRequired, 1, 'tracked report readiness snapshot expects 1 anchor result reconciliation');
  assert.equal(reportReadinessStatusSnapshot.anchorResultReconciliationComplete, reportReadinessStatusSnapshot.anchorResultReconciliationRequired, 'tracked report readiness snapshot completes the anchor result reconciliation');
  assert.equal(reportReadinessStatusSnapshot.anchorResultReconciliationIssueCount, 0, 'tracked report readiness snapshot anchor result reconciliation issues empty');
...
if (Number.isInteger(reportReadinessStatusSnapshot.localQuickResultReconciliationRequired)) {
  ...
  assert.ok(reportReadinessStatusSnapshot.details.join(' ').includes('局部快算計算書結果鏈'), 'tracked report readiness snapshot exposes local quick result reconciliation');
}
...
assert.ok(homeSource.includes("ratio('局部快算結果鏈'"), 'home status renders local quick result reconciliation count');
...
assert.ok(pagesDeployWorkflow.includes('actions/download-artifact@v8') && pagesDeployWorkflow.includes('pages-ci-performance-trend'), 'Pages deploy workflow aggregates private P50/P95 evidence');
...
assert.equal((pagesDeployWorkflow.match(/uses: actions\/cache@v5/g) || []).length, 4, 'Pages deploy workflow restores explicit npm and Playwright caches in both browser jobs');
...
assert.equal((pagesDeployWorkflow.match(/path: ~\/\.npm/g) || []).length, 2, 'Pages deploy workflow explicitly caches npm content twice');
assert.equal((pagesDeployWorkflow.match(/runner\.os }}-\${{ runner\.arch }}-pages-npm-\${{ hashFiles\('\.github\/pages-smoke\/package-lock\.json'\) }}/g) || []).length, 2, 'Pages deploy workflow binds npm cache identity to platform and lockfile');
...
assert.ok(pagesDeployWorkflow.includes('PAGES_HTTP_REQUEST_ATTEMPTS: 4') && pagesDeployWorkflow.includes('PAGES_HTTP_REQUEST_RETRY_DELAY_MILLISECONDS: 1000'), 'Pages deploy workflow bounds isolated live HTTP request retries');
assert.ok(pagesDeployWorkflow.includes('PAGES_HTTP_REQUEST_TIMEOUT_MILLISECONDS: 15000'), 'Pages deploy workflow bounds isolated live HTTP request duration');
assert.ok(pagesDeployWorkflow.includes('PAGES_HTTP_SMOKE_ATTEMPTS: 2') && pagesDeployWorkflow.includes('PAGES_HTTP_SMOKE_RETRY_DELAY_SECONDS: 5'), 'Pages deploy workflow bounds full-inventory live HTTP transient retries');
...
assert.equal((pagesDeployWorkflow.match(/if: always\(\)/g) || []).length, 6, 'Pages deploy workflow always prepares, uploads, and summarizes staged and live evidence');
assert.equal((pagesDeployWorkflow.match(/write-ci-summary\.js/g) || []).length, 4, 'Pages deploy workflow centralizes CI evidence preparation and summary rendering');
assert.ok(pagesDeployWorkflow.indexOf('- name: Upload GitHub Pages artifact') < pagesDeployWorkflow.indexOf('- name: Publish build CI evidence'), 'Pages build summary observes archive and upload outcomes');
assert.equal(pagesPerformanceBudget.mode, 'warning-only', 'Pages performance evidence remains advisory');
assert.equal(pagesPerformanceBudget.basis.sampleRunIds.length, 3, 'Pages performance evidence is based on three same-commit warm runs');
assert.equal((pagesDeployWorkflow.match(/retention-days: 14/g) || []).length, 3, 'Pages staged, live, and trend CI receipts are retained for fourteen days');
assert.ok(pagesDeployWorkflow.indexOf('- name: Upload build CI evidence') < pagesDeployWorkflow.indexOf('- name: Publish build CI evidence'), 'Pages build summary includes receipt upload health');
assert.ok(pagesDeployWorkflow.indexOf('- name: Upload live CI evidence') < pagesDeployWorkflow.indexOf('- name: Publish live CI evidence'), 'Pages live summary includes receipt upload health');
assert.ok(pagesLiveSmoke.includes("'.github/pages-smoke/write-ci-summary.js'"), 'Pages live smoke protects the summary generator from publication');
assert.ok(pagesLiveSmoke.includes("'.github/pages-smoke/performance-budget.json'"), 'Pages live smoke protects the performance budget from publication');
