...
assert.ok([31, 32, 33].includes(reportReadinessStatusSnapshot.renderedDeliveryEvidenceRequired), 'tracked report readiness snapshot covers a supported formal homepage portfolio');
...
assert.ok([135, 137, 139, 141, 143, 145].includes(reportReadinessStatusSnapshot.deliveryFileIntegrityRequired), 'tracked report readiness snapshot exposes a supported delivery-file transition count');
...
assert.ok(staging.includes('首頁正式工具實際交付物渲染 `33/33`'), 'STAGING_GROUPS records homepage rendered delivery evidence health');
