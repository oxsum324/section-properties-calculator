...
$attachmentPackageUpgradeWorkspaceCommand = @'
node 結構工具箱/tools/attachment-package-upgrade-workspace.test.js
exit $LASTEXITCODE
'@
...
$localQuickRunnerCache = [pscustomobject]@{
  roots = @(
    ...
    '結構工具箱\tools\equipment',
    '結構工具箱\tools\earth',
    '結構工具箱\tools\floor-slab',
    '結構工具箱\tools\cable-tension',
    '結構工具箱\tools\rc',
...
$checks = @(
  ...
  [pscustomobject]@{
    key = "attachment-package-upgrade-workspace"
  ...
  [pscustomobject]@{
    ...
    label = "Frame analysis interactive closed-form benchmark and report boundary contract"
    workdir = $root
