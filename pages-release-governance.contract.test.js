...
assert.ok(staging.includes('## A0. 報告閱讀狀態與 Pages release governance'), 'A0 staging package exists');
assert.ok(staging.includes('低風險可整檔 staging 的檔案'), 'A0 separates low-risk whole-file staging');
assert.ok(staging.includes('需要人工 hunk review'), 'A0 marks broad docs/contracts for hunk review');
assert.ok(staging.includes('不要混入本包'), 'A0 lists excluded neighboring work');
