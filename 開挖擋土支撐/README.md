...
前端會驗證 schema、目標種類、單位、來源計算指紋、`CSH-` 交接指紋及「必須明確採用、不得自動套用」邊界；後端不信任前端摘要，會再次重算 `CSH-`，核對控制軸力、來源欄位、目標柱 ID、重複階段及重複來源。每一階段均以 `N = Np + N1 + N2 + N3 + N4` 完整重算，並強制加入「無施工構台荷重基準案」；柱互制、基礎壓入及基礎拉拔分別取其最不利控制階段，PDF / DOCX 計算書列出階段結果、三種控制階段及精簡來源追溯。中間柱、柱位不符、缺少原始交接記錄、指紋不符、控制軸力不一致、重複來源或追溯欄位不完整時均失敗關閉。舊案只有單一 `construction_stage_load_t` 時會自動移轉成一個可追溯階段，維持既有專案相容性。

固定柱位與多階段包絡解決的是「資料不誤套」及「各已匯入配置逐案取最不利值」；哪一支柱實際承受反力、反力是否分配到多柱、各階段偏心與彎矩、支撐內力變化及臨時傳力路徑，仍須依施工配置與正式分析人工判定。
