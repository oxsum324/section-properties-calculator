...
完成最新計算後，可在「報表匯出」頁一鍵建立 `excavation-removal-transfer-handoff` JSON。後端會重新計算並只納入來源構件非 NG、生命週期完整且已確認拆撐處置的列；檔案保存來源計算指紋、逐列 `ERT-` 識別與整包 `ERH-` 指紋，並分開列示外部分析控制軸力及包含其他構件設計效應的設計軸力。所有交接列固定從 `pending` 開始，且不得自動套用或自動標示為承接構造已驗證；樓版、重撐、永久結構或其他接收端仍須以相同交接指紋完成方向、分配、偏心、載重組合及容量檢核後回簽。
