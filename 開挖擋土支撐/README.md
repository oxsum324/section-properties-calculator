...
外部分析檔的支撐或斜撐會分別保存安裝階段、控制內力階段候選及可辨識的拆撐階段；`.o` 包絡摘要所列的並列控制階段會結構化保存，不再誤把 BUT 設置階段當成控制內力階段。匯入列必須填寫實際施工步驟與階段對應依據，並勾選確認時序相符；未確認、候選階段重複、控制階段不是最大包絡值、採用內力不一致、控制早於安裝，或拆撐未晚於控制時，後端一律以「資料未完整」拒絕完成檢核。舊匯入列若缺少安裝階段須重新匯入；手動輸入列不受影響。

PDF／DOCX 計算書只列已採用的內力來源、安裝／控制／拆撐階段、實際施工步驟、對應依據及控制階段候選值；尚未確認、重新套用等操作提示只留在 HTML 工作頁。工具不會因辨識到樓版或拆撐事件就推定拆撐後已完成荷重轉移；樓版、重撐或其他結構的實際傳力路徑，仍須由設計者依正式模型、施工順序圖與專案文件複核。
