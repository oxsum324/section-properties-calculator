...
前端會驗證 schema、目標種類、單位、來源計算指紋、`CSH-` 交接指紋及「必須明確採用、不得自動套用」邊界；後端不信任前端摘要，會再次重算 `CSH-`，核對控制軸力、來源欄位、目標柱 ID、重複階段及重複來源。覆工板來源只證明軸力，不自行推定開挖柱座標方向；如施工配置存在額外傳力偏心，須在對應階段勾選「明確採用本階段附加傳力偏心」，輸入帶正負號的 `Δex / Δey` 與採用依據。後端依 `ΔMx = Np × Δex`、`ΔMy = Np × Δey` 與既有柱彎矩帶正負號組合，彎曲應力使用最終彎矩絕對值。未勾選卻夾帶非零偏心、已勾選但偏心均為零或缺少採用依據時均失敗關閉。

每一階段均以 `N = Np + N1 + N2 + N3 + N4` 完整重算，並強制加入「無施工構台荷重基準案」；柱互制、基礎壓入及基礎拉拔分別取其最不利控制階段，PDF / DOCX 計算書列出階段軸力、附加偏心、`ΔMx / ΔMy`、最終 `Mx / My`、三種控制階段及精簡來源追溯。中間柱、柱位不符、缺少原始交接記錄、指紋不符、控制軸力不一致、重複來源或追溯欄位不完整時均失敗關閉。舊案只有單一 `construction_stage_load_t` 時會自動移轉成一個不採用附加偏心的可追溯階段，維持既有專案相容性。

固定柱位、多階段包絡及逐階段附加偏心解決的是「資料不誤套」及「各已匯入配置按明確採用值逐案取最不利結果」；哪一支柱實際承受反力、反力是否分配到多柱、偏心座標正負方向、支撐內力變化及臨時傳力路徑，仍須依施工配置與正式分析人工判定。
