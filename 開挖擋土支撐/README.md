...
- 匯入支撐／斜撐逐階段內力包絡、控制階段與實際施工步驟的明確對應
...
外部分析檔中的同一支撐或斜撐若跨多個分析階段出現，工具會保留所有逐階段軸力候選，並以最大軸力所在階段作為控制分析階段，不再把多個階段名稱合併後失去控制來源。匯入列必須填寫實際施工步驟與階段對應依據，並勾選確認兩者相符；未確認、候選階段重複、控制階段不是最大包絡值，或目前採用內力與控制階段軸力不一致時，後端一律以「資料未完整」拒絕完成檢核。手動輸入列與既有手動專案維持相容，不會被要求補填外部分析對應資料。

PDF／DOCX 計算書只列已採用的內力來源、控制分析階段、實際施工步驟、對應依據及逐階段候選值；尚未確認、重新套用等操作提示只留在 HTML 工作頁。工具能封閉驗證候選值、最大控制值與採用內力的一致性，但分析模型階段是否真實對應施工現場的支撐施作、拆撐或重撐時點，仍由設計者依正式模型、施工順序圖與專案文件複核。
