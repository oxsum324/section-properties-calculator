...
樓版、重撐、永久結構或其他接收端完成方向、分配、偏心、載重組合及容量檢核後，可回傳 `receiver-capacity-verification-receipt` JSON。來源端只在 `RVR-` 指紋、來源計算指紋、ERH／ERT 關聯、全部交接列、採用需求值、容量利用率、逐列結論與彙整筆數一致時保存回簽；容量利用率大於 1 的列不得標示為 `passed`。RVR 指紋只證明回簽內容未遭修改，不是數位簽章；HTML 工作頁會分開顯示接收端結果及「回簽人身分待核對」，正式採用前仍須與回簽單位、人員及正式檢核文件人工核對。回簽資料屬工作流程治理狀態，不會插入 PDF／DOCX 計算書正文。
