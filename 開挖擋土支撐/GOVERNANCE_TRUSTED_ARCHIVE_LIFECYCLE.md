...
案件數量增加後，可執行 `檢查多案件外部歸檔生命週期.bat` 即時完整重驗案件群內全部 GSC，或以 `建立多案件外部歸檔生命週期總覽快照.bat` 建立封閉 JSON／HTML GSP。總覽會依相同保存端、保存庫、物件與不可變版本分鏈，去除相同檢查點複本，列出較舊歷史，並在下次重驗或保留期限接近時顯示 `upcoming`；相同最新時間衝突、不同 GAP 摘要、無效包或掃描中異動都會阻擋。GSP 自我驗證不會重新驗證來源，因此不能把舊總覽當成目前狀態；完整規則見 `GOVERNANCE_TRUSTED_ARCHIVE_LIFECYCLE_PORTFOLIO.md`。
