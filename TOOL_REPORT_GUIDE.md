...
- 使用者勾選「本計算內容已完成審閱，核可作為正式附件」後，文件改為 `文件狀態：正式附件`，並記錄核可時間與計算指紋。核可人與核可依據均為選填；留空不降低附件身分，也不得形成 NG 或 DRAFT。核可後若修改任一選填核可紀錄，須立即撤銷正式狀態、清除舊核可時間並回到內部審閱，待使用者確認後重新勾選，形成新的核可時間。核可是文件身分，不等於工程結果 PASS；正式附件仍可如實呈現 NG。
...
- 計算輸入、採用斷面、荷重或其他會影響結果的資料變更後，既有核可立即失效，下一份輸出回到內部審閱。
