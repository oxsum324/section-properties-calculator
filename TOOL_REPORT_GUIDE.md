...
- 預覽中的「下載目前版本 HTML」須保存一條不必執行 JavaScript 即可靜態辨識的文件狀態、核可時間與計算指紋；重新開啟下載檔時須沿用該狀態列並重建核可控制，不得重複插入或退回錯誤狀態。分頁標題須使用穩定的計算書名稱、目前文件狀態與計算指紋，使瀏覽器列印／存 PDF 的預設檔名可直接區分內部審閱與正式附件；反覆切換或重新開啟不得重複串接狀態文字。
