...
Schema v14 再加入群樁 X／Y 向側向荷重分配結果鏈。計算書直接列出採用 p-multiplier、各列分配力、每樁水平力與力平衡結果；專業 p-y JSON 只在 schema、固定單位、樁群幾何、分析範圍及分析 Hx／Hy 完全相符且經工程師勾選採用後，列入位移、剪力、彎矩、容量、分析識別與來源 SHA-256。LPile／FB-MultiPier 類表格轉換只接受已選定單位 profile 的明示欄名，不猜測專有報表文字；取深度 0 樁頭位移與全深度最大絕對剪力、彎矩，並保存各向列數及表格 SHA-256。代表單樁範圍須對應 p-multiplier 最大單樁荷重，群樁範圍才對應整組 Hx／Hy。候選檔、核對提示及不相容原因只留在頁面；改動模型即失敗封閉。未採用相符結果時附件的側向分析檢核維持待確認，RC 結果鏈維持 `34/34`。
