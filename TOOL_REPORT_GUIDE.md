...
首頁的閱讀狀態總覽應分開揭露兩種證據：頁面專用邊界完成數，與正式計算書可讀文字抽檢完成數。前者確認頁面提醒沒有流入交付物；後者確認正式輸出有足夠可讀文字且排除頁面專用文字。可讀文字抽檢完成數還必須對應最近一次完整交付前檢查中實際通過的瀏覽器 smoke gate；只有測試程式存在，不足以宣告完成。兩者都只是頁面治理資訊，不是計算書、PDF、Word / DOCX 或 workbook 的內容。
