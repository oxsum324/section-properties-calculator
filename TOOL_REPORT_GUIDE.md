...
計算指紋是來源資料與正式計算書之間的追溯鍵，不是單純的重複檔標記。同一案件、工具及版本只有一份來源 JSON 與一份計算書時，兩者指紋相同才是正常配對；指紋不同代表報告不是由該份來源狀態產生，必須 `blocked`。多份來源或多份輸出使用同一指紋時，才以重複附件維持 `review`。正式風力／地震工具匯出來源 JSON 時，必須直接重用同一次詳算式計算書的追溯快照，不得另建一套近似雜湊；瀏覽器 smoke 必須實際產生兩份輸出並逐工具比對指紋相等。
