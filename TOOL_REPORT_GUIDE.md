...
- 有自動產生 PDF 的 smoke，應抽取 PDF 文字並檢查頁數、文字量、必要標題與 page-only wording 排除清單，不得只驗 PDF 檔頭；有 Word / DOCX / workbook 交付物時，也應抽取 `word/document.xml`、段落 / 表格文字或 workbook cell text，確認正式輸出具備可讀內容且排除 page-only wording。
- 案件資訊只在頁首出現一次。
