...
- 精簡文件狀態：`內部審閱`，或經明確核可後的 `正式附件`；正式附件另記錄核可時間。
...
- 使用者勾選「本計算內容已完成審閱，核可作為正式附件」後，文件改為 `文件狀態：正式附件`，並記錄核可時間與計算指紋。核可是文件身分，不等於工程結果 PASS；正式附件仍可如實呈現 NG。
- 預覽中的「下載目前版本 HTML」須保存一條不必執行 JavaScript 即可靜態辨識的文件狀態、核可時間與計算指紋；重新開啟下載檔時須沿用該狀態列並重建核可控制，不得重複插入或退回錯誤狀態。分頁標題須使用穩定的計算書名稱、目前文件狀態與計算指紋，使瀏覽器列印／存 PDF 的預設檔名可直接區分內部審閱與正式附件；直接下載 HTML、DOCX、XLSX 等成品時，也應採同一「計算書名稱_文件狀態_計算指紋」命名邏輯。反覆切換或重新開啟不得重複串接狀態文字。
...
HTML 工作頁可以顯示待辦、如何補正、失效原因與操作說明；計算書只保留計算內容、工程結果、已完成複核的採用紀錄與精簡文件狀態，不得把表單、閱讀提示或操作教學帶入報告。核可方塊與下載控制只出現在螢幕預覽工具列，列印時隱藏；下載的目前版本 HTML 仍可重新開啟並列印。複核紀錄須隨專案 JSON 匯出與回讀，避免關閉頁面後失去追溯鏈。
