...
- 預覽中的「下載目前版本 HTML」須保存一條不必執行 JavaScript 即可靜態辨識的文件狀態、核可時間、選填核可紀錄與計算指紋；重新開啟下載檔時須沿用該狀態列並重建核可控制，不得重複插入或退回錯誤狀態。選填核可紀錄一旦填寫，必須與狀態、時間、標題、指紋及內容封印共同納入核可封印；空白值仍以明確空字串納入，不得被解讀為缺件。分頁標題須使用穩定的計算書名稱、目前文件狀態與計算指紋，使瀏覽器列印／存 PDF 的預設檔名可直接區分內部審閱與正式附件；直接下載 HTML、DOCX、XLSX 等成品時，也應採同一「計算書名稱_文件狀態_計算指紋」命名邏輯。反覆切換或重新開啟不得重複串接狀態文字。
