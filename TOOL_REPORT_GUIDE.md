...
Canonical family summary 另須保存 evidence bytes；schema v3 release aggregate 會把風力／地震、局部快算與鋼構的 30 組 PDF／evidence 去重為 60 份實體檔，逐份保存彙整端重驗結果及集合 SHA-256。PDF 或 evidence 任一被同大小替換都必須失敗關閉。Schema v14 納入群樁側向分配 RC 基礎案例後，成熟度矩陣只有在 canonical `60/60`、RC 視覺 `66/66`、混合格式 `13/13` 與 RC HTML `34/34` 同時通過時，才可採納該輪為完整 rendered delivery evidence；詳細清冊不進計算書或 Pages 公開狀態。Schema v15 再要求 32 組 RC 專案來源 JSON 與核可後正式 HTML 全部通過附件檢查器，並與重算結果共用同一計算指紋；Pages 只公開「RC 來源組包 `32/32`」。Schema v16 再要求 RC 設計與補強共 34 份核可 HTML 無網路依賴地重開並列印成 PDF，重驗正式狀態、工程內容與分頁；Pages 只公開「RC 核可 HTML 列印 `34/34`」。Schema v17 再要求 34 份 HTML 的計算主體各有可由瀏覽器與附件檢查器獨立重算的 SHA-256 內容封印；封印不符即阻擋，舊版缺封印轉人工確認。Schema v18 再以獨立核可封印綁定正式狀態、核可時間、計算指紋、標題與內容封印；核可資料異動即阻擋，舊版缺少核可封印則轉人工複核。Schema v19 將同一雙封印規則擴及 14 項風力／地震正式工具，實際下載 HTML 必須由附件檢查器重算通過，且內容竄改與核可資料竄改要能分別攔截。內容與核可封印都不是核可人數位簽章，不得用來宣稱核可身分。Pages 只公開各家族的雙封印完成數。Pages 的「成品檔案完整性」可顯示前三組合計 `139/139` 及各類別的 required／verified／issue／pass，但不得輸出原始 scope、案例、檔名、工具版本、bytes、逐檔或集合 SHA-256、來源雜湊或計算指紋。
