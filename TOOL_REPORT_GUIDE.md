...
目前 TXT 文字備查已涵蓋 RC 梁、柱、板，鋼構主工具連接板、主工具拉力構件、獨立連接板、鋼梁與鋼柱，錨栓檢討，4 個局部快算，以及 14 個風力／地震正式或案件報表頁。每一家族的代表案例須由 Edge 實際下載檔案，逐份證明 UTF-8 BOM、追溯檔名、畫面狀態一致、文字內容 SHA-256 可重算，且附件組包結果固定為 `blocked / non-formal-reference-text`；來源 HTML 即使已核可為正式附件，也不得使衍生 TXT 通過正式組包。錨栓 TXT 另須明列不取代完整基礎、樓板、底板與新舊混凝土介面設計。工作頁不得因此解除直接列印封鎖。
