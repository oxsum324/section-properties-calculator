...
Schema v14 再加入群樁 X／Y 向側向荷重分配結果鏈。計算書直接列出採用 p-multiplier、各列分配力、每樁水平力與力平衡結果；專業 p-y JSON 只在 schema、固定單位、樁群幾何及 Hx／Hy 完全相符且經工程師勾選採用後，列入位移、剪力、彎矩、容量、分析識別與來源 SHA-256。候選檔、核對提示及不相容原因只留在頁面；改動模型即失敗封閉。未採用相符結果時附件的側向分析檢核維持待確認，RC 結果鏈維持 `34/34`。
