...
RC、鋼構、錨栓或其他正式家族若已有獨立 audit，可採同一模式建立家族 catalog，例如 `鋼筋混凝土/tools/rc-traceability.catalog.json`、`鋼構工具/steel-traceability.catalog.json` 與 `螺栓檢討/bolt-review-tool/src/anchor-traceability.catalog.json`。這類 catalog 至少要能追到規範來源、輸入、計算核心、報告落點、回歸證據與人工複核邊界，並由該家族的 contract test 納入巡檢。
