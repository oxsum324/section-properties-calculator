...
Schema v34 將鋼構主工具梁柱彎矩接頭與全斷面 CJP 耐震柱續接納入鋼構結果鏈與 HTML 內容／核可雙封印，鋼構三條證據鏈均要求 `9/9`；兩組 PDF／evidence pair 使 canonical 完整性為 `94/94`、公開成品合計為 `173/173`、首頁 formal routes 維持 `40/40`。梁柱彎矩接頭計算書須依序列示選定平面、SMRF／IMRF 與轉角資格、`Mpr`／`MprFar`、`Vp=(Mpr+MprFar)/Lh`、六項固定強度檢核、增強節點逐梁 `ZbFyb + Vp·x` 強柱弱梁分母、外部硬體證據及未涵蓋 AISC 358 預認證／正交平面／完整接頭設計的責任邊界。柱續接計算書須列示同材同斷面對齊軋製 H 型鋼、全斷面 CJP、1.2 m 位置、40 mm 板厚、13.4.1 Eamp 原始與採用需求、七項強度檢核、13 項細部閘門、WPS／NDT 雜湊，並明示非完整柱構材設計與非現況驗收。兩份來源 JSON 必須分別恰為 88／49 欄，任何缺欄、增欄、錯誤 enum、非 finite 值或超出模型皆須交易式拒絕；Schema v33 的鋼構 `7/7`、canonical `90/90`、公開 `169/169` 只供 lineage，不得滿足 Schema v34。
