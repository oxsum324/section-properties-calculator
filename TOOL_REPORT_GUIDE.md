...
12. 具有獨立計算書產生器的工作頁不得以瀏覽器直接列印充當計算書；`Ctrl+P`／主頁列印只能顯示封鎖通知，內部 DRAFT、可送簽版與正式附件整理一律由受案件資料及檢核狀態管控的計算書流程產生。

## 正式計算書與 HTML 畫面分層

正式計算書不是工作頁的列印版，也不是把畫面卡片依序搬進 PDF。固定保留的內容為：

- 計畫資訊、產出工具、版本、輸出時間與計算指紋。
- 本案實際採用的斷面、材料、荷重、設計參數、來源與必要假設。
- 規範名稱、年版，以及放在對應檢核列或計算步驟旁的條文編號。
- 公式、代入值、計算結果、OK / NG 判定與最終結論。
- 會改變工程判讀或附件資格的必要限制、人工複核事項與 `DRAFT／非正式附件` 分類。
...
工具不得自行宣稱文件已成為「正式附件」。文件狀態固定分為三層：

- `blocked`：仍有數值或規範檢核不符，只能產生 `DRAFT／非正式附件 - 檢核不符` 的內部檢討版。
- `review`：案件資料不完整，或仍有未完成的人工複核，只能產生 `DRAFT／非正式附件 - 待人工複核`。
- `ready`：必要輸入、數值檢核、案件識別資料與人工複核均已完成，計算書必須以精簡列明確標示 `文件分類｜可送簽版`；完成公司簽認、技師簽章或其他專案核准程序後，才由文件管理流程認定為正式附件。

附件組包檢查只會把具備計畫名稱、計畫編號、設計人員，且明確標示 `文件分類｜可送簽版` 的計算書候選自動判為可整理。可辨識為計算書／計算報告或已具完整產出追溯欄位、但案件識別不完整，或沒有 DRAFT 也沒有可送簽分類的文件，必須回到 `review`，由原工具補正重產或人工確認，不得因「沒有警告字樣」就推定可交付；專案 JSON 是來源資料，不套用報告文件分類與完整報告案件識別要求。
...
HTML 工作頁可以顯示待辦、如何補正、失效原因與操作說明；計算書只保留已完成複核的採用紀錄與可送簽文件分類，不得把表單、按鈕、閱讀提示或操作教學帶入報告。複核紀錄須隨專案 JSON 匯出與回讀，避免關閉頁面後失去追溯鏈。

工作頁與計算書必須有不同的列印責任。若工具已提供「計算書」按鈕，瀏覽器直接列印工作頁時不得輸出輸入表單、操作按鈕、頁面結果或 DRAFT 計算內容，只能顯示「主頁列印已封鎖」及返回計算書流程的指引。DRAFT 不是工作頁列印的替代名稱；它仍是計算書產生器依 `blocked`／`review` 狀態輸出的正式版式內部檢討文件。
...
錨栓工作區備份另採 `bolt-review-tool-backup` v2：每一案例的來源紀錄需綁定採用產品、計算版本及目前計算核心的 SHA-256 結果指紋，匯入必須在資料庫合併前重新執行 `evaluateProjectBatch` 並完全比對；失敗時不得部分寫入。舊 v1 備份只能作為待複核資料救回，原留痕不得沿用，且計算版本未重新採用前，HTML / DOCX / XLSX 一律維持 DRAFT。案例合併會改寫 ID、名稱、附件識別或更新時間時，來源 audit hash 已不再對應目前案例，必須撤銷並由目前案例重建留痕。
