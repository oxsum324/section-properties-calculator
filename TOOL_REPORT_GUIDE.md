...
- 新產生的計算書一律預設為 `文件狀態：內部審閱`，可列印、可存成 PDF，且不加 DRAFT 橫幅或浮水印。
...
HTML 工作頁可以顯示待辦、如何補正、失效原因與操作說明；計算書只保留計算內容、工程結果、已完成複核的採用紀錄與精簡文件狀態，不得把表單、閱讀提示或操作教學帶入報告。核可方塊只出現在螢幕預覽工具列，列印時隱藏。複核紀錄須隨專案 JSON 匯出與回讀，避免關閉頁面後失去追溯鏈。
