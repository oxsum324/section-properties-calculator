...
計算指紋是來源資料與正式計算書之間的追溯鍵，不是單純的重複檔標記。同一案件、工具及版本只有一份來源 JSON 與一份計算書時，兩者指紋相同才是正常配對；指紋不同代表報告不是由該份來源狀態產生，必須 `blocked`。多份來源或多份輸出使用同一指紋時，才以重複附件維持 `review`。正式風力／地震工具及連接板／拉力構件／鋼梁／鋼柱匯出來源 JSON 時，必須直接重用同一次正式計算書的追溯快照，不得另建一套近似雜湊。正式風力／地震、鋼構正式工具，以及 RC 梁、柱、板、牆、剪力牆、基礎與單樁重新匯入來源時，必須先驗證 schema、工具種類、工具版本及來源計算指紋，再以同一正式計算流程重算；重算指紋不同時不得保留匯入狀態，且須回復原輸入。RC 計算書的產出工具名稱及版本須與同頁專案 JSON 一致；附件檢查器須直接辨識既有 `metadata`／`fields` 與單樁 `state` 專案格式，不得要求使用者為組包另造一份來源 JSON。動態時間戳、輸出時間或其他不影響工程結果的欄位不得納入計算指紋。瀏覽器 smoke 必須逐工具實際驗證正常來源重現、錯版本不改變既有狀態，以及錯指紋套用後確實回復原輸入。
