...
目前 TXT 文字備查已涵蓋 RC 梁、柱、板，以及鋼構主工具連接板、主工具拉力構件、獨立連接板、鋼梁與鋼柱。鋼構五份代表案例須由 Edge 實際下載檔案，逐份證明 UTF-8 BOM、追溯檔名、畫面狀態一致、文字內容 SHA-256 可重算，且附件組包結果固定為 `blocked / non-formal-reference-text`；舊版鋼梁與鋼柱延續頁不得因此解除工作頁直接列印封鎖。
