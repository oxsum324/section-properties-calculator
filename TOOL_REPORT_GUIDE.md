...
計算指紋是來源資料與正式計算書之間的追溯鍵，不是單純的重複檔標記。同一案件、工具及版本只有一份來源 JSON 與一份計算書時，兩者指紋相同才是正常配對；指紋不同代表報告不是由該份來源狀態產生，必須 `blocked`。即使指紋相同，案件、工具或版本不同也不得配對，必須 `blocked`。多份來源或多份輸出使用同一指紋時，才以重複附件維持 `review`。正式風力／地震工具及連接板／拉力構件／鋼梁／鋼柱匯出來源 JSON 時，必須直接重用同一次正式計算書的追溯快照，不得另建一套近似雜湊。正式風力／地震、鋼構正式工具，以及 RC 梁、柱、板、牆、剪力牆、基礎與單樁重新匯入來源時，必須先驗證 schema、工具種類、工具版本及來源計算指紋，再以同一正式計算流程重算；重算指紋不同時不得保留匯入狀態，且須回復原輸入。RC 計算書的產出工具名稱及版本須與同頁專案 JSON 一致；附件檢查器須直接辨識既有 `metadata`／`fields` 與單樁 `state` 專案格式，不得要求使用者為組包另造一份來源 JSON。動態時間戳、輸出時間或其他不影響工程結果的欄位不得納入計算指紋。瀏覽器 smoke 必須逐工具實際輸出核可後 HTML 與專案 JSON，交由正式附件檢查器證明正常組包為 `ready`，並證明竄改指紋與錯版本來源皆失敗封閉。
...
Schema v5 再建立 RC 七類正式工具、30 組實際瀏覽器案例的「專案快照重現 → PDF／正式 HTML 指紋」閉合。Producer 必須從工作頁取得含計算指紋的實際專案快照，完成匯入重現與報告結果斷言後，將核可後可攜 HTML 與真實專案 JSON 寫入暫存組包目錄，直接通過正式附件檢查器；同一案例還須證明竄改指紋及錯版本來源均被 `blocked`。通過後才可把案例 ID、快照 SHA-256、斷言數及同一計算指紋寫入 visual audit；aggregate 逐筆核對 PDF、可攜正式 HTML 與 reconciliation 指紋，並要求 `30/30` 且案例身分唯一。此鏈不公開案例資料或工程數值，也不取代設計者複核。
