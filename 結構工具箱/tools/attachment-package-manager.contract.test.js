...
assert.match(recoveryPicker, /copyButton\.Text = '複製精確路徑'[\s\S]*?copyButton\.Enabled = \$false[\s\S]*?AccessibleDescription = '只將仍有效且存在的精確輸出路徑複製到 Windows 剪貼簿，不驗證、不修改、不重建或核可附件包。'/, 'exact-path copy is initially disabled and discloses its clipboard-only boundary');
