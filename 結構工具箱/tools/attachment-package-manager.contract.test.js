...
assert.match(managerPs, /SmokeBuildRecoveryReceipt[\s\S]*?restartReceiptRestored[\s\S]*?exactPathOffered[\s\S]*?statusAndExpiryOverviewVisible[\s\S]*?pickerInitiallyUnselected[\s\S]*?pickerCancellationPreservedAll[\s\S]*?readOnlyVerifyStarted[\s\S]*?receiptRemovedAfterTrustedResult[\s\S]*?unselectedReceiptPreserved[\s\S]*?invalidReceiptRejected[\s\S]*?expiredReceiptRemoved[\s\S]*?activeReceiptIgnored[\s\S]*?buildProcessStarted[\s\S]*?built = \$false/, 'dynamic restart smoke proves the unselected status and expiry overview, cancellation preservation, exact selection cleanup, invalid receipt rejection, and no build');
