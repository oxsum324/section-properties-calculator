...
const fakeBuilder = {
  ...
  buildPackage() {
    return {
      ...
      selfVerification: { summary: { htmlDualSealExpected: 1, htmlDualSealVerified: 1, xlsxDualSealExpected: 1, xlsxDualSealVerified: 1, evidenceChainExpected: 1, evidenceChainVerified: 1 } },
    };
  },
};
const fakeVerifier = {
  verifyPackage() {
    return {
      ...
      summary: { expectedFiles: 2, verifiedFiles: 2, htmlDualSealExpected: 1, htmlDualSealVerified: 1, xlsxDualSealExpected: 1, xlsxDualSealVerified: 1, evidenceChainExpected: 1, evidenceChainVerified: 1, errors: 0, warnings: 0 },
      records: [{
        ...
        packagedFile: '01_正式附件/錨栓計算書.xlsx', role: 'formal', status: 'verified',
        xlsxDualSeal: { family: 'anchor', contentStatus: 'verified', approvalStatus: 'verified' },
      }],
      issues: [],
    };
  },
...
assert.match(build.displayText, /XLSX 雙封印複驗 1 \/ 1 份/);
assert.match(build.displayText, /開挖證據鏈複驗 1 \/ 1 組/);
assert.equal(build.counts.htmlDualSealVerified, 1);
assert.equal(build.counts.xlsxDualSealVerified, 1);
...
assert.equal(verify.records[1].result, 'verified｜錨栓 XLSX 雙封印已驗證');
assert.equal(verify.counts.htmlDualSealExpected, 1);
assert.equal(verify.counts.htmlDualSealVerified, 1);
assert.equal(verify.counts.xlsxDualSealExpected, 1);
assert.equal(verify.counts.xlsxDualSealVerified, 1);
...
assert.equal(buildRecoveryReceiptPayload.criticalReceiptHighlighted, true);
assert.equal(buildRecoveryReceiptPayload.normalReceiptUnhighlighted, true);
assert.equal(buildRecoveryReceiptPayload.multipleSelectionExpiryAutoCleared, true);
assert.equal(buildRecoveryReceiptPayload.multipleSelectionInvalidationAnnounced, true);
assert.equal(buildRecoveryReceiptPayload.invalidRowSelectionRejected, true);
assert.equal(buildRecoveryReceiptPayload.temporaryUnavailableKeepsRefreshing, true);
assert.equal(buildRecoveryReceiptPayload.allCandidatesExpiredHandled, true);
assert.equal(buildRecoveryReceiptPayload.multiExpiredReceiptCleanupRequested, true);
assert.equal(buildRecoveryReceiptPayload.shortcutHintVisible, true);
assert.equal(buildRecoveryReceiptPayload.folderPreviewRequestedWithoutExternalLaunch, true);
assert.equal(buildRecoveryReceiptPayload.exactPathCopyRequestedWithoutClipboardWrite, true);
assert.equal(buildRecoveryReceiptPayload.exactPathKeyboardCopyRequested, true);
assert.equal(buildRecoveryReceiptPayload.exactPathKeyboardEventHandled, true);
assert.equal(buildRecoveryReceiptPayload.unselectedKeyboardCopyBlocked, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffCopyOffered, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffExpiryVisible, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffExpiryTimerRunning, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffUrgentTierVisible, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffCriticalTierVisible, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffNormalTierVisible, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffShortcutHintVisible, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffPathCopyRequestedWithoutClipboardWrite, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffKeyboardCopyRequested, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffKeyboardEventHandled, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffUnfocusedKeyboardCopyBlocked, true);
assert.equal(buildRecoveryReceiptPayload.singleHandoffExpiryAutoDisabled, true);
assert.equal(buildRecoveryReceiptPayload.sameSessionHandoffHasNoFalseExpiry, true);
assert.equal(buildRecoveryReceiptPayload.multipleSelectorRestoredAfterSingleCopy, true);
...
assert.match(managerPs, /function Set-BuildRecoveryReceiptState[\s\S]*?State -eq 'pending-verification'[\s\S]*?managerPid -NotePropertyValue 0[\s\S]*?managerStartedAtUtc -NotePropertyValue ''/, 'a completed build with indeterminate handoff releases process ownership before the receipt becomes eligible for immediate read-only recovery');
assert.match(managerPs, /function Complete-BuildOperation[\s\S]*?Set-BuildRecoveryReceiptState -ReceiptPath \$recoveryReceiptPath -State 'pending-verification' -WorkerPid 0[\s\S]*?Show-BuildResultHandoffUnknown/, 'indeterminate build handoff transitions the exact receipt before rendering its governed recovery actions');
...
assert.match(keyboardHandler, /Control[\s\S]*?Keys\]::C[\s\S]*?ActiveControl[\s\S]*?active -ne \$script:BtnBuildHandoffCopy[\s\S]*?Set-ManagerKeyHandled[\s\S]*?singleHandoffKeyboardCopyRequested[\s\S]*?BtnBuildHandoffCopy\.PerformClick\(\)/, 'Ctrl+C is handled only while the single handoff copy button has focus and reuses its governed click route');
...
assert.match(managerPs, /Update-BuildRecoveryPickerRows[\s\S]*?remaining\.TotalHours -le 2[\s\S]*?remaining\.TotalMinutes -le 30[\s\S]*?30 分鐘內到期[\s\S]*?2 小時內到期[\s\S]*?FromArgb\(192, 57, 43\)[\s\S]*?FromArgb\(146, 64, 14\)[\s\S]*?FromArgb\(251, 234, 234\)[\s\S]*?FromArgb\(255, 247, 230\)/, 'multiple recovery rows present distinct critical, urgent and normal visual tiers with explicit labels');
assert.match(recoveryPicker, /intro\.Text = .*2 小時內深橙、30 分鐘內紅色並有明文提示[\s\S]*?grid\.AccessibleDescription = '唯讀單選清單，初始不選取任何項目；2 小時內及 30 分鐘內到期會在狀態欄顯示明文提示/, 'multiple recovery urgency tiers are explained visibly and to assistive technology instead of relying on color alone');
assert.match(recoveryPicker, /selectionStatus\.Text = '尚未選取項目。'[\s\S]*?AccessibleRole = \[System\.Windows\.Forms\.AccessibleRole\]::StatusBar[\s\S]*?LiveSetting = \[System\.Windows\.Forms\.Automation\.AutomationLiveSetting\]::Assertive/, 'the recovery picker exposes a visible assertive live status region for selection changes');
assert.match(recoveryPicker, /refreshPickerState = \{[\s\S]*?selectedRow[\s\S]*?Update-BuildRecoveryPickerRows -Grid \$grid -NowUtc \$NowUtc[\s\S]*?ClearSelection\(\)[\s\S]*?CurrentCell = \$null[\s\S]*?openButton\.Enabled = \$false[\s\S]*?copyButton\.Enabled = \$false[\s\S]*?verifyButton\.Enabled = \$false[\s\S]*?已自動取消選取[\s\S]*?RaiseLiveRegionChanged\(\)[\s\S]*?expiryTimer\.Add_Tick[\s\S]*?& \$refreshPickerState/, 'a timer refresh clears an invalidated selection, disables every action and raises an accessible live-region notification');
assert.match(recoveryPicker, /eligibleCount[\s\S]*?unexpiredCount[\s\S]*?eligibleCount -eq 0[\s\S]*?unexpiredCount -eq 0[\s\S]*?所有待確認項目均已到期，目前已無可驗證項目[\s\S]*?expiryTimer\.Stop\(\)[\s\S]*?部分輸出不存在，將持續重新檢查[\s\S]*?temporarily-unavailable[\s\S]*?可驗證項目已恢復/, 'the picker stops refreshing only after every receipt expires, but keeps checking recoverable missing outputs and announces recovery');
assert.match(recoveryPicker, /candidateExpired[\s\S]*?expiryCleanupAttempted[\s\S]*?Remove-BuildRecoveryReceipt -ReceiptPath \(\[string\]\$candidate\.path\) -CleanupArtifacts[\s\S]*?Add-Member -NotePropertyName expiryCleanupAttempted/, 'multi-recovery expiry immediately removes each private receipt and its managed artifacts once');
assert.match(recoveryPicker, /cancelButton\.Text = '關閉'[\s\S]*?AccessibleName = '關閉已無可驗證項目的待確認輸出總覽'[\s\S]*?所有待確認項目均已到期/, 'the terminal all-expired state exposes close as the only remaining action');
assert.match(recoveryPicker, /grid\.Add_SelectionChanged[\s\S]*?clearingSelection[\s\S]*?SelectedRows\.Count -eq 1[\s\S]*?Cells\['status'\]\.Tag[\s\S]*?& \$refreshPickerState[\s\S]*?invalidRowSelectionRejected/, 'an already-invalid row cannot remain selected or expose any action');
...
assert.match(recoveryPicker, /intro\.Text = .*可按 Ctrl\+C 複製精確路徑[\s\S]*?copyButton\.Text = '複製精確路徑 \(Ctrl\+C\)'[\s\S]*?copyButton\.Enabled = \$false[\s\S]*?AccessibleDescription = '按此按鈕或在清單按 Ctrl\+C，只將仍有效且存在的精確輸出路徑複製到 Windows 剪貼簿，不驗證、不修改、不重建或核可附件包。'/, 'the visible intro and initially disabled exact-path button disclose the controlled shortcut and clipboard-only boundary');
assert.match(recoveryPicker, /copyButton\.Add_Click[\s\S]*?Update-BuildRecoveryPickerRows -Grid \$grid[\s\S]*?SelectedRows\.Count -ne 1[\s\S]*?Test-Path -LiteralPath \$outputPath -PathType Container[\s\S]*?SmokeBuildRecoveryReceipt[\s\S]*?exactPathCopyRequested[\s\S]*?Clipboard\]::SetText\(\$outputPath\)/, 'exact-path copy rechecks one eligible existing selection and writes only that exact path outside smoke mode');
assert.match(recoveryPicker, /ClipboardCopyMode = \[System\.Windows\.Forms\.DataGridViewClipboardCopyMode\]::Disable[\s\S]*?Ctrl\+C 只複製有效選取項目的精確輸出路徑/, 'the recovery grid disables native row copying and discloses the controlled Ctrl+C behavior');
assert.match(recoveryPicker, /copyKeyHandler = \{[\s\S]*?eventArgs\.Control[\s\S]*?Keys\]::C[\s\S]*?SuppressKeyPress = \$true[\s\S]*?Handled = \$true[\s\S]*?copyButton\.Enabled[\s\S]*?exactPathKeyboardCopyRequested[\s\S]*?copyButton\.PerformClick\(\)[\s\S]*?grid\.Add_KeyDown\(\$copyKeyHandler\)/, 'controlled Ctrl+C suppresses native table copying and reuses the exact-path button route only for an eligible selection');
...
assert.match(managerPs, /SmokeBuildRecoveryReceipt[\s\S]*?restartReceiptRestored[\s\S]*?exactPathOffered[\s\S]*?statusAndExpiryOverviewVisible[\s\S]*?pickerInitiallyUnselected[\s\S]*?earliestExpiryFirst[\s\S]*?urgentReceiptHighlighted[\s\S]*?criticalReceiptHighlighted[\s\S]*?normalReceiptUnhighlighted[\s\S]*?multipleSelectionExpiryAutoCleared[\s\S]*?multipleSelectionInvalidationAnnounced[\s\S]*?invalidRowSelectionRejected[\s\S]*?temporaryUnavailableKeepsRefreshing[\s\S]*?allCandidatesExpiredHandled[\s\S]*?multiExpiredReceiptCleanupRequested[\s\S]*?shortcutHintVisible[\s\S]*?folderPreviewRequestedWithoutExternalLaunch[\s\S]*?exactPathCopyRequestedWithoutClipboardWrite[\s\S]*?exactPathKeyboardCopyRequested[\s\S]*?exactPathKeyboardEventHandled[\s\S]*?unselectedKeyboardCopyBlocked[\s\S]*?singleHandoffCopyOffered[\s\S]*?singleHandoffExpiryVisible[\s\S]*?singleHandoffExpiryTimerRunning[\s\S]*?singleHandoffUrgentTierVisible[\s\S]*?singleHandoffCriticalTierVisible[\s\S]*?singleHandoffNormalTierVisible[\s\S]*?singleHandoffShortcutHintVisible[\s\S]*?singleHandoffPathCopyRequestedWithoutClipboardWrite[\s\S]*?singleHandoffKeyboardCopyRequested[\s\S]*?singleHandoffKeyboardEventHandled[\s\S]*?singleHandoffUnfocusedKeyboardCopyBlocked[\s\S]*?singleHandoffExpiryAutoDisabled[\s\S]*?sameSessionHandoffHasNoFalseExpiry[\s\S]*?multipleSelectorRestoredAfterSingleCopy[\s\S]*?pickerCancellationPreservedAll[\s\S]*?readOnlyVerifyStarted[\s\S]*?receiptRemovedAfterTrustedResult[\s\S]*?unselectedReceiptPreserved[\s\S]*?invalidReceiptRejected[\s\S]*?expiredReceiptRemoved[\s\S]*?activeReceiptIgnored[\s\S]*?buildProcessStarted[\s\S]*?built = \$false/, 'dynamic restart smoke proves invalid-row rejection, temporary recovery polling, receipt cleanup, all-expired convergence, urgency tiers and no-build boundaries');
...
assert.match(handoffRecoveryDisplay, /BtnBuildHandoffCopy\.Text = '複製精確路徑 \(Ctrl\+C\)'[\s\S]*?BtnBuildHandoffCopy\.Visible = \$true[\s\S]*?BtnBuildHandoffCopy\.Enabled = \$true/, 'a single exact existing handoff exposes the dedicated path-copy action and visible shortcut');
assert.match(handoffRecoveryDisplay, /Get-EligibleBuildRecoveryReceipts -ReceiptPaths @\(\$RecoveryReceiptPath\)[\s\S]*?HandoffRecoveryExpiresAtUtc[\s\S]*?Update-SingleBuildHandoffExpiry[\s\S]*?HandoffRecoveryTimer\.Start\(\)/, 'receipt-backed single handoff exposes actions only after exact receipt revalidation and starts its expiry display timer');
const singleHandoffExpiry = managerPs.match(/function Update-SingleBuildHandoffExpiry \{[\s\S]*?(?=\nfunction Update-BuildRecoveryPickerRows)/)?.[0] || '';
assert.match(singleHandoffExpiry, /NowUtc[\s\S]*?expiresAt[\s\S]*?Remove-BuildRecoveryReceipt -ReceiptPath \$receiptPath -CleanupArtifacts[\s\S]*?Clear-BuildHandoffRecovery[\s\S]*?已到期[\s\S]*?自動停用/, 'single handoff expiry removes the expired receipt and its private artifacts before fail-closing both recovery actions');
assert.match(singleHandoffExpiry, /Get-EligibleBuildRecoveryReceipts[\s\S]*?outputPath\.Equals[\s\S]*?Test-Path[\s\S]*?已失效或輸出不存在[\s\S]*?Format-BuildRecoveryRemainingTime[\s\S]*?有效至/, 'single handoff timer refresh revalidates the exact receipt and folder before showing its remaining lifetime');
assert.match(singleHandoffExpiry, /remaining\.TotalMinutes -le 30[\s\S]*?'30 分鐘內到期'[\s\S]*?remaining\.TotalHours -le 2[\s\S]*?'2 小時內到期'[\s\S]*?FromArgb\(192, 57, 43\)[\s\S]*?FromArgb\(146, 64, 14\)[\s\S]*?SystemColors\]::ControlText/, 'single handoff expiry presents distinct critical, urgent and normal visual tiers');
assert.match(singleHandoffExpiry, /urgencyDescription[\s\S]*?'期限狀態一般'[\s\S]*?AccessibleDescription[\s\S]*?urgencyDescription/, 'single handoff urgency is stated in assistive text and never conveyed by color alone');
assert.doesNotMatch(singleHandoffExpiry, /Start-ReadOnlyOperation|Start-BuildOperation|Invoke-AttachmentWorker|Clipboard/, 'single handoff expiry refresh cannot copy, verify, build, modify, or approve a package');
assert.match(managerPs, /HandoffRecoveryTimer = New-Object System\.Windows\.Forms\.Timer[\s\S]*?Interval = 30000[\s\S]*?Update-SingleBuildHandoffExpiry/, 'single handoff expiry is refreshed every 30 seconds by a UI timer');
assert.match(managerPs, /StatusMeta\.AccessibleName = '附件包管理狀態補充資訊'[\s\S]*?到期後複製與唯讀驗證會自動停用/, 'single handoff expiry and automatic-disable status are exposed to assistive technology');
...
const handoffCopyHandler = managerPs.match(/\$script:BtnBuildHandoffCopy\.Add_Click\(\{[\s\S]*?(?=\n\}\)\n\n\$script:BtnOpenOutput\.Add_Click)/)?.[0] || '';
assert.match(handoffCopyHandler, /BuildHandoffRecoveryOutput[\s\S]*?HandoffRecoveryReceiptPath[\s\S]*?Get-EligibleBuildRecoveryReceipts -ReceiptPaths @\(\$recoveryReceiptPath\)[\s\S]*?outputPath\.Equals\(\$recoveryOutput[\s\S]*?Test-Path -LiteralPath \$recoveryOutput -PathType Container[\s\S]*?singleHandoffPathCopyRequested[\s\S]*?Clipboard\]::SetText\(\$recoveryOutput\)/, 'single handoff copy revalidates its receipt and exact existing folder before copying only that path outside smoke mode');
assert.doesNotMatch(handoffCopyHandler, /Start-ReadOnlyOperation|Start-BuildOperation|Action build|Invoke-AttachmentWorker/, 'single handoff copy cannot verify, build, modify, or approve a package');
...
assert.match(managerPs, /BtnBuildHandoffCopy\.AccessibleDescription = '按此按鈕或在焦點位於本按鈕時按 Ctrl\+C，只將仍有效且存在的單筆待確認輸出路徑複製到 Windows 剪貼簿，不驗證、不修改、不重建或核可附件包。'/, 'assistive text preserves the focus-scoped shortcut and single handoff copy boundary');
