...
assert.match(handoffRecoveryDisplay, /BtnBuildHandoffCopy\.Text = '複製精確路徑'[\s\S]*?BtnBuildHandoffCopy\.Visible = \$true[\s\S]*?BtnBuildHandoffCopy\.Enabled = \$true/, 'a single exact existing handoff exposes the dedicated path-copy action');
...
assert.match(managerPs, /BtnBuildHandoffCopy\.AccessibleDescription = '只將仍有效且存在的單筆待確認輸出路徑複製到 Windows 剪貼簿，不驗證、不修改、不重建或核可附件包。'/, 'assistive text preserves the single handoff copy boundary');
