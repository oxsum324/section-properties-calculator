...
assert.match(managerPs, /SmokeBuildRecoveryReceipt[\s\S]*?restartReceiptRestored[\s\S]*?exactPathOffered[\s\S]*?pickerCancellationPreservedAll[\s\S]*?readOnlyVerifyStarted[\s\S]*?receiptRemovedAfterTrustedResult[\s\S]*?unselectedReceiptPreserved[\s\S]*?invalidReceiptRejected[\s\S]*?expiredReceiptRemoved[\s\S]*?activeReceiptIgnored[\s\S]*?buildProcessStarted[\s\S]*?built = \$false/, 'dynamic restart smoke proves cancellation preserves all receipts, then an exact multi-candidate read-only selection clears only the selected receipt, rejects malformed/expired/active receipts, and never builds');
...
assert.match(handoffRecoveryHandler, /BuildRecoveryCandidates\.Count -gt 1[\s\S]*?Select-BuildRecoveryReceipt[\s\S]*?if \(-not \$recoveryCandidate\) \{ return \}[\s\S]*?recoveryCandidate\.outputPath[\s\S]*?recoveryCandidate\.path/, 'multiple receipts require an explicit picker result and cancellation preserves every candidate');
