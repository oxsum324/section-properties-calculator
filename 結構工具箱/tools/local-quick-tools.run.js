...
function runNode(label, relativePath) {
  ...
  for (let attempt = 0; attempt < transientRunDelaysMs.length; attempt += 1) {
    ...
    if (canRetry) {
      ...
      process.stderr.write(`[local-quick] ${label} transient launch failure on attempt ${attempt + 1}: ${detail}; retrying in ${delayMs}ms\n`);
      waitSync(delayMs);
      continue;
    }
...
assertFile(manifest.shared.loadComboV2Test);
...
runNode('load combination tuple regression', manifest.shared.loadComboV2Test);
