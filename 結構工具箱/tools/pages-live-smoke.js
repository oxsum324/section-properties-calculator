...
const PUBLIC_ROUTE_SAMPLES = [
  { path: 'field-survey/recorder.html', needles: ['現況鑑定紀錄 V0.6.0', './manifest.webmanifest', './app.js', '備份還原', '原始照片'], checkAssets: true },
  { path: 'field-survey/sw.js', needles: ['condition-survey-shell-0.6.0', './sketch.js', 'cache.addAll', 'ACTIVATE_UPDATE'] },
...
async function main() {
  ...
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatus.steelHtmlContentSealRequired), 'report readiness expects a supported 5-to-9 steel formal HTML content seal transition count');
  ...
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatus.steelHtmlApprovalSealRequired), 'report readiness expects a supported 5-to-9 steel formal HTML approval seal transition count');
  ...
  assert.ok([5, 6, 7, 9].includes(reportReadinessStatus.steelResultReconciliationRequired), 'report readiness expects a supported 5-to-9 steel result reconciliation transition count');
