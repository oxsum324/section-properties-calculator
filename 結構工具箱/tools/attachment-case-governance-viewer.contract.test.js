...
[
  ...
  "[string]$InitialPath = ''", "[ValidateSet('case', 'portfolio')][string]$InitialMode = 'case'", '[switch]$AutoInspect',
...
assert.match(viewerPs, /\$previousOutputEncoding = \[Console\]::OutputEncoding[\s\S]*?UTF8Encoding\(\$false\)[\s\S]*?finally \{\s*\[Console\]::OutputEncoding = \$previousOutputEncoding/, 'viewer decodes Node JSON as UTF-8 and restores the prior console encoding');
...
assert.match(viewerPs, /Add_Shown\(\{[\s\S]*?\$AutoInspect[\s\S]*?\$script:BtnInspect\.PerformClick\(\)[\s\S]*?\}\)/, 'explicit AutoInspect triggers only the viewer read-only inspection');
