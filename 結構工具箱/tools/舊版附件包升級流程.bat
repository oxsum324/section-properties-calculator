...
if "%~1"=="" (
  ...
  echo   舊版附件包升級流程.bat "C:\case\input" ["C:\case\output"] [計畫編號] ["C:\case\history"]
  pause
  exit /b 3
)

if not "%~4"=="" (
  node "%~dp0attachment-package-upgrade-flow.js" --input "%~1" --output "%~2" --project-no "%~3" --history-dir "%~4"
) else if "%~2"=="" (
