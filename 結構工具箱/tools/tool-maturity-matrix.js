...
function isCompleteRenderedDeliveryEvidence(evidence, runId) {
  ...
  return Boolean(
    ...
    && (!localQuickResultReconciliationDeclared || (
      ...
      && evidence.localQuickResultReconciliation.complete === expectedLocalQuickResultReconciliationCount
...
function buildHomepageReportReadinessStatus(matrixPayload, sourceHash, preflightStatus = null, preflightPayload = null, renderedDeliveryEvidence = null) {
  ...
  const docxPackageIntegrity = renderedDeliveryPayload?.docxPackageIntegrity;
  const docxPackageIntegrityDeclared = Boolean(
    Number(renderedDeliveryPayload?.schemaVersion) >= 22
    && docxPackageIntegrity
    && docxPackageIntegrity.scope === 'formal-docx-clean-ooxml-package'
    && Number.isInteger(docxPackageIntegrity.required)
    && Number.isInteger(docxPackageIntegrity.complete)
  );
  const docxPackageIntegrityRequired = docxPackageIntegrityDeclared ? compactNumber(docxPackageIntegrity.required) : 0;
  const docxPackageIntegrityComplete = docxPackageIntegrityDeclared ? compactNumber(docxPackageIntegrity.complete) : 0;
  const docxPackageIntegrityIssueCount = !docxPackageIntegrityDeclared
    ? 0
  ...
  const details = [
    ...
    ...(localQuickResultReconciliationDeclared ? [`局部快算計算書結果鏈：${localQuickResultReconciliationToolLabels}共 ${localQuickResultReconciliationComplete} / ${localQuickResultReconciliationRequired} 組來源 JSON 已完成全輸入與全結果逐值重播，再核對 PDF 的同一計算指紋與成品雜湊。公開狀態只顯示完成數，不公開來源 JSON、輸入／結果資料、計算指紋或成品雜湊。`] : []),
  ...
  return {
    ...
    ...(docxPackageIntegrityDeclared ? {
      ...
      docxPackageIntegrityComplete,
      docxPackageIntegrityIssueCount,
...
function checkMatrix(payload, markdown, options = {}) {
  ...
  if (preserveHomepageStatus) {
    ...
    assert.ok(homepageReportReadinessStatus.independentBenchmarkVerified >= 0
      && homepageReportReadinessStatus.independentBenchmarkVerified <= payload.independentBenchmarkCoverage.summary.independentlyVerifiedRoutes,
    'preserved homepage report readiness independent benchmark verified routes bounded by current coverage');
