...
function checkMatrix(payload, markdown, options = {}) {
  ...
  assert.equal(payload.independentBenchmarkCoverage.summary.pilotVerified, 10, 'tool maturity matrix independent benchmark pilot verified');
  assert.equal(payload.independentBenchmarkCoverage.summary.pilotRequired, 10, 'tool maturity matrix independent benchmark pilot required');
  assert.equal(payload.independentBenchmarkCoverage.summary.independentlyVerifiedRoutes, 10, 'tool maturity matrix independent benchmark verified route count');
  ...
  if (preserveHomepageStatus) {
    ...
    assert.ok(homepageReportReadinessStatus.independentBenchmarkVerified >= 0 && homepageReportReadinessStatus.independentBenchmarkVerified <= 10, 'preserved homepage report readiness independent benchmark verified routes bounded by current coverage');
    ...
    assert.equal(homepageReportReadinessStatus.independentBenchmarkVerified, 10, 'homepage report readiness independent benchmark verified routes');
    assert.equal(homepageReportReadinessStatus.independentBenchmarkPilotRequired, 10, 'homepage report readiness independent benchmark pilot required');
    assert.equal(homepageReportReadinessStatus.independentBenchmarkPilotVerified, 10, 'homepage report readiness independent benchmark pilot verified');
  }
