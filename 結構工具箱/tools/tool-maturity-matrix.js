...
function checkMatrix(payload, markdown, options = {}) {
  ...
  if (preserveHomepageStatus) {
    ...
    assert.ok(homepageReportReadinessStatus.independentBenchmarkVerified >= 0 && homepageReportReadinessStatus.independentBenchmarkVerified <= 6, 'preserved homepage report readiness independent benchmark verified routes bounded by current coverage');
