...
function assertNewHomeState(state, tools, label, preflightStatusPayload) {
  ...
  ['頁面邊界', '可讀文字', '瀏覽器 smoke', '成品渲染', '成品檔案完整性'].forEach(metric => {
  ...
  if (state.protocol === 'file:') {
    ...
    assert.ok(state.reportReadinessStatusMeta.includes('成品檔案完整性 137 / 137'), `${label} new home report readiness delivery file integrity metric`);
  ...
  try {
    ...
    const loadComboLimitStateCases = [
      ...
      },
      {
        ...
        prefix: 'lcBeam',
        sourceWrites: [
          { id:'lcBeam_M_D', value:40 },
          { id:'lcBeam_M_W', value:-100 },
          { id:'lcBeam_V_D', value:10 },
          { id:'lcBeam_V_W', value:-50 },
          { id:'lcBeam_P_D', value:100 },
        ],
        targetIds: ['MuPos', 'MuNeg', 'Vu', 'Pu', 'Tu'],
        expectedName: '1.2D+1.0L-1.0W',
        expectedIndex: 3,
        expectedValues: { M:148, V:62, P:120, T:0 },
        expectedTargets: { MuPos:'148.000', MuNeg:'0.000', Vu:'62.000', Pu:'120.000', Tu:'0.000' },
      ...
      },
      {
        ...
        prefix: 'lcWall',
        sourceWrites: [
          { id:'lcWall_P_D', value:100 },
          { id:'lcWall_P_W', value:-180 },
          { id:'lcWall_M_D', value:10 },
          { id:'lcWall_M_W', value:-80 },
          { id:'lcWall_V_D', value:10 },
          { id:'lcWall_V_W', value:-50 },
        ],
        targetIds: ['Pu', 'MuIn', 'Vu'],
        expectedName: '0.9D+1.0W',
        expectedIndex: 6,
        expectedValues: { P:-90, M:-71, V:-41 },
        expectedTargets: { Pu:'-90.000', MuIn:'71.000', Vu:'41.000' },
        expectedCriterion: 'custom',
        expectedCapacityStatus: 'evaluated',
      },
      {
