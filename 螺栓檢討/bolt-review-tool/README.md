...
workbook 本身另保存 A4 列印設定：兩欄摘要與窄表採直向，七欄以上寬表採橫向；所有工作表固定單頁寬、不強制單頁高、列印範圍只涵蓋實際資料，並在續頁重複第一列表頭。Schema v24 release 會由 Microsoft Excel 唯讀逐張輸出 PDF，再驗證 9/9 工作表的頁面數、文字、像素、頁緣、續頁表頭及橫向溢出；稽核器不會修改下載 XLSX。逐張 PDF、工作表名稱與雜湊只留在私人 release 證據，工作頁僅顯示完成數。

Schema v25 另在 `Summary` 可見列保存 XLSX 內容／核可雙封印。內容 SHA-256 涵蓋全部工作表的非空白儲存格、公式與快取結果；核可 SHA-256 綁定內容封印、文件狀態、核可資訊、產出工具、版本、固定格式輸出時間及計算指紋。獨立 OOXML 驗證器會在 release 重算兩者並測試正文、公式、內容 SHA 與核可欄位竄改。這是防止附件被無聲修改的完整性證據，不是核可人身分的數位簽章；公開工作頁只顯示雙封印完成數，不公開封印值或工作表內容。
