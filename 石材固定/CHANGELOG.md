...
版本同步：`APP_VERSION = V3.0.8`；`SERVER_VERSION = 3.0.8`。

### 重點：SsD、地盤係數與 SDS 來源鏈修正

- 修正鄉鎮震區資料第一欄為 `SsD`、不得直接作為 `SDS` 的語意錯置；改由地盤第一、二、三類之 `Fa` 依表 2-4(a) 內插，再依式 (2-4) 計算 `SDS = Fa × SsD`。
- 保留「專案指定 SDS」模式，但正式匯出前必須填寫規範判定、地盤調查報告或專案文件依據；來源鏈不完整時以阻擋級檢查攔截。
- 主文、Word、DOCX 與結構化摘要同步揭露行政區、SsD、地盤種類、Fa、SDS 與表／式來源。
- 將風速、震區、工址係數與行政區正規化資料集中於結構工具箱核心，石材 vendor 副本由同步腳本產生並以 SHA-256 契約防止漂移。
- 新增跨工具共用工址：可在石材或四支正式耐震頁明確儲存縣市、鄉鎮與地盤分類，再於耐風案件總覽或其他支援頁明確套用；設定檔不保存 V₁₀(C)、SsD、Fa 或 SDS，每次套用均由當下規範主庫重新解析。
- 修正員林市、二崙鄉等行政區名稱，並將釣魚臺列嶼列為無風速對照資料的明示例外；跨表契約要求所有其餘可比對鄉鎮零衝突。
- 條文語意追蹤 catalog 升級至 0.7.0；首頁卡片同步顯示 V3.0.8。
