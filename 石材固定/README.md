...
平台正式 release 會保存當輪 PDF、DOCX 與 audit JSON，並對 DOCX 執行共通 OOXML 乾淨封裝檢查；未引用媒體、實際批註、未接受修訂、外掛範本／外部圖片、嵌入物件、巨集或非預期 custom XML 都會阻擋放行。封裝檢查屬內部交付證據，不會出現在計算書正文。
...
風速、震區、工址係數與行政區名稱以 `../結構工具箱/core/loads/` 為單一來源；石材離線版的 `vendor/loads/` 由 `node ../結構工具箱/tools/sync-stone-regulatory-vendor.js` 同步。`structural-project-location.v1` 共用工址只保存縣市、鄉鎮與地盤分類，使用者按下套用時才由目前主庫重新取得 V₁₀(C)、SsD、Fa 與 SDS，避免舊案凍結過期規範值。同步後應依序執行 `node ../結構工具箱/tools/project-location.test.js` 與 `node ../結構工具箱/tools/regulatory-data.contract.test.js`，確認跨工具欄位事件接線、vendor 雜湊一致、鄉鎮風速交叉比對零衝突，以及 `SsD → Fa → SDS` 計算與共用核心一致。
`js/version-sync-smoke.test.js` 會用 mock DOM 檢查側欄版本標示 helper。
`js/code-profiles-registry-smoke.test.js` 會檢查 active profile、指紋、profile-owned 輸入同步、專案 profile 正規化與 profile-owned migration 預設；切換 profile 時只更新仍等於舊 profile 預設的欄位，使用者自行覆寫的值會保留。`ui_smoke_test.py` 另以隔離瀏覽器驗證專案 JSON 在匯入／重新載入後會完整取代 runtime profile；每次 `load()` 會先清空既有案例，再還原存檔案例，避免重載、復原／重做或 migration 重試時案例重複累加。缺少 `Ip / ap / Rp` 時依 active profile 補齊並記錄 `meta.applied_defaults`，明示人工值則保留。來源 `input_hash` 只與移轉前內容比對，正規化寫回會重新產生指紋；一般存檔也會先深拷貝不可變輸入快照，對同一份快照計算 hash 並寫出，並以 request token 確保密集輸入時只有最新非同步存檔可以更新 localStorage。載入、復原或匯入新專案也會使舊專案尚未完成的寫回失效。中斷的 `pending / failed` migration 寫回會在下次載入重試，`pending` 期間不允許匯出。補值提示只在本次移轉顯示，已正規化專案重載不會重複提示，真正的外部修改仍會導致 `mismatch`。藉此避免換電腦遺失選擇、被另一個專案的 localStorage 狀態汙染、舊快照倒寫、假完整性警報，或出現 profile 與實算欄位矛盾。
