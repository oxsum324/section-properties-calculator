...
(function (global) {
  ...
  const SOURCE = Object.freeze({
    authority: '內政部國土管理署',
    document: '建築物耐震設計規範及解說（113 年版）',
    issued: '2024-03-01',
    order: '台內國字第 1130801422 號令',
    zonalTable: '表 2-1',
    siteCoefficientTables: Object.freeze(['表 2-4(a)', '表 2-4(b)']),
    equation: '式 (2-4)',
  });
   ...
   * Ref: 規範 表 2-4(a)
   */
  function normalizeSiteClass(siteClass) {
    const value = Number(siteClass);
    if (![1, 2, 3].includes(value)) {
      throw new RangeError('地盤種類必須為 1、2 或 3。');
    }
    return value;
  }

  function getFa(siteClass, SS) {
    const row = FA_TABLE[normalizeSiteClass(siteClass)];
  ...
  function getFv(siteClass, S1) {
    const row = FV_TABLE[normalizeSiteClass(siteClass)];
