...
- `SRC工具/` 的 SRC 柱研究核心須將 `core/src-column-core.js`、`core/src-column-h-section-catalog.js`、`core/src-column-rc-biaxial.js`、`core/src-column-shear.js`、`core/src-column-seismic-axial.js`、`core/src-column-seismic-detailing.js`、`core/src-column-oracle.js`、六個對應測試、`src-column-core.test.js`、`src-column-traceability.catalog.json`、共用 `鋼筋混凝土/shared/pmsection.js`／測試與 `README.md` 一起 staging。它不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；表 3.4-2 寬厚比、鋼骨雙軸互制、RC 定軸力 Mux-Muy 曲面、第 9.3 節耐震軸向強度、第 9.6.2 節強軸耐震柱剪力、第 9.6.1 節單一強軸接頭強柱弱梁、第 9.6.3 節強軸矩形柱圍束，以及獨立 oracle 已完成。catalog 擴充、現況材料與材證、弱軸耐震路徑、第 9.3 節專案拉力強度／傳力容量來源、第 8.4.2 節接頭傳力比例、全構架所有接頭、其餘耐震細節與計算書邊界仍須完成，才可另案評估正式入口。
