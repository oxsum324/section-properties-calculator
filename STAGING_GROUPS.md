...
`public-release-decision-cloud-checkpoint.js` 必須以 provider API 直接列出的目前 PRB／PDR 中繼資料建立封閉 `PDC-` 檢查點。因目前 Drive 連線採唯讀授權，PDC 經 DriveFS 寫入私人 `cloud-checkpoints/` 後，必須再由 API 下載服務端原始檔，逐 byte 核對本機 PDC 並由工作站重算 SHA-256；只有含檔名、bytes、內容雜湊與 provider 中繼資料的 schema v2 `PCV-` 才可用 `--accept-new-tip` 接受單調鏈尾，舊版 schema v1 PCV 只保留歷史讀取相容。檢查點保存目前 PRD、PRB、已證明的 PDR 前綴與來源 provider file ID 雜湊；PCV 私下保存 PDC 的內容級回讀證據；去識別健康摘要只保存筆數、往返布林值、確認年齡與問題代碼。PDC 串鏈、PCO／PCV、staging、provider ID、檔名、雜湊及私有錨點不得發布。後續同 release 的 PDR 追加可沿用既有 PDC；新 PRD／PRB 或 PCV 超過 8 日必須重新以 API 確認。這項證據仍不等同 Google 提供簽章雜湊、WORM、第三方不可刪除保存、身分簽章或雲端帳號安全保證。
