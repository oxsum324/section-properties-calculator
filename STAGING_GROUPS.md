...
每日健康排程由 `manage-public-release-decision-backup-health-task.ps1` 管理，固定使用目前登入者、`Interactive`、`Limited`、隱藏 PowerShell 視窗、禁止重疊並啟用漏跑補執行；Install 必須先以 `run-public-release-decision-backup-health.ps1` 完成一次 `--require-external` 真實健康檢查，失敗不得註冊。Status 要核對 task action、runner 絕對路徑、每日 trigger 與非 Disabled 狀態；Remove 只移除固定 task，不刪備份、收據或健康摘要。排程證明僅限本機執行與掛載目錄可讀，不得宣稱雲端服務端同步完成。

`public-release-decision-restore-drill.js` 必須在正式 release 的鏡像健康檢查通過後，從成對的目前外部 PRB 建立系統暫存隔離工作區，只複製 tracked 鏈尖錨點後實際套用完整還原，重驗收據數與鏈尖、證明來源決策檔案未變並完成安全清理。成功才可建立封閉 `PDR-` 私人演練收據；本機與外部 `restore-drills/` 收據要以同一交易寫入，任一失敗須回滾當輪全部收據。ignored 演練狀態不得含路徑、PRB／PDR ID、雜湊或收據內容。沒有外部設定的可攜環境可明示 local-only 演練；工作站與排程必須使用 `--require-external`。

每週隔離還原排程由 `manage-public-release-decision-restore-drill-task.ps1` 管理，預設每週日 09:15；Install 必須先執行一次外部鏡像真實還原，Status 要核對唯一 action、runner 絕對路徑、weekly trigger、星期、時間、目前使用者 `Interactive`／`Limited`、禁止重疊與非 Disabled 狀態。Remove 只移除固定 task，不刪 PRB、PDR 或狀態摘要。演練只證明目前掛載副本可在隔離環境重建決策鏈，不代表雲端服務端同步、第三方保存或工程核可。
