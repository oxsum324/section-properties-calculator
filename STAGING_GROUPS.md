...
- `SRC工具/`：第一階段納入 `core/src-beam-core.js`、`src-beam-core.test.js`、`src-traceability.catalog.json`、`README.md` 與 `結構工具箱/tools/independent-engineering-adapters/src-beam-candidate.js`。這些檔案與獨立基準 catalog／runner／test、成熟度矩陣、`preflight-tools.ps1`、根目錄 README／邊界文件、首頁建置中狀態及 browser smoke 必須一起 staging。正式入口仍固定 31 / 31；SRC 只以 `candidateBenchmarks` 記為 1 / 1 個非公開候選能力。候選失敗須阻擋獨立基準 gate，但不得增加正式入口計數、首頁工具卡、clean route 或正式工具宣稱；官方教材例題只能作 golden regression，不能取代獨立工程基準。
