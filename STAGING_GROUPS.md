...
- `SRC工具/` 的 SRC 柱研究工作頁須將 `src-column.html`、`src-column.css`、`src-column.js`、頁面契約／瀏覽器測試、核心、catalog、oracle、外部弱軸剪力對照、所有對應測試、追溯 catalog、共用 `鋼筋混凝土/shared/pmsection.js`／測試、共用 `結構工具箱/core/ui/report.js` 與 `README.md` 一起 staging。外部 AISC G6 模組只供專案指定時在 HTML 對照，不得接入 production、報告、PDF 或指紋。研究頁不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；目前可輸出乾淨的內部審閱研究計算書，但 `formalApprovalAllowed=false` 必須維持，直到 catalog 擴充、現況材料與材證、臺灣 SRC 路徑下的弱軸自動推導、第 9.3 節專案拉力強度／傳力容量來源、全構架所有接頭、接頭區剪力與接合細部、其餘耐震細節與正式計算書升格審查均完成。
