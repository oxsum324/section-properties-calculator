...
git add -- "SRC工具/core/src-column-oracle.js" "SRC工具/src-column-oracle.test.js"
...
- `SRC工具/` 的 SRC 柱研究核心須將 `core/src-column-core.js`、`core/src-column-oracle.js`、`src-column-core.test.js`、`src-column-oracle.test.js`、`src-column-traceability.catalog.json` 與 `README.md` 一起 staging。它不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；表 3.4-2 寬厚比與第一階段獨立鋼骨 oracle 已完成，仍須完成型鋼性質來源核對、獨立 RC P-M、雙向互制、剪力與計算書邊界，才可另案評估正式入口。
