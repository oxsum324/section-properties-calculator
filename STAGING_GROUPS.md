...
- 公開狀態宣告一致性：首頁 44 個工具 metadata 是唯一公開清冊；RC 家族入口只重述一致版本與 formal 狀態；鋼構四個正式版本對齊共用 metadata；風力／地震 17 個入口對齊 `FormalToolMetadata`，報告將公開語意版本與內部計算引擎分欄；連續梁、平面剛架、斷面性質與合成斷面對齊 `AnalysisSectionToolMetadata`，畫面、案件 JSON 與計算書分列公開版本／內嵌計算引擎；石材畫面、核心與報告對齊 `StonePublicMetadata`，不得殘留舊 `V2`／`V2.1`；錨栓公開版本與計算引擎 build 分欄；覆工板頁面、JSON、報表共用 metadata；開挖維持服務型且不固化通過數；舊網址相容入口不得再保存第二份版本、能力、日期或通過狀態。`public-status-claims-contract` 必須留在 preflight summary 與 Global Governance Gates，Pages HTTP／Playwright 需覆蓋 canonical 版本、私有路徑排除及 RC／分析／斷面／鋼構／風力／地震／石材／覆工板／開挖／舊網址入口的直接列印封鎖。
- 報告閱讀狀態：`頁面專用`，page-only boundary `4/4`、可讀文字 `17/17`、瀏覽器 smoke `2/2`、首頁正式工具實際交付物渲染 `32/32`、補充報告 / 服務成品 `2/2`、成品檔案完整性 `143/143`、風力／地震數值結果鏈 `14/14`、RC 結果鏈 `34/34`、RC 來源組包 `32/32`、RC 核可 HTML 列印 `34/34`、RC HTML 內容封印 `34/34`、鋼構結果鏈 `5/5`、石材結果鏈 `1/1`、錨栓結果鏈 `1/1`、覆工板結果鏈 `1/1`、開挖結果鏈 `1/1`，issue `0`；此總覽只能出現在頁面或工具本身，不得附入計算書、列印輸出或 PDF，也不得寫入 Word / DOCX 或 workbook。公開的 143 份只分成正式 PDF／證據 `64`、RC PDF／PNG `66` 與混合格式附件 `13` 三類計數；結果鏈、RC 來源組包、核可 HTML 列印及內容封印也只公開完成數，不得帶出 scope、檔名、工具版本、bytes、雜湊、golden case 內容、RC 案例資料、補強表單快照、鋼構／石材／錨栓／覆工板／開挖來源資料、預期數值、重現指紋、成品雜湊或計算指紋。
...
```powershell
git add -- "結構工具箱/tools/public-status-claims.contract.test.js" "鋼筋混凝土/index.html" "結構工具箱/index-classic.html"
...
git add -- "結構工具箱/tools/pages-live-smoke.js" "結構工具箱/tools/pages-live-browser-smoke.js" "結構工具箱/tools/run-pages-browser-smoke.sh" "結構工具箱/tools/build-pages-artifact.js" "結構工具箱/tools/build-pages-clean-routes.js" "結構工具箱/tools/build-pages-deployment-manifest.js" "結構工具箱/tools/verify-pages-release-lineage.js" "結構工具箱/tools/tool-maturity-matrix.js" "結構工具箱/tools/public-evidence-schema.test.js" "結構工具箱/tools/public-release-change-governance.test.js" "結構工具箱/tools/public-release-change-assistant.js" "結構工具箱/tools/public-release-change-assistant.test.js" "結構工具箱/tools/public-release-decision-receipt.js" "結構工具箱/tools/public-release-decision-receipt.test.js" "結構工具箱/tools/public-release-decision-backup.js" "結構工具箱/tools/public-release-decision-backup.test.js" "結構工具箱/tools/public-release-decision-backup-health.js" "結構工具箱/tools/public-release-decision-backup-health.test.js" "結構工具箱/tools/public-release-decision-backup-task.test.js" "結構工具箱/tools/run-public-release-decision-backup-health.ps1" "結構工具箱/tools/manage-public-release-decision-backup-health-task.ps1" "結構工具箱/tools/public-release-decision-restore-drill.js" "結構工具箱/tools/public-release-decision-restore-drill.test.js" "結構工具箱/tools/public-release-decision-restore-drill-health.js" "結構工具箱/tools/public-release-decision-restore-drill-health.test.js" "結構工具箱/tools/public-release-decision-cloud-checkpoint.js" "結構工具箱/tools/public-release-decision-cloud-checkpoint.test.js" "結構工具箱/tools/public-release-decision-restore-drill-task.test.js" "結構工具箱/tools/run-public-release-decision-restore-drill.ps1" "結構工具箱/tools/manage-public-release-decision-restore-drill-task.ps1"
git add -- ".github/pages-smoke/normalize-playwright-result.js"
git add -- ".github/public-release-reduction-authorization.json" ".github/public-release-decision-anchor.json" "結構工具箱/assets/status/public-evidence-schema.js" "結構工具箱/assets/status/platform-status.json" "結構工具箱/assets/status/preflight-summary.json" "結構工具箱/assets/status/report-readiness-status.json"
```
...
正式 release 在收據完成後必須以 `public-release-decision-backup.js --export` 自動產生封閉 `PRB-` 可攜備份；預設寫入 ignored `output/private-backups/public-release-decisions/`，也可由工作站私下設定 `PUBLIC_RELEASE_DECISION_BACKUP_DIR` 指向受控同步資料夾，設定值不得寫入 preflight、Git 或公開狀態。`--restore` 預設只預覽，只有再加 `--apply` 才能還原；還原僅接受完全相同鏈、缺少的 reset 或目前鏈的向前延伸。乾淨 clone 只有 Git 錨點時，備份鏈尖必須與該錨點精確相符；較舊、分叉、竄改、覆寫既有收據或交易中途失敗均拒絕並回滾。PRB、PRD、PRA 及其路徑、雜湊與內容都不得 staging 或發布。

設定 `PUBLIC_RELEASE_DECISION_BACKUP_DIR` 後，自動匯出不得把外部位置取代本機副本，而要用同一個 PRB ID／內容同時建立 ignored 本機副本與外部鏡像；任一寫入失敗須移除當輪已新增的全部副本。`public-release-decision-backup-health.js` 在每次正式 release 匯出後驗證目前鏈尖、本機與外部有效副本、相同 PRB 鏡像配對、無損毀檔及 365 日保存提醒，並只把不含路徑、PRB ID、雜湊或收據內容的 ignored 摘要寫到 `output/audit/public-release-decision-backup-health.json`。未設定外部位置時可明示 `local-only`；工作站治理檢查應使用 `--require-external`，相同資料夾不得冒充雙副本，過期檔只提醒人工處理，不得自動刪除。

每日健康排程由 `manage-public-release-decision-backup-health-task.ps1` 管理，固定使用目前登入者、`Interactive`、`Limited`、隱藏 PowerShell 視窗、禁止重疊並啟用漏跑補執行；Install 必須先以 `run-public-release-decision-backup-health.ps1` 完成外部 PRB 雙副本、PDR 兩地串鏈、單調檢查點、目前決策鏈尖、8 日演練時效，以及目前 PDC provider 往返證據的真實健康檢查，失敗不得註冊。Status 要核對 task action、runner 絕對路徑、每日 trigger 與非 Disabled 狀態；Remove 只移除固定 task，不刪備份、收據、檢查點或健康摘要。排程只能驗證最近一次 provider API 確認及其回同步檔案；新 release 或超過 8 日未重新確認必須提醒，不得把掛載目錄可讀冒充新的服務端查詢。

`public-release-decision-restore-drill.js` 必須在正式 release 的鏡像健康檢查通過後，從成對的目前外部 PRB 建立系統暫存隔離工作區，只複製 tracked 鏈尖錨點後實際套用完整還原，重驗收據數與鏈尖、證明來源決策檔案未變並完成安全清理。既有 schema v1 PDR 原樣保留；新 schema v2 PDR 以連續序號與前筆 ID／canonical SHA-256 鏈回既有鏈尖。本機與外部 `restore-drills/` 收據要與 ignored 私有鏈尾錨點 `output/audit/public-release-decision-restore-drill-anchor.json` 以同一交易更新並於寫後重驗整條兩地歷史；任一失敗須回滾當輪全部收據及原錨點。schema v2 ignored 去識別狀態只保存單調歷史筆數與演練結果，不得含路徑、PRB／PDR ID、雜湊或收據內容。`public-release-decision-restore-drill-health.js` 再核對兩地完整配對、鏈內刪除、雙地鏈尾回退、私有錨點、目前 PRD 鏈尖與預設 8 日時效；工作站與排程必須使用 `--require-external`，沒有外部設定的可攜環境才可明示 local-only。

每週隔離還原排程由 `manage-public-release-decision-restore-drill-task.ps1` 管理，預設每週日 09:15；Install 必須先執行一次外部鏡像真實還原，並通過 PDR 串鏈與目前 PDC provider 往返健康檢查。Status 要核對唯一 action、runner 絕對路徑、weekly trigger、星期、時間、目前使用者 `Interactive`／`Limited`、禁止重疊與非 Disabled 狀態。Remove 只移除固定 task，不刪 PRB、PDR、PDC 或狀態摘要。演練證明掛載副本可在隔離環境重建決策鏈；PDC 另證明最近一次 provider API 觀察與回同步，但兩者都不等同第三方 WORM 保存、身分簽章或工程核可。

`public-release-decision-cloud-checkpoint.js` 必須以 provider API 直接列出的目前 PRB／PDR 中繼資料建立封閉 `PDC-` 檢查點。因目前 Drive 連線採唯讀授權，PDC 經 DriveFS 寫入私人 `cloud-checkpoints/` 後，必須再由 API 下載服務端原始檔；`--confirm` 必須取得獨立於外部掛載目錄的 `--provider-content-file`，由 CLI 自行核對實際 bytes 並重算 SHA-256。只有含 `contentReadback=provider-download`、檔名、bytes、內容雜湊與 provider 中繼資料的 schema v3 `PCV-` 才可用 `--accept-new-tip` 接受單調鏈尾；schema v1 中繼資料與 v2 宣告雜湊只保留歷史讀取相容。檢查點保存目前 PRD、PRB、已證明的 PDR 前綴與來源 provider file ID 雜湊；PCV 私下保存 PDC 的內容級回讀證據；去識別健康摘要只保存筆數、往返布林值、確認年齡與問題代碼。PDC 串鏈、PCO／PCV、provider 原始下載檔、staging、provider ID、檔名、雜湊及私有錨點不得發布。後續同 release 的 PDR 追加可沿用既有 PDC；新 PRD／PRB 或 PCV 超過 8 日必須重新以 API 確認。這項證據仍不等同 Google 提供簽章雜湊、WORM、第三方不可刪除保存、身分簽章或雲端帳號安全保證。
...
```powershell
git add -- .gitignore README.md TOOL_BOUNDARIES.md TOOL_REPORT_GUIDE.md STAGING_GROUPS.md CONTEXT.md docs/ docs/adr/0001-page-only-report-readiness.md .github/workflows/pages-deploy.yml .github/workflows/pr-validation.yml preflight-tools.ps1 run-preflight-tools.bat run-preflight-tools-quick.bat run-preflight-tools-ci.bat run-preflight-tools-release.bat run-pages-artifact-smoke.ps1 push-pages-release.ps1 push-pages-release.bat sync-anchor-deployment.ps1 continuous-beam-regression.test.js test-continuous-beam.ps1 "連續梁分析.html" browser-dialogs.contract.test.js decking-tools.contract.test.js frame-analysis.contract.test.js pages-release-governance.contract.test.js pr-validation.contract.test.js section-tools.contract.test.js stone-feedback.contract.test.js struct-dx.contract.test.js toolbox-entrypoints.contract.test.js "SRC工具/core/src-beam-core.js" "SRC工具/src-beam.html" "SRC工具/src-beam.css" "SRC工具/src-beam.js" "SRC工具/src-beam-core.test.js" "SRC工具/src-beam.contract.test.js" "SRC工具/src-beam-browser-smoke.test.js" "SRC工具/src-traceability.catalog.json" "SRC工具/core/src-column-core.js" "SRC工具/src-column.html" "SRC工具/src-column.css" "SRC工具/src-column.js" "SRC工具/src-column-page.contract.test.js" "SRC工具/src-column-browser-smoke.test.js" "SRC工具/src-column-core.test.js" "SRC工具/src-column-traceability.catalog.json" "SRC工具/README.md" "結構工具箱/tools/independent-engineering-adapters/src-beam.js"
git add -- "SRC工具/core/src-column-h-section-catalog.js" "SRC工具/core/src-column-rc-biaxial.js" "SRC工具/core/src-column-shear.js" "SRC工具/core/src-column-weak-axis-shear-reference.js" "SRC工具/core/src-column-seismic-axial.js" "SRC工具/core/src-column-seismic-detailing.js" "SRC工具/core/src-column-oracle.js" "SRC工具/src-column-h-section-catalog.test.js" "SRC工具/src-column-rc-biaxial.test.js" "SRC工具/src-column-shear.test.js" "SRC工具/src-column-seismic-axial.test.js" "SRC工具/src-column-seismic-detailing.test.js" "SRC工具/src-column-oracle.test.js"
git add -- run-all-tests.ps1 run-all-tests.bat run-all-tests.contract.test.js "結構工具箱/tools/external-tool-resolver.js" "結構工具箱/tools/external-tool-resolver.test.js"
git add -- serve-local.js serve-local-browser-smoke.test.js "啟動斷面計算工具.bat" "啟動螺栓檢討工具.bat" "結構工具箱/assets/home/home.js"
...
- `SRC工具/`：正式 `/src-beam` 須一起納入 `core/src-beam-core.js`、`src-beam.html`、`src-beam.css`、`src-beam.js`、三個 core／contract／browser smoke 測試、`src-traceability.catalog.json`、`README.md` 與 `結構工具箱/tools/independent-engineering-adapters/src-beam.js`。這些檔案與獨立基準 catalog／runner／test、rendered delivery inventory／aggregate、`preflight-tools.ps1`、`vercel.json`、首頁卡片與根目錄文件必須一起 staging；瀏覽器輸出的 PNG／PDF／案件 JSON 仍留在 ignored output，不得提交。正式入口為 32 / 32；release 當輪必須在 `src-formal` 保存 PDF、來源 JSON、計算指紋與雜湊證據，且 Pages 須驗證首頁卡片、clean route 與私有證據邊界。
- `SRC工具/` 的 SRC 柱私有研究工作頁須將 `src-column.html`、`src-column.css`、`src-column.js`、頁面契約／瀏覽器測試、核心、catalog、oracle、外部弱軸剪力對照、所有對應測試、追溯 catalog、共用 `鋼筋混凝土/shared/pmsection.js`／測試、共用 `結構工具箱/core/ui/report.js` 與根目錄文件一起 staging。V0.8 的單向案件 schema v7、雙向案件 schema v2 與 traceability schema v16 的責任矩陣／材料來源政策必須同時驗證；雙向配對須比對共同構材身分、X／Y 子指紋與雙向總指紋。本附件、主文／專案引用輸入及另案附件的三個工作包不得混併；官方教材斷面性質、專案指定設計材料、現況供貨與材證亦須分層。計算書只列採用的教材表頁、catalog 版本與設計值，責任矩陣及供貨／材證說明只留在 HTML，不能成為 DRAFT 警告或構材 NG。專案未採用 AISC G6 時，外部對照仍不得進報告、PDF 或指紋；專案明確選定並確認適用時，production 才採用受控 G6 路徑，報告必須列採用依據、代入與結果。研究頁不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；`src-column-traceability.catalog.json.formalPromotionReadiness.blocking` 非空時，`formalApprovalAllowed=false`、`research-core-not-public` 與 Pages 私有邊界必須維持。
...
共用計算書文件狀態：調整 `結構工具箱/core/ui/report.js` 的核可方塊、選填核可人／核可依據、內部審閱／正式附件頁尾、目前版本 HTML 下載、PDF 預設標題、空白案件欄位省略或輸入變更撤銷核可時，必須同步 staging `鋼構工具/core/ui/report.js`、`鋼筋混凝土/shared/report.js`、`section-tools.contract.test.js`、`continuous-beam-regression.test.js`、`frame-analysis.contract.test.js`、附件檢查器與反向測試、相關頁面、README、邊界文件、ADR、首頁／矩陣／dashboard 說明與 browser smoke。下載 HTML 必須保存唯一且不執行 JavaScript 也能辨識的文件狀態列、核可時間、已填核可紀錄與計算指紋，移除互動控制，重新開啟時沿用該列且不得重複插入狀態 UI；核可人與依據留空不得降低正式附件身分，填寫後則必須受核可封印保護。分頁標題必須由穩定報告名稱、目前文件狀態與指紋組成，不得在重新載入或切換時重複串接；頁面診斷明細仍維持 page-only。
...
```powershell
git add -- "石材固定/石材計算書產生器_規範版V2.html" "石材固定/server.py" "石材固定/self_check.py" "石材固定/ui_smoke_test.py" "石材固定/js/version-sync.js" "石材固定/js/version-sync-smoke.test.js" "石材固定/js/code-profiles-registry.spec.js" "石材固定/js/code-profiles-registry-smoke.test.js" "石材固定/js/regression-smoke.test.js"
...
git add -- "結構工具箱/core/ui/report.js" "結構工具箱/core/direct-print-boundary.css" "結構工具箱/core/loads/loadcombo.js" "結構工具箱/tools/local-quick-tools.manifest.json" "結構工具箱/tools/local-quick-tool-metadata.js" "結構工具箱/tools/local-quick-tools.run.js" "結構工具箱/tools/local-quick-export.js" "結構工具箱/tools/local-quick-export.test.js" "結構工具箱/tools/local-quick-output-consistency.test.js" "結構工具箱/tools/loadcombo-v2.test.js" "結構工具箱/tools/local-quick-browser-smoke.test.js" "結構工具箱/tools/project-meta-profile.js" "結構工具箱/tools/project-meta-profile.test.js" "結構工具箱/tools/project-storage.test.js" "結構工具箱/tools/formal-tools.manifest.json" "結構工具箱/tools/formal-traceability.catalog.json" "結構工具箱/tools/formal-traceability.contract.test.js" "結構工具箱/tools/formal-tools.run.js" "結構工具箱/tools/formal-tools.contract.test.js" "結構工具箱/tools/formal-browser-smoke.test.js" "結構工具箱/tools/calculation-book-content-boundary.js" "結構工具箱/tools/rendered-delivery-evidence.js" "結構工具箱/tools/rendered-delivery-evidence.contract.test.js" "結構工具箱/tools/rendered-delivery-evidence.inventory.json" "dev_tools/attachment-integrity-diagnostic.js" "結構工具箱/tools/attachment-integrity-diagnostic.test.js" "dev_tools/html-attachment-integrity.js" "結構工具箱/tools/html-attachment-integrity.test.js" "結構工具箱/tools/report-disclosure.contract.test.js" "結構工具箱/tools/delivery-artifacts.contract.test.js" "結構工具箱/tools/release-readiness.contract.test.js" "結構工具箱/tools/tool-maturity-matrix.js" "結構工具箱/tools/local-quick-tools.contract.test.js"
...
- `結構工具箱/tools/rendered-delivery-evidence.js` 與 formal / local / steel / SRC browser runner 必須同批 staging；release 證據需落在當輪 `PREFLIGHT_RUN_DIR/rendered-delivery-evidence/`，不得只留下 HTML 字串檢查，且多頁 PDF 必須拒絕頁尾與資料列混在同一文字行、頁尾孤立章節標題、從孤立公式 / 單一資料列 / 無標籤片段起頁，以及文字量與渲染墨量同時過低的稀疏末頁。錨栓正式報告證據改動需同批 staging `螺栓檢討/bolt-review-tool/tests/reportArtifacts.test.ts`、anchor report contract、rendered inventory 與 aggregate / release contract，並保留當輪 HTML、DOCX、XLSX 供總閘門解析 Office XML，不能只用 `anchor-report-contract` 日誌代替成品。石材正式報告證據改動則需同批 staging `石材固定/auto_word_artifact_test.py` 及相同治理檔，並保留當輪 PDF、DOCX、audit JSON。覆工板正式報告證據改動需同批 staging `覆工板/decking-report.contract.test.js`、rendered inventory、aggregate / delivery / release contract 與覆工板 README，並在 `decking-formal` 保留當輪 DOCX 與結構摘要供總閘門重新解析，不能依賴 `output/preflight/cover-slab-report-contract.docx` 或通過日誌。開挖正式報告證據改動需同批 staging `開挖擋土支撐/excavation-report.contract.test.js`、`backend/tests/release_report_artifacts.py`、aggregate / delivery / release contract、成熟度矩陣、首頁狀態顯示與開挖 README；`excavation-formal` 必須保留當輪 PDF、DOCX 與 latest download 副本。動力分析摘要證據改動需同批 staging `formal-browser-smoke.test.js`（若產報本體有變）、aggregate / release contract、成熟度矩陣、首頁狀態顯示與相關文件，並重新解析 `seismic-dynamic-formal-report.pdf` 及 evidence JSON。兩者合計為補充報告 / 服務成品 `2/2`，與首頁 `32/32` 分開計數。共用報告樣式與 RC 視覺 PDF 必須同批納入，確保標題跟隨內容、表格列不拆頁、跨頁表頭重複，以及 `uncontextualPageStartCount=0`；個別 RC 輸入群組若使用 `keepTogether`，對應工具頁與共用 report helper 也必須同批 staging。
- Canonical rendered evidence aggregate 改動必須同批 staging `rendered-delivery-evidence.js`、aggregate contract、三個 producer browser runner 的既有 `writeEvidenceSummary` 呼叫路徑、`release-readiness.contract.test.js`、`tool-maturity-matrix.js`、`assets/home/home.js`、`pages-live-smoke.js` 與報告邊界文件。Schema v3 固定要求 32 組 PDF／evidence、64 份實體檔全數通過；schema v14 納入群樁側向分配 RC 基礎案例後，RC 視覺固定 `66`、混合格式固定 `13`，彙成「成品檔案完整性 `143/143`」的匿名計數。Family summary 缺 PDF 或 evidence bytes／SHA-256、總閘門未去重重驗、或同大小替換負向 fixture 未封鎖時不得 release；公開狀態不得複製 aggregate 私有欄位、scope、artifact 清冊或 set hash。
...
git add -- "結構工具箱/core/ui/report.js" "鋼筋混凝土/shared/report.js" "結構工具箱/tools/calculation-book-content-boundary.json" "結構工具箱/tools/calculation-book-content-boundary.js" "結構工具箱/tools/formal-tool-metadata.js" "結構工具箱/tools/formal-tools.manifest.json" "結構工具箱/tools/formal-tools.contract.test.js" "結構工具箱/tools/report-approval-invalidation.catalog.json" "結構工具箱/tools/report-approval-invalidation.contract.test.js" "結構工具箱/tools/report-disclosure.contract.test.js"
