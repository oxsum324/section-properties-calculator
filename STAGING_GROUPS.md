...
每日健康排程由 `manage-public-release-decision-backup-health-task.ps1` 管理，固定使用目前登入者、`Interactive`、`Limited`、隱藏 PowerShell 視窗、禁止重疊並啟用漏跑補執行；Install 必須先以 `run-public-release-decision-backup-health.ps1` 完成外部 PRB 雙副本、PDR 兩地串鏈、單調檢查點、目前決策鏈尖及 8 日演練時效的真實健康檢查，失敗不得註冊。Status 要核對 task action、runner 絕對路徑、每日 trigger 與非 Disabled 狀態；Remove 只移除固定 task，不刪備份、收據或健康摘要。排程證明僅限本機執行與掛載目錄可讀，不得宣稱雲端服務端同步完成。
...
每週隔離還原排程由 `manage-public-release-decision-restore-drill-task.ps1` 管理，預設每週日 09:15；Install 必須先執行一次外部鏡像真實還原及串鏈健康檢查，Status 要核對唯一 action、runner 絕對路徑、weekly trigger、星期、時間、目前使用者 `Interactive`／`Limited`、禁止重疊與非 Disabled 狀態。Remove 只移除固定 task，不刪 PRB、PDR 或狀態摘要。演練只證明目前掛載副本可在隔離環境重建決策鏈，不代表雲端服務端同步、第三方保存或工程核可。
