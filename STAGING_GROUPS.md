...
- `SRC工具/`：候選垂直切片須一起納入 `core/src-beam-core.js`、`src-beam.html`、`src-beam.css`、`src-beam.js`、三個 core／contract／browser smoke 測試、`src-traceability.catalog.json`、`README.md` 與 `結構工具箱/tools/independent-engineering-adapters/src-beam-candidate.js`。這些檔案與獨立基準 catalog／runner／test、`preflight-tools.ps1`、根目錄 README／邊界文件及首頁建置中狀態必須一起 staging；瀏覽器輸出的 PNG／PDF／案件 JSON 仍留在 ignored output，不得提交。正式入口仍固定 31 / 31；SRC 只以 `candidateBenchmarks` 記為 1 / 1 個非公開候選能力。候選失敗須阻擋獨立工程基準 gate，但在完整 release 升格前不得增加正式入口計數、首頁工具卡、clean route 或公開工具宣稱；官方教材例題只能作 golden regression，不能取代獨立工程基準。
