...
- `SRC工具/` 的 SRC 柱研究核心須將 `core/src-column-core.js`、`src-column-core.test.js`、`src-column-traceability.catalog.json` 與 `README.md` 一起 staging。它不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；只有寬厚比、雙向互制、剪力、計算書邊界與獨立 oracle 都完成後，才可另案評估正式入口。
