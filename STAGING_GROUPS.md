...
git add -- "SRC工具/core/src-column-h-section-catalog.js" "SRC工具/core/src-column-oracle.js" "SRC工具/src-column-h-section-catalog.test.js" "SRC工具/src-column-oracle.test.js"
...
- `SRC工具/` 的 SRC 柱研究核心須將 `core/src-column-core.js`、`core/src-column-h-section-catalog.js`、`core/src-column-oracle.js`、`src-column-core.test.js`、`src-column-h-section-catalog.test.js`、`src-column-oracle.test.js`、`src-column-traceability.catalog.json`、共用 `鋼筋混凝土/shared/pmsection.js`／測試與 `README.md` 一起 staging。它不得連同首頁、Pages route、正式 manifest、rendered delivery inventory 或正式工具數一起升格；表 3.4-2 寬厚比、獨立鋼骨與連續中性軸 RC P-M oracle，以及教材附錄 H500×300 群組 7 筆的來源／頁碼鎖定與衝突阻擋已完成。catalog 擴充、現況材料與材證核對、雙向互制、剪力與計算書邊界仍須完成，才可另案評估正式入口。
