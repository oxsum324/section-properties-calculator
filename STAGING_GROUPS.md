...
- `SRC工具/`：第一階段只納入 `core/src-beam-core.js`、`src-beam-core.test.js`、`src-traceability.catalog.json` 與 `README.md`。四檔與 `preflight-tools.ps1`、根目錄 README／邊界文件、首頁建置中狀態及 browser smoke 必須一起 staging。未建立公開 UI、正式計算書與核可流程前，不得新增首頁工具卡、clean route 或正式工具宣稱；官方教材例題只能作 golden regression，不能取代獨立工程基準。
