...
此組需一起提交，避免只有測試案例卻沒有 production core、preflight、成熟度與報告邊界。試辦通過只可標示 20 / 20，整體正式入口需同時標示 20 / 31；golden case 或同核心結果鏈不等同獨立工程驗證，也不得算入獨立覆蓋。第十九條路由為正式區域風壓 C&C，獨立核對 18 m 高度界線、部分封閉內壓高度及屋面角隅負壓提前收斂；第二十條路由為正式女兒牆風壓，獨立核對 MWFRS 式 2.3、圖 3.4 與圖 3.5 三條設計路線。既定 P0／P1 基準路由已完成；分析摘要身分的 `/seismic-dynamic` 不得計入正式附件入口覆蓋。
