...
局部快算計算書文件狀態變更必須把三個頁面、`local-quick-tools.manifest.json`、`core/ui/report.js`、共同契約、Edge/CDP browser smoke、成熟度矩陣與上述文件一起 staging。smoke 必須同時驗證完整案件與缺件案件的實際輸出；review / blocked 計算書應帶 DRAFT，ready 計算書不得帶 DRAFT，工作頁直接列印仍只允許邊界通知。
...
若本包調整風力 / 地震正式工具的 HTML 彈窗型計算書，`formal-tools.contract.test.js` 需同步保留輸出文字抽檢、共用 DRAFT／ready 文件狀態與 page-only wording 排除規則；`formal-browser-smoke.test.js` 必須實測完整案件不帶 DRAFT、缺件案件帶 DRAFT。調整案件 JSON 時，14 頁均須驗證完全相同的 schema、工具與版本，並實測正常重現、錯版本不改畫面及錯指紋回復原輸入。只檢查 HTML 片段或 popup 是否開啟，不足以作為正式報告或來源回放邊界證據。
