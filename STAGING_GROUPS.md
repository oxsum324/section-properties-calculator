...
若本包調整風力 / 地震正式工具的 HTML 彈窗型計算書，`formal-tools.contract.test.js` 需同步保留輸出文字抽檢、共用 DRAFT／ready 文件狀態與 page-only wording 排除規則；`formal-browser-smoke.test.js` 必須實測完整案件不帶 DRAFT、缺件案件帶 DRAFT。只檢查 HTML 片段或 popup 是否開啟，不足以作為正式報告邊界證據。
