...
- RC 群樁側向分配改動必須同批 staging 側向分配核心與單元測試、RC 基礎 producer／案例／contract、traceability catalog、rendered evidence inventory／aggregate、release contract、成熟度矩陣、首頁狀態、Pages live smoke 與報告邊界文件。Schema v14 固定要求 RC 結果鏈 `34/34`、RC PDF／PNG `66/66` 與 RC HTML `34/34`；p-multiplier 分配證據不得取代專業 p-y 位移與樁身內力分析。
