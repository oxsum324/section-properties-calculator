...
- 樁基報告須揭露土層與單樁承載、群樁構造、服務性與樁身、樁帽結構四類檢核；輸入水平力時另輸出 FHWA / AASHTO p-multiplier、各列與最大單樁水平力。專業 p-y JSON 或 Excel／CSV 表格換算結果通過固定單位、樁群模型、分析範圍與實際分析 Hx／Hy 核對且經工程師明確採用後，計算書直接列出位移、剪力、彎矩、容量、分析識別、來源列數與雜湊；LPile 類單樁結果使用代表單樁 p-multiplier 荷重，群樁結果使用整組荷重。未採用相符結果時維持 NG。
