...
async function main() {
  ...
  try {
    for (const tc of cases) {
      ...
      const portableHtml = await assertPortableFormalHtml(report, `${tc.key} report`, assert, { outputDir: OUT_DIR, sourceSnapshot });
    ...
    const portableHtml = await assertPortableFormalHtml(report, 'resolved review report', assert, { outputDir: OUT_DIR, sourceSnapshot: resolvedSourceSnapshot });
    results[results.length - 1].portableHtml = portableHtml;
    await report.close();
    await page.close();
  } finally {
