...
- 載重組合完整向量 regression：
  ...
  - 多內力載重組合保留同一個實際組合列的來源完整有號 tuple；不得分別取各分量最大絕對值後拼成不存在的設計內力。RC 梁／柱／牆／剪力牆與舊案鋼梁／鋼柱的工作頁會依正／負彎矩、剪力、扭矩或正規化 P-M 需求向量列出控制組合候選；候選只作頁面工況篩選，P-M 容量仍由構件檢核判定。使用者選取候選後，系統套用的仍是該列全部有號內力；寫入時再依構件欄位語意映射，來源 tuple 的符號與組合係數完整保存並列入計算書。控制候選與篩選說明不進計算書。RC 板單一 `wu` 保留原流程。
- 內力 Picker（內部流程）：
