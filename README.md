...
- GitHub Pages deploy / live smoke：
  ...
  - 由 `.github/workflows/pages-deploy.yml` 在 `master` push 後以 GitHub Actions 發布 Pages artifact；staging 完成後，`build-pages-clean-routes.js` 依 `vercel.json` 為 GitHub Pages 產生不覆寫既有頁面的靜態短網址轉向頁，使首頁公開的 `/rc-column`、`/steel-beam-formal` 等 clean route 在 Pages 專案子路徑下也能使用，並保留 query 與 hash。workflow 先以短暫 HTTP server 直接服務 staged `_site`，在 archive / upload / deploy 前完成全部 43 個首頁 route、公開狀態、私有邊界與 Playwright 86 組真實瀏覽；任一錯誤都會阻擋壞版上線。deploy 成功後再對正式網址重跑相同 HTTP 與 browser smoke，形成上線前阻擋與上線後確認。共用 runner 以 1280 × 800 與 390 × 844 實際瀏覽全部 route，攔截 console / page / request / HTTP 錯誤、整頁橫向溢位，並專項驗證 RC 單樁寬表、風壓參考圖及石材 A4 局部捲動；CLI 必須依 JSON `isError` 判定，不得只採用程序碼。`--check-private-boundary` 會確認 Markdown、`CONTEXT.md`、`docs/adr/`、PowerShell / batch、測試檔、合約檔、route builder、browser smoke source / runner、`dev_tools/`、Python / TypeScript source、backend、package manifest 與本機註冊檔未被發布。部署前也可先跑 [run-pages-artifact-smoke.ps1](/C:/Users/USER/Desktop/AI/小工具製作/run-pages-artifact-smoke.ps1:1)，它會在本機 temp 目錄產生同樣含 clean route 的 Pages artifact，依序呼叫共享 HTTP 與瀏覽器 smoke。
