...
SRC 梁已建立第一版純計算核心、現行規範追溯 catalog、官方教材例題、失敗封閉反例，以及不讀取 production golden 答案的獨立工程候選基準，並納入 preflight。目前仍只能視為「核心建置中」；在補上公開 UI、正式計算書、文件核可與完整發布證據前，不提供工具入口，也不宣稱可產出正式附件。
...
SRC 梁另以 catalog v2 的 `candidateBenchmarks` 登錄為 1 / 1 個非公開候選能力。production 核心以應變相容與二分法求中性軸，獨立 oracle 則在張力筋降伏、壓力筋彈性的明確適用域內用二次方程封閉解重算，並以 4 組案例、41 項斷言核對撓曲、寬厚比、鋼骨／RC 剪力分擔、一般剪力、剪力摩擦與失敗控制。候選失敗會阻擋整體基準 gate，但不計入正式工具 31 / 31、不建立首頁卡片或 `/src-beam` 路由，也不表示已具正式計算書或附件資格。
