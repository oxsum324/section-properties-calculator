...
SRC 柱目前只建立非公開的 `src-column.core.v0.1.0-research` 可審查核心：依官方第 6、7 章與建築研究所教材例題 8，重算剛度分配、鋼骨軸壓與壓彎互制、規範重新分配及 RC 應變相容 P-M。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；鋼骨寬厚比自動重算、雙向彎矩、柱剪力、柱腳／接頭與耐震細節完成前，不得當作正式附件。
