...
SRC 柱目前只建立非公開的 `src-column.core.v0.7.0-research` 可審查核心：依官方第 3、5、6、7、9 章、112 年混凝土結構設計規範與建築研究所教材例題 8／14，自動重算表 3.4-2 翼板／腹板寬厚比、x／y 向剛度分配、鋼骨雙軸壓彎互制、規範重新分配、RC 應變相容 P-M，以及第 9.6.2 節強軸耐震柱剪力、第 9.6.1 節單一強軸接頭強柱弱梁與第 9.6.3 節強軸矩形柱圍束子檢核。圍束量使用現行式 (9.6-6)／(9.6-7)，包含 `Phcc` 並採現行非圍束區 15 cm 上限；舊教材省略 `Phcc` 及 20 cm 舊值只保留為歷史算術邊界。剪力需求由 Mct、Mcb 與 hn 推得，鋼骨與 RC 依 Mns／Mprc 分別檢核，RC 同時計算一般剪力與剪力摩擦並取較小值；只支援常重混凝土、整體澆置、垂直閉合箍筋與剪力釘貢獻為零。雙軸 RC 核心採精確多邊形裁切，獨立 oracle 改用 320 條數值積分並另行重算三項耐震子檢核，共保存 84 項精確比對、14 項容許差比對及四組漂移哨兵。第一批斷面 catalog 鎖定教材附錄表 1-1 的 H500×300 群組 7 筆資料；衝突即阻擋，手動輸入固定複核。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；弱軸耐震路徑、第 9.3 節軸向強度、第 8.4.2 節接頭傳力比例、全構架所有接頭、柱腳／接頭、施工階段與其餘耐震細節完成前，不得當作正式附件。
