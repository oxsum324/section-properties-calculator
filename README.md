...
開挖擋土支撐的 ERH、RVR、SEV、SCV 與可選公開 RTB 也可和核可後計算書放在同一個組包來源資料夾。檢查器會把這五種 JSON 辨識為治理證據，不拿來冒充計算來源／報告配對；SCV 所列來源檔名、SHA-256、各層指紋、工程結果、身分採用狀態及非工程核可邊界必須一致。完整鏈會隨 v3 包保存在 `99_內部追溯_勿附入主報告/來源資料/`，正式計算書仍只進入 `01_正式附件/`；缺檔或內容替換 blocked，孤立且未被 SCV 連結的證據維持 review。組包器及事後驗證器都會重做這項檢查並只顯示完成組數。此關係檢查不重算 ERH／RVR／SEV 的受控內容或 Ed25519 簽章，正式複驗仍須使用包內來源檔重跑 `驗證SEV證據鏈.bat`。
