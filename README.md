...
- GitHub Pages deploy / live smoke：
  ...
  - 正式推送可執行 `push-pages-release.bat` 或 `powershell -ExecutionPolicy Bypass -File .\push-pages-release.ps1`。入口會先拒絕 dirty、錯誤分支與遠端分歧，只使用非 force push；推送後等待同一 SHA 的 push workflow，逾時且沒有既有 run 才使用既有 `workflow_dispatch` 後備入口。完成判定要求 build、deploy、live-smoke 三個 job 全部成功，並核對公開 `pages-deployment.json` 的 commit、runId 與 `sourceDirty=false`。若 GitHub Actions 頂層 run 狀態延遲但三個必要 job 已成功，結果會標示 `aggregateStatusStale=true`；若個別 job 聚合狀態延遲，則只有在 run 已 completed/success 且該 job 每一個 step 都 completed/success 時，才標示 `aggregateJobStatusStale=true` 並接受相同證據。任何 job 或 step 失敗都直接停止，不以另一次 dispatch 掩蓋。`-VerifyOnly` 可唯讀複驗現有 HEAD；只有搭配 `-VerifyOnly -AllowDirtyVerification` 時才可在 dirty 工作樹讀取既有部署，該選項永遠不能授權 push 或 dispatch。
