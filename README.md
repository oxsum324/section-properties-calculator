...
現況鑑定現場紀錄 V0.4.0：入口 `/condition-survey`，支援鏡頭授權與預覽拍攝、部位複選、≤0.3 mm 裂縫快捷或實測值、門窗簡圖、端點／牆線吸附與方向箭頭、原圖與圈註分存、離線紀錄和完整備份。操作與限制見 [field-survey/README.md](field-survey/README.md)。電腦可用 `啟動現況鑑定紀錄.bat`。
