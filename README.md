...
- GitHub Pages live smoke：
  [結構工具箱/tools/pages-live-smoke.js](/C:/Users/USER/Desktop/AI/小工具製作/結構工具箱/tools/pages-live-smoke.js:1)
  - 由 `.github/workflows/pages-live-smoke.yml` 在 `master` push 後等待相同 commit 的 `pages-build-deployment` 成功，再檢查公開首頁、`home.js` 與 `結構工具箱/assets/status/*.json` 是否可讀，避免首頁狀態卡再次依賴未部署的 `output/`。
- 局部快算工具 manifest：
