...
- 載重組合完整向量 regression：
  ...
  - RC 矩形柱設計模式另跨 #6～#11、對稱每邊位置數及 1～4 束筋配置搜尋容量式主筋候選；每列以正式應變相容 P-M 曲線、逐工況二階放大及雙軸容量面篩選，並檢查配筋率與排內可配置性。候選比較只留在工作頁，套用後切至正式檢核，再判定剪力、圍束、錨定、搭接與耐震細節；計算書只保留實際採用配置及正式結果。
- 內力 Picker（內部流程）：
