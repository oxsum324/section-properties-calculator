...
SRC 梁已完成候選垂直切片：純計算核心、現行規範追溯、官方教材回歸、獨立工程候選基準、完整輸入／結果頁、案件 JSON 指紋重播、精簡計算書、內部審閱／正式附件核可、直接列印封鎖及真實瀏覽器 PDF 驗證，並納入 preflight。工程名稱與設計者可留空並由主文承接；附件不因空白表頭欄位被判定為 NG。正式路由、首頁卡片與 release／Pages 證據尚未完成，因此目前仍不對外提供入口。
...
SRC 梁另以 catalog v2 的 `candidateBenchmarks` 登錄為 1 / 1 個非公開候選能力。production 核心以應變相容與二分法求中性軸，獨立 oracle 則在張力筋降伏、壓力筋彈性的明確適用域內用二次方程封閉解重算，並以 4 組案例、41 項斷言核對撓曲、寬厚比、鋼骨／RC 剪力分擔、一般剪力、剪力摩擦與失敗控制。候選頁已具案件 JSON、精簡計算書、核可與真實瀏覽器證據；候選失敗會阻擋整體基準 gate，但在正式 release 升格前仍不計入正式工具 31 / 31，也不建立首頁卡片或 `/src-beam` 路由。
