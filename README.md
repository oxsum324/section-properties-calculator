...
SRC 柱目前是非公開的 `src-column.core.v0.12.0-research` 與私有研究工作頁 V0.7，案件 schema v6。已完成表 3.4-2、x／y 剛度分配、鋼骨雙軸壓彎、RC 應變相容 P-M／P-M-M、第 9.3 節軸力及選定方向的第 8.4.2、9.6.1、9.6.2、9.6.3 節子檢核；Y 向 RC 可依第 5.5.2 節自動計算，Y 向鋼骨則只在專案明確指定 ANSI/AISC 360-22 G6 且確認無扭轉時才自動計算，否則採專案確認值。同一構材可分別記錄 X、Y 核算，產生 14 頁實測的雙向彙總計算書與可重現雙向 JSON；兩方向斷面、材料、主筋、柱長或構造條件不一致時失敗封閉。獨立 oracle 鎖定 174 項精確比對、27 項容許差及七組漂移哨兵。catalog schema v15 已把 SRC 柱構材核算、主文／專案引用輸入及全構架／接頭區／接合細部另案附件明確分工；責任矩陣只留在 HTML，另案項目不會因未塞入本計算書而把構材結果判定 NG。工作頁、單向與雙向報告仍固定 `formalApprovalAllowed=false`、不登錄首頁且不計入正式工具數；型鋼／材證來源政策，以及正式工具清冊、獨立基準與 rendered-delivery 當輪證據完成前，不得開放核可。
