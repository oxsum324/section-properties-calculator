...
SRC 柱目前只建立非公開的 `src-column.core.v0.6.0-research` 可審查核心：依官方第 3、5、6、7、9 章、112 年混凝土結構設計規範與建築研究所教材例題 8／14，自動重算表 3.4-2 翼板／腹板寬厚比、x／y 向剛度分配、鋼骨雙軸壓彎互制、規範重新分配、RC 應變相容 P-M，以及第 9.6.2 節強軸耐震柱剪力子檢核。剪力需求由 Mct、Mcb 與 hn 推得，鋼骨與 RC 依 Mns／Mprc 分別檢核，RC 同時計算一般剪力與剪力摩擦並取較小值；只支援常重混凝土、整體澆置、垂直閉合箍筋與剪力釘貢獻為零。雙軸 RC 核心採精確多邊形裁切，獨立 oracle 改用 320 條數值積分並以連續中性軸另求 1.25Fyr 可能彎矩，共保存 60 項精確比對、8 項容許差比對及三組漂移哨兵。第一批斷面 catalog 鎖定教材附錄表 1-1 的 H500×300 群組 7 筆資料；衝突即阻擋，手動輸入固定複核。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；弱軸剪力、強柱弱梁、圍束、柱腳／接頭、施工階段與其餘耐震細節完成前，不得當作正式附件。
