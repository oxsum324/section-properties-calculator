...
SRC 柱目前是非公開的 `src-column.core.v0.12.0-research` 與私有研究工作頁 V0.7，案件 schema v6。已完成表 3.4-2、x／y 剛度分配、鋼骨雙軸壓彎、RC 應變相容 P-M／P-M-M、第 9.3 節軸力及選定方向的第 8.4.2、9.6.1、9.6.2、9.6.3 節子檢核；Y 向 RC 可依第 5.5.2 節自動計算，Y 向鋼骨則只在專案明確指定 ANSI/AISC 360-22 G6 且確認無扭轉時才自動計算，否則採專案確認值。同一構材可分別記錄 X、Y 核算，產生 14 頁實測的雙向彙總計算書與可重現雙向 JSON；兩方向斷面、材料、主筋、柱長或構造條件不一致時失敗封閉。獨立 oracle 鎖定 174 項精確比對、27 項容許差及七組漂移哨兵。工作頁、單向與雙向報告目前仍固定 `formalApprovalAllowed=false`、不登錄首頁且不計入正式工具數；型鋼來源政策、全構架接頭與接頭區剪力／接合細部的正式附件分工、正式工具清冊、獨立基準與 rendered-delivery 證據完成前，不得開放核可。
