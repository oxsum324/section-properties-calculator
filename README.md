...
SRC 柱目前有非公開的 `src-column.core.v0.8.0-research` 可審查核心與私有研究工作頁：依官方第 3、5、6、7、9 章、112 年混凝土結構設計規範與建築研究所教材例題 8／14，自動重算表 3.4-2 翼板／腹板寬厚比、x／y 向剛度分配、鋼骨雙軸壓彎互制、規範重新分配、RC 應變相容 P-M，以及第 9.3 節耐震軸向強度、第 9.6.2 節強軸耐震柱剪力、第 9.6.1 節單一強軸接頭強柱弱梁與第 9.6.3 節強軸矩形柱圍束子檢核。工作頁已接通強軸單向 P-M 與上述四項耐震子檢核，並提供個別啟用、案件 JSON、計算斷面及乾淨的內部審閱研究計算書；正式附件核可固定封閉。9.3 路徑由第 6.4.1、6.4.3 節重算完全包覆橫箍筋柱受壓強度，明示處理 Fu 上限、活載例外、可選傳力上限與免檢條件；專案拉力強度與接頭傳力容量不得由工具臆測。獨立 oracle 另行重算四項耐震子檢核，共保存 117 項精確比對、14 項容許差比對及五組漂移哨兵。第一批斷面 catalog 鎖定教材附錄表 1-1 的 H500×300 群組 7 筆資料；衝突即阻擋，手動輸入固定複核。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；弱軸耐震路徑、第 9.3 節專案拉力強度與傳力容量來源、第 8.4.2 節接頭傳力比例、全構架所有接頭、柱腳／接頭、施工階段與其餘耐震細節完成前，不得核可為正式附件。
