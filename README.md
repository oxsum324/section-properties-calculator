...
現行 ERH／RVR v4 已將每筆承接驗算的正式模型、控制載重組合、傳力與分配、偏心與二次效應、已檢核極限狀態及其他檢核彙整納入受控指紋；只有需求／承載力比不大於 1 且其他檢核均通過時才判定通過。這是外部正式計算結果的可追溯回簽，不是承接構造容量計算器；下一步為具有明確適用範圍與規範依據的特定承接型式容量計算模組，以及 SRC 構件能力。
