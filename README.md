...
接收單位尚無既有 Ed25519 私鑰時，可執行 `開挖擋土支撐\建立RVR組織簽章金鑰.bat`，在本機建立密碼加密私鑰與公開 RKE 登錄包。來源端匯入 RKE 後仍須透過獨立管道核對組織與 Key ID，持有證明不得自行視為組織身分證明。輪替 RKE 只先建立新舊 Key ID 關聯，不自動撤銷舊金鑰；新金鑰完成測試簽署與使用端切換後，管理者另須在專用流程填寫處理者、變更編號與切換摘要，明確確認後才能撤銷對應舊金鑰。一般金鑰撤銷另須填寫實際原因、處理者與選用的事故編號，並以串接 `RVE-...` 指紋保存不可由畫面覆寫的本機生命週期事件。
