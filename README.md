...
SRC 柱目前只建立非公開的 `src-column.core.v0.4.0-research` 可審查核心：依官方第 3、6、7 章、112 年混凝土結構設計規範與建築研究所教材例題 8，自動重算表 3.4-2 翼板／腹板寬厚比、剛度分配、鋼骨軸壓與壓彎互制、規範重新分配及 RC 應變相容 P-M。第一批斷面 catalog 已逐列核對教材附錄表 1-1（印刷頁 289／PDF 第 301 頁）的完整 H500×300 群組 7 筆資料；catalog 輸入會鎖定尺寸與 A、Ix、Iy、Zx、Zy，衝突即阻擋，手動輸入則固定要求複核。研究 oracle 不引用正式核心、catalog 或共用 P-M 模組，除鋼骨路徑外，另以連續中性軸求解獨立重算 RC P-M，共比對 34 項算術結果；例題鋼筋依斷面圖建成四層，連續解與教材查圖值分開保存。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；catalog 尚未擴充至其他斷面，也不證明現況供貨或取代專案材證，雙向彎矩、柱剪力、柱腳／接頭與耐震細節完成前，不得當作正式附件。
