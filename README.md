...
- RC 工具入口：
  ...
  - RC 梁、柱、板、牆、剪力牆、基礎、單樁與補強斷面，只有案件資料完整且計算狀態 ready 時才可產生「可送簽版」；工具不自行宣稱已成為正式附件，仍須完成公司簽認、技師簽章或專案核准程序。非 ready 的計算書明確標示 `DRAFT／非正式附件`。RC 柱的人工複核需留下依據、文件編號、複核人、時間與計算條件 context key，相關輸入變更後自動失效；完成紀錄會隨專案 JSON 保存並寫入計算書。8 個 RC 正式輸出工作頁的瀏覽器直接列印已完全封鎖，只顯示一頁操作指引，不會輸出表單、結果或 `DRAFT` 浮水印；內部 DRAFT 與可送簽版都必須由「計算書」流程產生。
- 基礎局部檢核：
