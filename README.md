...
補充：多筆復原總覽的「複製精確路徑」只在明確單選、收據仍有效且精確輸出資料夾存在時啟用；按下按鈕或在清單使用 `Ctrl+C` 時會再次確認資格與存在性，只將該列完整路徑寫入 Windows 剪貼簿。表格原生整列複製已停用，無有效選取時快捷鍵只會被攔截；兩種路由都不執行 verifier、不修改附件包，也不形成重建或核可權限。
