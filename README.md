...
Windows 日常操作建議先執行 `結構工具箱/tools/啟動案件附件工作台.bat`。`attachment-governance-hub.ps1` 以「新案或重產後組包／唯讀查看案件狀態／v1、v2 舊包升級」三個目的分流到既有正式附件包管理器、案件附件治理檢視器與舊版附件升級助手；共用起始資料夾可在工作台只選一次，再以 `InitialPath` 預填到所選工具，不必重複瀏覽。此路徑交接不讀取案件內容、不自動檢查、不建立、不升級、不核可，也不另做任何 `ready / review / blocked` 判定；子工具開啟後仍需由使用者明確執行原有檢查或建立動作。三個入口完整保留各自原有權限：管理器只有明確建立時可新建 v3 包，檢視器永遠唯讀，升級助手須通過兩步確認才可另建升級產物。工作台固定提醒工程結果、完整性、治理 ready 與正式附件核可是不同層次，避免把選錯工具或看到 ready 誤當成附件已核可。它不啟動網路服務，三個工作台檔案均不發布至 GitHub Pages；`attachment-governance-hub.contract.test.js` 固定檢查入口完整性、唯讀路由層、路徑只預填不自動執行、UTF-8 BOM 與私有發布邊界。
