...
接收端回簽助手另提供五類補充證據的本機受控範本庫，可重用查核依據、文件編號、版次、日期與頁碼，亦可匯出／匯入或批次套用至同類交接列。schema v2 會保存內容版號、修訂差異、本機審核人、核准時間與有效期限；只有已核准且未過期的範本可用，修訂會自動升版並撤銷核准，外部匯入與舊 v1 範本一律待本機重新核准。範本不保存檔名或 SHA-256，套用後必須重新選取各列實際證據檔，因此不會降低 RVR v5／SEV v2 的逐案證據門檻。
