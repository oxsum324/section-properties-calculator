...
接收單位尚無既有 Ed25519 私鑰時，可執行 `開挖擋土支撐\建立RVR組織簽章金鑰.bat`，在本機建立密碼加密私鑰與公開 RKE 登錄包。來源端匯入 RKE 後仍須透過獨立管道核對組織與 Key ID，持有證明不得自行視為組織身分證明。輪替 RKE 只先建立新舊 Key ID 關聯，不自動撤銷舊金鑰；新金鑰完成測試簽署與使用端切換後，第一人須建立具 `RVE-...` 指紋與 72 小時期限的輪替申請，再由不同姓名的第二人覆核，後端重驗關聯與狀態後才撤銷舊金鑰。姓名與職責目前是程序性聲明，不是帳號或身分提供者驗證。疑似外洩、確認外洩或私鑰遺失仍可立即使用一般撤銷，並以串接事件保存不可由畫面覆寫的本機生命週期紀錄。
