...
日常組包與 v3 正式附件包事後驗證會重新計算 RC、風力／地震共用正式 HTML 及錨栓正式 HTML 的內容／核可封印。錨栓舊版正式 HTML 缺少任一封印時降為 `review`；正文、文件狀態、核可時間、計算指紋或文件標題被改寫時固定 `blocked`。即使同步重算檔案雜湊、附件包清單與包指紋，也不能繞過附件內容的雙封印驗證。
