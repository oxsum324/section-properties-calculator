...
附件整理完成、送出計算附件前，可將附件資料夾拖曳到 `結構工具箱/tools/檢查附件組包.bat`，或執行 `node 結構工具箱/tools/attachment-package-check.js --input <附件資料夾> --project-no <計畫編號>`。它只讀取 PDF、DOCX、XLSX、JSON、HTML 與文字附件，檢查案件資訊、同工具版本、計算指紋重複與頁面專用文字；結果只供內部組包整理，不得附入計算書、列印或 PDF。若需要保留機器可讀結果，另以 `--output <內部檢查結果.json>` 指定存放於附件資料夾外的位置。
