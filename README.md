...
SRC 柱目前只建立非公開的 `src-column.core.v0.5.0-research` 可審查核心：依官方第 3、6、7 章、112 年混凝土結構設計規範與建築研究所教材例題 8，自動重算表 3.4-2 翼板／腹板寬厚比、x／y 向剛度分配、鋼骨雙軸壓彎互制、規範重新分配及 RC 應變相容 P-M。雙軸 RC 路徑要求每支主筋具距左、上緣的 x／y 座標，正式研究核心以精確多邊形裁切建立定軸力 Mux-Muy 互制曲面；獨立 oracle 不引用該模組，改用 320 條數值積分重算，保存 46 項精確比對、3 項雙軸容許差比對及兩組漂移哨兵。第一批斷面 catalog 已逐列核對教材附錄表 1-1（印刷頁 289／PDF 第 301 頁）的完整 H500×300 群組 7 筆資料；catalog 輸入會鎖定尺寸與 A、Ix、Iy、Zx、Zy，衝突即阻擋，手動輸入則固定要求複核。例題鋼筋依斷面圖保存四層及 12 支實際座標，連續解與教材查圖值分開保存。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；catalog 尚未擴充至其他斷面，也不證明現況供貨或取代專案材證，柱剪力、柱腳／接頭、施工階段與耐震細節完成前，不得當作正式附件。
