...
- 平台巡檢儀表板：
  ...
  可直接檢視平台 history、preflight latest / full / quick、近期異常趨勢、耗時與最慢檢查，並固定提醒頁面診斷明細不會寫入計算書、列印或 PDF；採正式附件治理的計算書若非 ready，內部檢討輸出會另帶 `DRAFT／非正式附件` 文件分類並由附件組包檢查阻擋，ready 則在計算書內明確標示 `文件分類｜可送簽版`，但仍須完成公司簽認、技師簽章或專案核准程序。首頁的頁面專用總覽另會顯示最近一次正式放行的實際交付物渲染完成數，但公開快照不包含交付檔名或案件內容。
- 鋼構正式規範工具：
...
- 鋼構正式頁 regression：
  ...
  - 固定檢查連接板、拉力構件、鋼梁與鋼柱的正式頁、共用報表 helper、page-only 產報前檢查與計算書內容邊界；結果區的「輸出正式報表」與頁首輸出都走同一個可追溯報表流程，不直接列印工作頁。四類正式核算皆可匯出及重新匯入機器可讀的計算來源 JSON，與同次正式計算書共用計算指紋；匯入會驗證 schema、工具種類、工具版本與快照指紋，重新計算後只有指紋完全相同才保留套用結果，否則回復原輸入。browser runner 會實際下載來源快照、以真實檔案重新匯入、驗證錯誤版本遭拒並逐頁比對，不只檢查欄位存在。四個鋼構正式工作頁的瀏覽器直接列印已由共用 CSS 封鎖，只顯示一頁操作指引；正式報表彈窗仍可列印或存成 PDF。連接板／拉力構件頁在本機讀取鋼構細部巡檢，公開部署則讀取已發布的平台狀態快照，不再請求私有 `output/audit` 路徑。正式輸出固定以採用輸入、公式代入、結果與結論為主，不輸出輸入模式、換算、流程、介面摘要卡、符號教學或長篇條文說明。browser runner 會實際點擊結果區按鈕，把 HTML 彈窗型計算書轉成 PDF 與可讀文字，也會把四個工作頁各自直接列印成 PDF，確認只有一頁邊界通知且不含表單、案件資料或 `DRAFT`。
- 鋼梁 / 鋼柱既有案件延續頁：
...
- RC 工具入口：
  ...
  - RC 梁、柱、板、牆、剪力牆、基礎、單樁與補強斷面，只有案件資料完整且計算狀態 ready 時才可產生「可送簽版」，並在計算書內顯示精簡的 `文件分類｜可送簽版`；工具不自行宣稱已成為正式附件，仍須完成公司簽認、技師簽章或專案核准程序。非 ready 的計算書明確標示 `DRAFT／非正式附件`。梁、柱、板、牆、剪力牆、基礎與單樁的專案 JSON 重新匯入時會驗證 schema、工具種類、工具版本及來源計算指紋；套用後必須重算出相同指紋才保留結果，否則自動回復原輸入。RC 柱的人工複核需留下依據、文件編號、複核人、時間與計算條件 context key，相關輸入變更後自動失效；完成紀錄會隨專案 JSON 保存並寫入計算書。8 個 RC 正式輸出工作頁的瀏覽器直接列印已完全封鎖，只顯示一頁操作指引，不會輸出表單、結果或 `DRAFT` 浮水印；內部 DRAFT 與可送簽版都必須由「計算書」流程產生。
- 基礎局部檢核：
...
- 風力 / 地震正式工具共同契約測試：
  ...
  - 固定檢查正式工具 manifest、乾淨路由、報表分流、示意圖角色與 HTML 彈窗型計算書可讀文字，避免頁面專用「優先建議報告閱讀狀態」混入列印計算書。14 個正式風力／地震計算書統一由 `core/ui/report.js` 把頁面 readiness 轉成文件狀態：案件識別或複核未完成時輸出精簡 `DRAFT／非正式附件` 與列印浮水印，完整 ready 報告不帶 DRAFT。案件 JSON 匯入固定驗證完全相同的 schema、工具識別、工具版本與來源計算指紋，套用後必須由目前計算核心重算出同一指紋；錯版本、輸入不合格或重算不一致時會保留／回復原輸入。browser smoke 逐工具實測正常 round-trip、錯版本不改畫面及錯指紋套用後回復，並確認匯出來源 JSON 與同次詳算式計算書使用相同計算指紋。14 個工作頁的瀏覽器直接列印另由共用邊界樣式完全封鎖，只輸出一頁操作指引；真正的簡易／詳算計算書仍由頁面上的「列印計算書」流程產生。
- 耐風共用案件檔與一鍵預填契約：
...
- 錨栓報告邊界契約測試：
  ...
  - 包裝 `reportExport` / `reportDocx` / `reportWorkbook` / `attachmentReadiness` / `reportDocumentState` / `backup` 與 `reportArtifacts` 七組 vitest，確認「優先建議報告閱讀狀態」與頁面輔助文字只留在工作頁面，不混入錨栓計算書、列印 PDF 或 workbook/docx 輸出；計算書只保留必要文件分類，整批載重通過、計算版本一致且計畫名稱、案號、公司、設計人、複核人、發行日期完整時為「可送簽版」，待確認、版本不一致、案件資料缺漏或不符合時分別為 `DRAFT / 待人工複核`、`DRAFT / 檢核不符`，且非 ready 檔名附加 `_DRAFT`。工作區 JSON v2 為每一案例保存重算指紋，匯入會在合併前以目前核心重算；錯產品、缺案例或指紋不符時保留原工作區。舊 v1 備份可救回但會撤銷無法覆驗的留痕並維持 DRAFT。正式 release 另會把同一案例的 ready / review / blocked HTML 與 ready DOCX、XLSX 保存在當輪 `anchor-formal` 證據目錄並驗證文件分類。
- 石材報告邊界契約測試：
...
- 工具箱入口合約測試：
  ...
  - `rc-column-report-contract` 會完整渲染三個柱案例與可送簽人工複核案例；其 preflight 專屬上限固定為 `timeoutSeconds = 300`，避免接近 180 秒的正常完整證據被誤判為 timeout。
...
附件整理完成、送出計算附件前，可將附件資料夾拖曳到 `結構工具箱/tools/檢查附件組包.bat`，或執行 `node 結構工具箱/tools/attachment-package-check.js --input <附件資料夾> --project-no <計畫編號>`。它讀取 PDF、DOCX、XLSX、JSON、HTML 與文字附件，會自動略過 `.evidence.json`、成對的文字擷取檔、渲染摘要及 `Thumbs.db`／`desktop.ini` 等系統檔，並將舊案的 `V4.0`、`工具.v1` 等版本值正規化為 `v…` 後比對，檢查案件資訊、同工具版本、來源 JSON 與正式計算書的計算指紋配對、真正重複的來源／輸出及頁面專用文字。同一案件、工具與版本若只有一份來源 JSON 及一份報告，指紋相同視為正常追溯鏈，指紋不同則直接阻擋；多份來源或多份輸出使用相同指紋仍要求人工確認。任何其他未支援格式（例如舊版 `.doc/.xls`、掃描圖片或壓縮檔）都會列出檔名並降為 `review`，不得因未能讀取而靜默略過。含「非正式附件」「內部檢討版」或「不得作為正式附件」等 DRAFT 文件標記時會直接阻擋組包。可辨識為計算書／計算報告，或已具完整產出追溯欄位的文件，必須同時具備計畫名稱、計畫編號、設計人員及 `文件分類｜可送簽版` 才能自動判為可整理；案件識別不完整、未分類或缺少產出工具、版本、輸出時間的附件都只會要求人工確認，不會被靜默放行。專案 JSON 屬來源資料，不要求報告文件分類或完整報告案件識別，但仍接受計畫編號與其餘追溯一致性檢查。命令列採失敗封閉：只有 `ready` 回傳退出碼 `0`；`review` 回傳 `1`、`blocked` 回傳 `2`，參數或執行錯誤回傳 `3`，因此自動流程不會把「需人工確認」誤當成功。結果只供內部組包整理，不得附入計算書、列印或 PDF。若需要保留機器可讀結果，另以 `--output <內部檢查結果.json>` 指定存放於附件資料夾外的位置。
