...
SRC 柱目前有非公開的 `src-column.core.v0.11.0-research` 可審查核心與私有研究工作頁 V0.5：依官方第 3、5、6、7、8、9 章、112 年混凝土結構設計規範與建築研究所教材例題 8／14，自動重算表 3.4-2 翼板／腹板寬厚比、x／y 向剛度分配、鋼骨雙軸壓彎互制、規範重新分配、RC 應變相容 P-M，以及第 9.3 節耐震軸向強度、一次選定 X 或 Y 方向的第 8.4.2、9.6.1、9.6.2、9.6.3 節柱子檢核。工作頁明確限制一份計算書只核算一個方向，提供方向性主筋計算層列、個別啟用、案件 JSON v4、計算斷面及乾淨的內部審閱研究計算書；正式附件核可固定封閉。X 向剪力依式 (5.5-3) 與 (5.5-4)~(5.5-13) 自動重算；Y 向不挪用鋼骨腹板公式，鋼骨弱軸 Vns 仍須由專案確認，RC 剪力則預設依第 5.5.2 節以選定方向的 b、d、b′、Av、Avf、s 與 fyh 自動重算 Vnrc 及 Ash,shear，也保留專案確認模式並相容重播舊 v3 案件，且 H 型鋼 Y 向圍束強制 `Ahcc=0`。研究畫面另以完全獨立模組重現 AISC 360 G6 與官方 Example G.6，供專案明確指定該設計基礎時對照鋼骨弱軸 Vns；對照值不自動採用、不建立 RC 的 Vnrc 或 Ash,shear，也不進計算書、列印、PDF 或計算指紋。第 8.4.2 節會區分 SRC 梁／鋼梁接 SRC 柱，鋼梁路徑預設門檻 1.0，只有專案分析確認平順傳力時才能採 0.7；同時啟用 9.6.1 時，構成分量與梁柱總彎矩不一致即阻擋。第 9.3 節路徑由第 6.4.1、6.4.3 節重算完全包覆橫箍筋柱受壓強度；專案拉力強度與接頭傳力容量不得由工具臆測。獨立 oracle 共保存 162 項精確比對、27 項容許差比對及七組漂移哨兵；外部 G6 模組另有官方算例、catalog 對照、分支與負向測試。第一批斷面 catalog 鎖定教材附錄表 1-1 的 H500×300 群組 7 筆資料；衝突即阻擋，手動輸入固定複核。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；臺灣 SRC 路徑下的鋼骨 Y 向名義剪力強度自動推導、同案兩方向彙總、全構架所有接頭、接頭區剪力與接合細部、柱腳、施工階段與其餘耐震細節完成前，不得核可為正式附件。
