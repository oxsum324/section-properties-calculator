...
SRC 柱目前只建立非公開的 `src-column.core.v0.2.0-research` 可審查核心：依官方第 3、6、7 章與建築研究所教材例題 8，自動重算表 3.4-2 翼板／腹板寬厚比、剛度分配、鋼骨軸壓與壓彎互制、規範重新分配及 RC 應變相容 P-M。另有不引用正式核心或共用 P-M 模組的研究 oracle，獨立比對寬厚比與鋼骨計算路徑；RC P-M 尚未具獨立 oracle。它不登錄首頁、不計入正式工具數且固定維持 `REVIEW`；型鋼斷面性質來源核對、雙向彎矩、柱剪力、柱腳／接頭與耐震細節完成前，不得當作正式附件。
