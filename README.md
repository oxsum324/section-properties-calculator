...
已確認的拆撐處置可另由報表匯出頁建立具來源計算指紋、逐列 `ERT-` 與整包 `ERH-` 完整性指紋的承接構造交接 JSON，已發版本會保存於專案。接收端完成正式模型檢核後，可用 `receiver-capacity-verification-receipt` 回傳逐列結果；來源端會驗證 `RVR-` 指紋、ERH／ERT 關聯、來源計算指紋及完整涵蓋後保存。交接輸出或回簽匯入成功都不會自動推定承接構造正式核可；未使用數位簽章時，回簽單位、人員與正式文件仍須人工核對。
