...
需要在新電腦、專案移動後或捷徑遺失時重建 Windows 入口，可執行根目錄 `安裝案件附件工作台捷徑.bat`；它透過 `install-attachment-governance-shortcuts.ps1` 建立「案件附件工作台」桌面捷徑、同名開始功能表捷徑，以及「以附件工作台檢查」傳送到捷徑。因此可直接按 Windows 鍵搜尋「案件附件工作台」開啟。安裝器可重複執行，會先檢查三個目的地，只更新由本工具管理或已指向同名受治理入口的捷徑；任一處存在同名但屬於使用者的其他捷徑時，會保留原檔並在寫入前停止整批安裝。捷徑不帶固定參數，因此「傳送到」會把當次選取的單一資料夾原樣交給現有工作台封閉驗證。要清除這三個 Windows 入口時，可執行根目錄 `移除案件附件工作台捷徑.bat`；它只刪除帶本工具管理標記或精確指向目前 repo 受治理啟動器的捷徑，不存在可重複執行，同名使用者捷徑與指向其他資料夾同名批次檔的捷徑都會保留。安裝器、移除入口、捷徑與動態合約都是私有本機治理資產，不發布至 GitHub Pages。
