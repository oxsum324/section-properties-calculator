const CACHE = 'condition-survey-shell-0.1.0';
const ASSETS = ['./index.html', './app.css', './app.js', './model.js', './store.js', './bundle.js', './annotation.js', './manifest.webmanifest', './icon.svg', './icon-192.png', './icon-512.png'];
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') { event.respondWith(caches.open(CACHE).then(cache => cache.match('./index.html')).then(response => response || fetch(event.request))); return; }
