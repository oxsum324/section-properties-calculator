import { id, WIDTH_MODES, CRACK_PATTERNS, assert } from './model.js';
...
export function createCrackFields(root, changed, legacyValues) {
  ...
  function read() {
    ...
    const result = [...root.querySelectorAll('[data-crack-id]')].map(card => {
      ...
      return { id: card.dataset.crackId, measured, widthMode: mode, width: measured && mode === 'exact' ? numeric('width') : null, length: measured ? numeric('length') : null, pattern: get('pattern').value, notes: get('notes').value };
    });
  ...
  root.onclick = e => {
    ...
    if (b.hasAttribute('data-remove')) {
      ...
      if (count > 26) return;
      cracks ??= [];
      while (cracks.length < count) cracks.push({ id: id(), measured: false, widthMode: 'unknown', width: null, length: null, pattern: '', notes: '' });
      cracks.length = count;
    }
    render(); changed();
  };
