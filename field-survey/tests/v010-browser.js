...
export async function verifyV010(browser, base, out) {
  ...
  try {
    ...
    await click('#editRecordDate'); await page.locator('#observedOn').fill('2026-09-10'); await click('#saveRecord');
    assert.equal((await current()).records[0].observedOn, '2026-09-10');
    await click('[data-view=case]'); await click('#importRoster'); await page.locator('#rosterText').fill('戶別\t地址\t棟別\t種類\nA-001\t重複戶別\tA\t住戶'); await click('#reviewRoster'); assert(await page.locator('#applyRoster').isDisabled());
    ...
    await click('[data-view=work]'); await click('#editUnit'); await page.locator('#unitForm [name=status]').selectOption('partial'); await page.locator('#unitForm [name=reason]').fill('房間尚未開門'); await page.locator('#unitForm [name=visitDate]').fill('2026-09-12'); await page.locator('#unitForm [name=visitScope]').fill('客廳及廚房'); await click('#unitForm .primary');
    let data = await current(); assert.equal(data.units[0].visitHistory.length, 1); assert.equal(data.records[0].observedOn, '2026-09-10');
    await click('[data-view=case]'); await page.locator('#visitSelect').selectOption(seed.visit); await idle(); await click('[data-view=work]'); await click('#editUnit'); await page.locator('#unitForm [name=status]').selectOption('partial'); await page.locator('#unitForm [name=reason]').fill('樓上待補'); await page.locator('#unitForm [name=visitDate]').fill('2026-09-10'); await click('#unitForm .primary'); data = await current(); assert.equal(data.units[0].visitHistory.length, 2); assert.equal(data.units[0].reason, '房間尚未開門');
    await click('[data-view=review]'); await page.locator('#reviewList details > summary').filter({ hasText: '歷次進場' }).first().click(); assert.match(await page.locator('#reviewList').innerText(), /第 2 次會勘/); await page.locator('#reviewFilter').selectOption('public'); await idle(); assert.equal(await page.locator('.review-unit').count(), 1); await page.locator('#reviewFilter').selectOption(''); await idle();
    await click('[data-view=report]'); await page.locator('#reportPageSize').selectOption('8'); await idle(); await page.locator('#reportAdvanced').evaluate(el => el.open = true); assert.equal(await page.locator('[data-report-record]').count(), 8);
    ...
    await click(first + ' [data-do=edit-detail]'); assert.equal(await page.locator('[data-detail-preset]').count(), 12); assert(!(await page.locator('#modalBody').innerText()).includes('圖層'));
    ...
    await page.locator('#reportAdvanced').evaluate(el => el.open = true); await page.locator('#reportLayout').evaluate(el => el.open = true); await page.locator('#reportPlans').selectOption('2'); await page.locator('#reportPageStart').fill('41'); await page.locator('#reportPublicFloors').check(); await page.locator('#volumeMaxPages').fill('5'); await click('#planVolumes');
