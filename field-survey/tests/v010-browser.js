...
export async function verifyV010(browser, base, out) {
  ...
  try {
    ...
    await click('[data-view=report]'); await page.locator('#reportAdvanced').evaluate(el => el.open = true); assert.equal(await page.locator('[data-report-record]').count(), 8);
    ...
    await context.setOffline(true); await page.reload(); await page.locator('#recordForm').waitFor(); await click('[data-view=report]'); await page.locator('#reportAdvanced').evaluate(el => el.open = true); await click(first + ' [data-do=edit-detail]'); assert.equal(await page.locator('#detailStage svg polyline').count(), 1); await click('#closeModal'); await context.setOffline(false);
