...
export async function verifyFieldV08Workflow(browser, base, out) {
  ...
  try {
    ...
    for (const [i, values] of [[0, ['range0103', '', '1.2']], [1, ['exact', '.45', '2.1']]]) {
      ...
      await card.locator('[data-key=measured]').check(); await card.locator(values[0] === 'exact' ? '[data-exact]' : '[data-narrow]').click();
      if (values[1]) {
    ...
    assert(roundtrip.equal); assert.equal(roundtrip.version, 16); assert.equal(roundtrip.stairs, 5); assert.match(roundtrip.description, /裂縫 B.*0.45 mm.*2.1 m/);
