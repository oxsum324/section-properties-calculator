...
export async function clickSurvey(page, selector) {
  ...
  const ancestors = await target.evaluate(el => {
    const list=[]; for(let p=el.parentElement;p;p=p.parentElement) if(p.tagName==='DETAILS' && !p.open) list.unshift(p.id);
    return list;
  });
  for (const id of ancestors) {
    if (id) await page.locator(`#${id}>summary`).click();
    else await target.evaluate(el => { for(let p=el.parentElement;p;p=p.parentElement) if(p.tagName==='DETAILS' && !p.id) p.open=true; });
  }
