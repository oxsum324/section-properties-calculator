...
export async function verifyV011(browser, base, out) {
  ...
  try {
    ...
    assert.equal(backup.version, 12); assert(backup.equal); assert.equal(backup.layer, 'structural'); assert.equal(backup.countText, '二十餘塊'); assert.equal(backup.hash, backup.sourceHash); assert.deepEqual(errors, []);
