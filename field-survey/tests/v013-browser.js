...
export async function verifyV013(browser, base, out) {
  ...
  try {
    ...
    assert.equal(await count(),1); assert.equal(await page.locator('[data-detail-symbol]').count(),4);assert.equal(await page.locator('[data-detail-tool=tile]').count(),0);
    ...
    assert(reports.output.every(x=>x.hash===reports.expected));assert(reports.equal);assert.equal(reports.version,9);
    ...
    assert.deepEqual(errors,[]);await fs.writeFile(path.join(out,'v0.13-result.json'),JSON.stringify({passed:true,hash:reports.expected,backupVersion:8,errors,physicalPhoneTested:false},null,2));console.log('PASS V0.13 tap lines/regions, vertex/segment edits, four symbols, gestures, responsive canvas, identical PNG/report images and legacy/offline backup');
