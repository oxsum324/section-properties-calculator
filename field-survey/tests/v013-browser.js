...
export async function verifyV013(browser, base, out) {
  ...
  try {
    ...
    await page.reload();await page.locator('#recordForm').waitFor();const original=(await current()).records[0];await click('[data-view=report]'); await page.locator('#reportAdvanced').evaluate(el => el.open = true);await click('[data-do=edit-detail]');
    ...
    assert(reports.output.every(x=>x.hash===reports.expected));assert(reports.equal);assert.equal(reports.version, 15);
    ...
    await page.waitForFunction(()=>document.querySelector('#offlineStatus').textContent==='離線已就緒');await context.setOffline(true);await page.reload();await click('[data-view=report]'); await page.locator('#reportAdvanced').evaluate(el => el.open = true);await click('[data-do=edit-detail]');assert.equal(await count(),7);await click('#closeModal');
