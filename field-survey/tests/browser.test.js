...
import { verifyV012 } from './v012-browser.js';
...
try {
  ...
  assert.equal(await page.locator('#recordIssues').innerText(), '本位置未列出必要欄位待補。'); assert((await page.locator('#widthLabel').innerText()).includes('選填')); assert((await page.locator('#lengthLabel').innerText()).includes('選填'));
  await page.reload(); await page.locator('#crackPattern').waitFor(); assert.equal(await page.locator('#crackPattern').inputValue(), 'network'); assert.equal(await page.locator('#recordIssues').innerText(), '本位置未列出必要欄位待補。');
  ...
  assert.equal(await page.locator('#recordIssues').innerText(), '本位置未列出必要欄位待補。');
  ...
  await page.locator('#crackPattern').selectOption('network'); await click('#saveRecord'); assert.equal(await page.locator('#recordIssues').innerText(), '本位置未列出必要欄位待補。'); assert.equal((await projects())[0].records[0].width, .2);
  await page.screenshot({ path: path.join(out, '06-network-crack-optional.png'), fullPage: true });
  await page.locator('#measured').uncheck(); await click('#saveRecord'); assert.equal(await page.locator('#recordIssues').innerText(), '本位置未列出必要欄位待補。');
