...
export async function verifyV014(browser, base, out) {
  ...
  try {
    ...
    const backup=await page.evaluate(async()=>{const s=await import('./store.js'),b=await import('./bundle.js'),p=(await s.allProjects())[0],bundle=await b.readBundle((await b.makeBundle(p,async id=>(await s.getMedia(id)).blob)).blob);return{version:bundle.manifest.version,detail:bundle.project.records[0].detail};});assert.equal(backup.version, 15);assert.deepEqual(backup.detail,saved.records[0].detail);
