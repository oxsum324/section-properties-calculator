...
export async function verifyV014(browser, base, out) {
  ...
  try {
    ...
    await click('[data-view=report]'); await page.locator('#reportAdvanced').evaluate(el => el.open = true);await click(`[data-report-record="${rid}"] [data-do=edit-detail]`);assert.equal(await page.locator('#detailStage [data-detail-index]').count(),2);await click('#closeModal');
    const backup=await page.evaluate(async()=>{const s=await import('./store.js'),b=await import('./bundle.js'),p=(await s.allProjects())[0],bundle=await b.readBundle((await b.makeBundle(p,async id=>(await s.getMedia(id)).blob)).blob);return{version:bundle.manifest.version,detail:bundle.project.records[0].detail};});assert.equal(backup.version, 16);assert.deepEqual(backup.detail,saved.records[0].detail);
