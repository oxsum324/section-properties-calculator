...
export async function verifyV015(browser, base, out) {
  ...
  try {
    ...
    assert.equal(exported.version, 14);assert.deepEqual(exported.detail,saved.records[0].detail);assert(exported.hashes.every(hash=>hash===exported.expected));assert(!exported.media.includes(seed.photos[0]));assert(!exported.media.includes(seed.photos[2]));
