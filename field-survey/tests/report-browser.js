...
export async function verifyReportWorkflow(browser, base, out) {
  ...
  try {
    ...
    assert.deepEqual(restored, { version: 9, equal: true, hash: seed.hash });
    ...
    await fs.writeFile(path.join(out, 'report-v0.9-result.json'), JSON.stringify({ passed: true, version: VERSION, backupVersion: 7, originalHash: seed.hash, selectedPhotos: 3, physicalPhoneTested: false, checkedAt: new Date().toISOString() }, null, 2));
