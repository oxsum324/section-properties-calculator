...
test('editable openings retain geometry through backup and mirror and reject degenerate or photo-only marks', async () => {
  ...
  const backup = await readBundle((await makeBundle(p, id => blobs.get(id))).blob); assert.equal(backup.manifest.version, 9); assert.deepEqual(backup.project.records[0].detail, r.detail);
...
test('detail signs and regions preserve observed quantities, old strokes, manual text and isolated backup', async () => {
  ...
  const restored = await readBundle((await makeBundle(p, mid => blobs.get(mid))).blob); assert.equal(restored.manifest.version, 9); assert.deepEqual(restored.project.records[0], original);
...
test('expanded conditions and independent observation layers survive backup and both report indexes', async () => {
  ...
  assert.equal(saved.manifest.version, 9); assert.deepEqual(saved.project.records[0], r);
...
test('detail drawings and custom originals survive scoped backup and restore with fresh media identities', async () => {
  ...
  assert.equal(result.manifest.version, 9); r.detail.mediaId = r.photos[0].mediaId; assert.throws(() => validateProject(p), /細部圖原檔/);
...
test('individual cracks preserve independent units, uncertainty and legacy group through backup', async () => {
  ...
  assert.equal(restored.manifest.version, 9);
...
test('multiple conditions share originals and retain independent measured or estimated areas', async () => {
  ...
  assert.equal(result.manifest.version, 9); assert.deepEqual(result.project.records[0], r); assert.equal(result.project.records[0].photos.length, 1);
