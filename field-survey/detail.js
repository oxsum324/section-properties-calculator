...
export async function openDetailEditor(api, recordId) {
  ...
  openModal('細部示意圖', `<div class="detail-controls"><div class="choice-chips"><button id="chooseDetail" aria-expanded="${!record.detail}">底圖</button><button id="mirrorDetail">左右鏡射</button><span id="detailName"></span></div><div id="detailChoices" ${record.detail ? 'hidden' : ''}><div class="detail-presets">${Object.entries(DETAIL_PRESETS).map(([key, name]) => `<button data-detail-preset="${key}">${presetSVG(key)}<span>${name}</span></button>`).join('')}</div><div class="choice-chips">${Object.entries(DETAIL_TEXTS).map(([key, name]) => `<button data-detail-text="${key}">${name}</button>`).join('')}</div><label>自訂底圖<input id="detailFile" type="file" accept="image/png,image/jpeg,image/webp"></label></div><div id="detailDrawControls"><div class="detail-tools">${[['line','裂隙'],['region','範圍'],['symbol','符號'],['text','文字'],['select','選取'],['pan','移動']].map(([key,name])=>`<button data-detail-tool="${key}">${name}</button>`).join('')}</div><div id="detailSymbols" hidden>${Object.entries(DETAIL_SYMBOLS).map(([key,name])=>`<button data-detail-symbol="${key}">${symbolPreview(key)}<span>${name}</span></button>`).join('')}</div><label id="detailRegionLabel" hidden>範圍類型<select id="detailRegion">${Object.entries(REGION_TYPES).map(([key,name])=>`<option value="${key}">${name}</option>`).join('')}</select></label><label id="detailTextLabel" hidden>標記文字<input id="detailText" maxlength="120" placeholder="例如裂縫 A"></label><div id="detailSelection" hidden><label>選取標記<select id="detailObject"></select></label><div class="choice-chips"><button id="detailDelete">刪除這筆</button><button id="detailDeleteSegment">刪除這段</button><button id="detailDeletePoint">刪除端點</button><button id="detailCopy">複製</button></div><div id="detailSymbolActions" class="choice-chips"><button id="detailShrink">符號縮小</button><button id="detailGrow">符號放大</button><button id="detailRotate">旋轉 15°</button></div><details><summary>位置微調</summary><div class="choice-chips">${[['-1,0','←'],['1,0','→'],['0,-1','↑'],['0,1','↓']].map(([dir,name])=>`<button data-detail-nudge="${dir}">${name}</button>`).join('')}</div></details></div><details id="detailMore"><summary>其他畫法／說明</summary><div class="choice-chips">${[['pen','自由手繪'],['arrow','兩點箭頭'],['circle','兩點圈選'],['rect','兩點範圍框'],['erase','快速刪除']].map(([key,name])=>`<button data-detail-tool="${key}">${name}</button>`).join('')}</div><p class="micro">裂隙逐點連線，範圍至少三點後完成；雙指縮放，移動模式可單指平移。符號僅示意，不代表實測範圍或量測值。</p></details></div></div><div class="detail-workspace"><div id="detailNavigation" class="choice-chips"><button id="detailZoomOut" aria-label="縮小圖面">−</button><button id="detailZoomIn" aria-label="放大圖面">＋</button><button id="detailFit">全圖</button><span id="detailLegend" class="micro"></span></div><div id="detailStage"></div><p id="detailTextPreview" hidden></p><p id="detailHint" class="micro" role="status"></p></div><div class="detail-dock"><button id="detailUndo">復原</button><button id="detailRedo">重做</button><button id="detailFinish">完成筆畫</button><button id="detailCancel">取消筆畫</button><button id="downloadDetail" class="secondary">下載 PNG</button><button id="saveDetail" class="primary">保存</button></div>`, cleanup);
  ...
  function state(st) {
    ...
    $('#detailHint').textContent = st.message || (st.pending ? `已點 ${st.count} 點；完成或取消目前筆畫。` : ({line:'依序點起點與轉折點，再按完成。',region:'點選範圍角點，再按完成。',symbol:'點圖面放置符號；選取後可移動及縮放。',select:'點標記附近選取；可拖動整筆或藍色端點。',pan:'單指移動；雙指縮放。',pen:'按住拖曳手繪；雙指可縮放。',arrow:'依序點起點與箭頭終點。',circle:'點兩個對角畫圈。',rect:'點兩個對角畫範圍。',text:'填文字後，點圖面放置。',erase:'點線條附近刪除一筆，可復原。'}[st.mode] || ''));
  ...
  function confirmReplace() { assert(!editor?.pending, '請先完成或取消目前筆畫'); remember(); return !detail.marks?.length || confirm('更換底圖會清除目前損害標註。繼續更換？取消此視窗仍可保留原已存紀錄。'); }
