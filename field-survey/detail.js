...
export async function openDetailEditor(api, recordId) {
  ...
  openModal('細部示意圖', `<div class="detail-controls"><p class="detail-context micro" title="${e(api.contextLabel || '')}">${e(api.contextLabel || '本筆位置的照片共用細圖')}</p><div class="choice-chips"><button id="chooseDetail" aria-expanded="${!record.detail}">底圖</button><button id="mirrorDetail">左右鏡射</button></div><div id="detailChoices" ${record.detail ? 'hidden' : ''}>${DETAIL_PRESET_GROUPS.map(([group, keys]) => `<p class="micro detail-preset-group">${e(group)}</p><div class="detail-presets">${keys.map(key => `<button data-detail-preset="${key}">${presetSVG(key)}<span>${e(DETAIL_PRESETS[key])}</span></button>`).join('')}</div>`).join('')}<div class="choice-chips">${Object.entries(DETAIL_TEXTS).map(([key, name]) => `<button data-detail-text="${key}">${name}</button>`).join('')}</div><label>自訂底圖<input id="detailFile" type="file" accept="image/png,image/jpeg,image/webp"></label></div><details id="detailLinkage"><summary>照片內容與細圖對照 <span id="detailCompareCount"></span></summary><div id="detailComparison"></div><div id="detailSuggested" class="choice-chips">${detailComparison(record).conditions.filter(c=>!['normal','other'].includes(c)).map(c=>`<button data-detail-suggest="${c}">補畫${e(CONDITIONS[c])}</button>`).join('')}</div><label>細圖補充說明<textarea id="detailNote" rows="2" maxlength="2000" placeholder="例如：裂縫 A 位於窗角；磁磚破損在其下方，詳本位置近照。">${e(detail.note || '')}</textarea></label><p class="micro">保存後列於照片內容下方，同位置照片共用。</p></details><div id="detailDrawControls"><div class="detail-tools">${[['door','＋ 門框'],['window','＋ 窗框'],['line','裂隙'],['region','範圍'],['symbol','圖示'],['text','文字'],['select','選取'],['pan','移動']].map(([key,name])=>`<button data-detail-tool="${key}">${name}</button>`).join('')}</div><div class="choice-chips detail-options"><label id="detailToneLabel" class="check-label">線色<select id="detailTone">${Object.entries(MARK_TONES).map(([key,name])=>`<option value="${key}">${name}</option>`).join('')}</select></label><label class="check-label"><input id="detailQuad" type="checkbox">四點門窗框（斜面）</label></div><div id="detailSymbols" hidden>${Object.entries(DETAIL_SYMBOLS).map(([key,name])=>`<button data-detail-symbol="${key}">${symbolPreview(key)}<span>${name}</span></button>`).join('')}</div><label id="detailRegionLabel" hidden>範圍類型<select id="detailRegion">${Object.entries(REGION_TYPES).map(([key,name])=>`<option value="${key}">${name}</option>`).join('')}</select></label><label id="detailTextLabel" hidden>標記文字<input id="detailText" maxlength="120" placeholder="例如裂縫 A"></label><div id="detailSelection" hidden><label>選取標記<select id="detailObject"></select></label><div class="choice-chips"><button id="detailDelete">刪除這筆</button><button id="detailDeleteSegment">刪除這段</button><button id="detailDeletePoint">刪除端點</button><button id="detailCopy">複製</button></div><div id="detailSymbolActions" class="choice-chips"><button id="detailShrink">圖示縮小</button><button id="detailGrow">圖示放大</button><button id="detailRotate">旋轉 15°</button></div><details><summary>位置微調</summary><div class="choice-chips">${[['-1,0','←'],['1,0','→'],['0,-1','↑'],['0,1','↓']].map(([dir,name])=>`<button data-detail-nudge="${dir}">${name}</button>`).join('')}</div></details></div><details id="detailMore"><summary>其他畫法／說明</summary><div class="choice-chips">${[['pen','自由手繪'],['arrow','兩點箭頭'],['circle','兩點圈選'],['rect','兩點範圍框'],['erase','快速刪除']].map(([key,name])=>`<button data-detail-tool="${key}">${name}</button>`).join('')}</div><p class="micro">門窗以立面開口框表示，可在預設或自訂底圖補畫多個；兩個對角調整寬高，不代表實測尺寸。內建斜視底圖點兩個對角時會自動貼合該面牆；自訂照片或其他斜面請勾「四點門窗框」，依序點四個角。「展開立面（2D）」底圖為正投影，門窗框直接是正矩形。線色只在螢幕上區分（紅＝裂縫、藍＝水分／其他損害、黑＝補畫底圖線），下載 PNG 與附件預設為黑白線稿，以粗細、虛線與網點區分。裂隙逐點連線，範圍至少三點後完成；雙指縮放，移動模式可單指平移。圖示僅示意，不代表實測範圍或量測值。</p></details></div></div><div class="detail-workspace"><div id="detailNavigation" class="choice-chips"><button id="detailZoomOut" aria-label="縮小圖面">−</button><button id="detailZoomIn" aria-label="放大圖面">＋</button><button id="detailFit">全圖</button><span id="detailLegend" class="micro"></span></div><div id="detailStage"></div><p id="detailTextPreview" hidden></p><p id="detailHint" class="micro" role="status"></p></div><div class="detail-dock"><button id="detailUndo">復原</button><button id="detailRedo">重做</button><button id="detailFinish">完成筆畫</button><button id="detailCancel">取消筆畫</button><button id="downloadDetail" class="secondary">下載 PNG</button><button id="saveDetail" class="primary">保存</button></div>`, cleanup);
  ...
  function updateComparison(d = detail) {
    ...
    $('#detailCompareCount').textContent = hints.length ? `（${hints.length} 項待核對）` : '';
  }
  function state(st) {
    ...
    $('#detailHint').textContent = st.message || (st.pending ? (['door','window'].includes(st.mode) ? (st.openingPoints === 4 ? `已點 ${st.count} 個角，依序點滿四個角完成開口框；或按取消筆畫。` : '再點另一個對角完成開口框；或按取消筆畫。') : `已點 ${st.count} 點；完成或取消目前筆畫。`) : ({door: st.openingPoints === 4 ? '依序點開口的四個角放置門框（適合斜面）；選取後可拖動各角。' : '點開口的兩個對角放置門框；斜視底圖會自動貼合所點的那面牆，自訂照片請勾四點框。選取後可拖動各角。',window: st.openingPoints === 4 ? '依序點開口的四個角放置窗框（適合斜面）；選取後可拖動各角。' : '點開口的兩個對角放置窗框；斜視底圖會自動貼合所點的那面牆，自訂照片請勾四點框。選取後可拖動各角。',line:'依序點起點與轉折點，再按完成。',region:'點選範圍角點，再按完成。',symbol:'點圖面放置圖示；選取後可移動及縮放。',select:'點標記附近選取；可拖動整筆或藍色端點。',pan:'單指移動；雙指縮放。',pen:'按住拖曳手繪；雙指可縮放。',arrow:'依序點起點與箭頭終點。',circle:'點兩個對角畫圈。',rect:'點兩個對角畫範圍。',text:'填文字後，點圖面放置。',erase:'點線條附近刪除一筆，可復原。'}[st.mode] || ''));
  ...
  async function paint() {
    ...
    if(disposed) { next.dispose(); return; } editor = next; editor.setText($('#detailText').value); editor.setCondition($('#detailRegion').value); editor.setTone($('#detailTone').value); editor.setOpeningPoints($('#detailQuad').checked ? 4 : 2); $('#detailSymbols').hidden = true;
  }
  ...
  for(const b of $('#modalBody').querySelectorAll('[data-detail-symbol]')) b.onclick = () => { editor?.setSymbol(b.dataset.detailSymbol); $('#detailSymbols').hidden = true; $('#detailHint').textContent = DETAIL_SYMBOLS[b.dataset.detailSymbol] + '：點圖面放置。'; };
  $('#detailNote').oninput = () => { detail.note = $('#detailNote').value; setDirty(true); updateComparison(); };
  $('#detailSuggested').onclick = event => { const b = event.target.closest('[data-detail-suggest]'); if (!b || !editor) return; const c = b.dataset.detailSuggest; const symbol = c === 'crack' ? 'network' : c, mode = c === 'crack' && record.crackPattern !== 'network' ? 'line' : Object.hasOwn(DETAIL_SYMBOLS, symbol) ? 'symbol' : 'text'; if (!editor.setMode(mode)) return; if (mode === 'symbol') editor.setSymbol(symbol); if (mode === 'text') { $('#detailText').value = CONDITIONS[c]; editor.setText(CONDITIONS[c]); } $('#detailSymbols').hidden = true; $('#detailLinkage').open = false; };
