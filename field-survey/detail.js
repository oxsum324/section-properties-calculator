...
export async function openDetailEditor(api, recordId) {
  ...
  function state(st) {
    ...
    $('#detailHint').textContent = st.message || (st.pending ? (['door','window'].includes(st.mode) ? '再點另一個對角完成開口框；或按取消筆畫。' : `已點 ${st.count} 點；完成或取消目前筆畫。`) : ({door:'點開口的兩個對角放置門框；選取後拖動或調整藍色對角。',window:'點開口的兩個對角放置窗框；選取後拖動或調整藍色對角。',line:'依序點起點與轉折點，再按完成。',region:'點選範圍角點，再按完成。',symbol:'點圖面放置符號；選取後可移動及縮放。',select:'點標記附近選取；可拖動整筆或藍色端點。',pan:'單指移動；雙指縮放。',pen:'按住拖曳手繪；雙指可縮放。',arrow:'依序點起點與箭頭終點。',circle:'點兩個對角畫圈。',rect:'點兩個對角畫範圍。',text:'填文字後，點圖面放置。',erase:'點線條附近刪除一筆，可復原。'}[st.mode] || ''));
